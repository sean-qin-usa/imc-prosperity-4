...
class Trader:
    ...
    # Baskets — fixed thresholds, no hedging.  Asymmetry (+80 upper vs
    # -40 lower) reflects the real positive-mean premium of the P3 R2
    # distribution (see P3R2_RECIPE.md §2).  Sensitivity sweep: ±10 %
    # PnL for ±25 % parameter moves — healthy plateau per playbook §8.
    B1_UPPER = 80.0
    B1_LOWER = -40.0
    B2_UPPER = 80.0
    B2_LOWER = -40.0
    ...
    # Stop-reduce: when spread runs past STOP_MULT · threshold in the
    # adverse direction while we're already at the position limit,
    # trim 25 % into the adverse side.  Caps runaway-loss days.
    BASKET_STOP_MULT = 2.0
    ...
    # ---- Basket arb (v3: v1 thresholds + stop-reduce) ----
    def _basket_orders(self, od_basket, od_legs, weights, pos_basket, limit, upper, lower):
        """Fixed-threshold basket spread trade with runaway-loss stop-reduce."""
        ...
        # Stop-reduce: if we're long and spread keeps dropping past 2·lower,
        # or short and spread keeps rising past 2·upper, trim 25 %.
        stop_up = upper * self.BASKET_STOP_MULT
        stop_dn = lower * self.BASKET_STOP_MULT
        if spread > stop_up and pos_basket < 0:
            trim = min(abs(pos_basket) // 4 + 1, bb["sells"].get(basket_ask, 0))
            if trim > 0:
                orders.append(Order(prod_name, basket_ask, trim))
                return orders
        elif spread < stop_dn and pos_basket > 0:
            trim = min(pos_basket // 4 + 1, bb["buys"].get(basket_bid, 0))
            if trim > 0:
                orders.append(Order(prod_name, basket_bid, -trim))
                return orders
