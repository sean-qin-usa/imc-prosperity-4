...
class Trader:
    ...
    def _append_chunked_order(
        self, orders: List[Order], product: str, price: int, quantity: int, chunk_size: int
    ) -> None:
        qty = int(quantity)
        if qty == 0:
            return
        ...
        while remaining > 0:
            ...
            orders.append(Order(product, int(price), sign * piece))
            remaining -= piece
    ...
    def _trade_aco(
        ...
        for ask, vol in sells.items():
            ...
            if ask <= skewed - self.ACO_TAKE_EDGE:
                ...
                if qty > 0:
                    self._append_chunked_order(
                        orders, product, ask, qty, self.ACO_ORDER_CHUNK
                    )
                    pos += qty
        ...
        for bid, vol in buys.items():
            ...
            if bid >= skewed + self.ACO_TAKE_EDGE:
                ...
                if qty > 0:
                    self._append_chunked_order(
                        orders, product, bid, -qty, self.ACO_ORDER_CHUNK
                    )
                    pos -= qty
        ...
        if timestamp >= self.ACO_LATE_UNWIND_START:
            if (
                ...
                if qty > 0:
                    self._append_chunked_order(
                        orders, product, best_bid, -qty, self.ACO_ORDER_CHUNK
                    )
                    pos -= qty
            elif (
                pos < -self.ACO_LATE_UNWIND_TARGET
                ...
                if qty > 0:
                    self._append_chunked_order(
                        orders, product, best_ask, qty, self.ACO_ORDER_CHUNK
                    )
                    pos += qty
        ...
        if bid_px < ask_px:
            if bid_sz > 0:
                self._append_chunked_order(
                    orders, product, bid_px, bid_sz, self.ACO_ORDER_CHUNK
                )
            if ask_sz > 0:
                self._append_chunked_order(
                    orders, product, ask_px, -ask_sz, self.ACO_ORDER_CHUNK
                )
    ...
    def _trade_ipr(
        ...
        if timestamp <= self.IPR_EARLY_TAKE_WINDOW:
            for ask, vol in sells.items():
                ...
                if qty > 0:
                    self._append_chunked_order(
                        orders, product, ask, qty, self.IPR_ORDER_CHUNK
                    )
                    pos += qty
        ...
        if excess > 0 and best_bid >= benchmark + self.IPR_BAND_SELL_EDGE:
            ...
            if qty > 0:
                self._append_chunked_order(
                    orders, product, best_bid, -qty, self.IPR_ORDER_CHUNK
                )
                pos -= qty
                sold_this_tick = True
        ...
        if pos < limit and best_ask <= benchmark + self.IPR_BAND_RELOAD_EDGE:
            ...
            if qty > 0:
                self._append_chunked_order(
                    orders, product, best_ask, qty, self.IPR_ORDER_CHUNK
                )
                pos += qty
        ...
        if (not sold_this_tick) and pos < limit and spread > 2:
            ...
            if bid_qty > 0 and bid_px < best_ask:
                self._append_chunked_order(
                    orders, product, bid_px, bid_qty, self.IPR_ORDER_CHUNK
                )
                ...
                if (
                    ...
                    if comp_px > bid_px:
                        ...
                        if comp_qty > 0:
                            self._append_chunked_order(
                                orders, product, comp_px, comp_qty, self.IPR_ORDER_CHUNK
                            )
