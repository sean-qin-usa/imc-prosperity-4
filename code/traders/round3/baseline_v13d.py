"""
Round 3 baseline v13b — adaptive-anchor variant.

Same logger as v13, but a different fix philosophy:

  H_ANCHOR is no longer a fixed constant 9990. It tracks an EMA of
  touch_mid with τ ≈ 5000 ticks (α = 1/5000). The hard CLIP=30
  remains, but it's now relative to the running EMA rather than a
  static prior. This means:

    - In a stationary regime (the 3 backtest days), the EMA hovers
      near 9990 and behavior is essentially identical to v12.
    - In a drift regime (live day-2 style 100-tick excursion), the
      anchor follows the drift gradually. Fair eventually catches
      up to the new mid; we stop accumulating against the trend.

  Tradeoff vs v13: simpler (just one new piece of state, no
  regime-conditional branches in TAKE/MAKE), but slower to react —
  EMA half-life ≈ 3500 ticks, so the first 3-5k ticks of a drift
  still bleed before fair catches up. v13's hard regime gate stops
  the bleed at the first tick the threshold is crossed.

  This file is for A/B comparison: which fix philosophy preserves
  more in-sample alpha? Pick whichever wins, or compose both.
"""
...
class Trader:
    ...
    H_ANCHOR_INIT = 9990.0
    H_ANCHOR_ALPHA = 1.0 / 20000.0  # EMA mixing (half-life ~14k ticks)
                                    # ~1.4 days half-life — slow enough to
                                    # not wander on stationary noise, fast
                                    # enough to track a multi-day drift
    H_CLIP = 30.0
    H_TAKE_EDGE = 0.0
    H_REDUCE_EDGE = 1.0
    H_PENNY_EDGE = 1.5
    ...
    def _trade_hydrogel(self, od: OrderDepth, pos: int,
                        prev_mid: Optional[float], anchor: float
                        ) -> Tuple[List[Order], float, float, Dict]:
        prod = "HYDROGEL_PACK"
        limit = self.LIMITS[prod]
        book = self._book(od)
        if not book:
            return [], prev_mid if prev_mid is not None else 0.0, anchor, {}
        bb, ba = book["bb"], book["ba"]
        spread = book["spread"]; touch_mid = book["touch_mid"]

        # Update EMA anchor
        new_anchor = (1 - self.H_ANCHOR_ALPHA) * anchor + self.H_ANCHOR_ALPHA * touch_mid
        ...
        if shock:
            ...
            fair_adj = max(-self.H_CLIP, min(self.H_CLIP, touch_mid - new_anchor))
            fair = new_anchor + fair_adj
        ...
        log_state = {
            "fair": round(fair, 1),
            "anchor": round(new_anchor, 1),
            "drift": round(touch_mid - new_anchor, 1),
            "pos_in": pos,
            "pos_after_take": working,
        }

        if shock:
            return orders, touch_mid, new_anchor, log_state
    ...
    def run(self, state: TradingState):
        ...
        anchor = saved.get("h_anchor", self.H_ANCHOR_INIT)

        if "HYDROGEL_PACK" in state.order_depths:
            prev_mid = saved.get("h_prev_mid")
            h_orders, new_mid, new_anchor, h_state = self._trade_hydrogel(
