"""
Round 3 combined SHIP v24 — 2026-04-25 (session 8 + Citadel port).

DELTA vs v23 (451,331 bt, alpha 280,836) — replaces VFE UNDER_MR + drift
carry with Citadel-style bipolar long-EMA z-score MR (port from dad.py).

  - Long EMA on VFE mid (alpha=0.0005, HL ≈ 1386 ticks)
  - Variance EMA on (mid - ema)² (sigma_alpha=0.0005, sigma_floor=1.0)
  - z = (mid - ema) / sigma; warmup 500 ticks before any trade
  - When z ≥ 2.0, side=-1 (target -LIMIT); when z ≤ -2.0, side=+1
  - Sticky side; flip only on opposite cross.  Take-aggressive up to
    80/tick to reach target.

3-day backtest match-trades all:  +508,770 (155,606 / 174,004 / 179,159)
3-day backtest match-trades none: +352,580 (alpha floor)
vs v23 (451,331 / 280,836):  +57,439 bt / +71,744 ALPHA
vs dad.py (485,676 bt, v11 chassis + Citadel only):  +23,094 bt

The Citadel layer is REAL alpha, not matching-engine artifact —
+71,744 of pure alpha-floor gain over v23, indicating the slow z-score
identifies clean MR opportunities the short-window UNDER_MR missed.

Citadel param sweep on v24 chassis (all peaks at dad.py defaults):
  VFE_Z_ENTER 1.5→468k, 1.75→495k, **2.0→508k**, 2.25→451k, 3.0→437k
  VFE_EMA_ALPHA 1e-4→442k, 5e-4→**508k**, 1e-3→504k, 2e-3→481k
  MAX_TAKE 40→508k, 60→509k (+113), 80→508k, 100+→508k saturated
  WARMUP 200→494k, 500→508k, 1000→492k

What v24 keeps from v23 (cumulative session 4-8 work):
  - HYDROGEL h_only_v16 retune + 4-knob v12 micro-tune
  - **HYDROGEL drift-regime CLIP gate** (v15_hdrift, +1,964 bt / +1,130 alpha)
  - **Asymmetric INV_SKEW** (LONG=-0.004, SHORT=0.014, +4,289 bt)
  - VEV drift carry V5000-5300 target=300 (+1,594 bt)
  - VFE multi-level passive carry (in v20; superseded by Citadel)
...
What was REPLACED by Citadel:
  - Timo's short-window underlying MR (cross-take when |ema_o_dev| > 6)
  - v15 VFE passive drift carry (long-bias when |ema_o_dev| < 6)
  - Both subsumed by Citadel's bipolar slow-EMA z-score regime.

Live forecast: dad.py (Citadel only) was tagged "highest live-conversion
RISK" because directional position bet has unknown live conversion vs
the ladder.  v24 stacks Citadel on v23's passive-make sleeves which
DO convert at 1.0-2.27x.  Best-case live ~470k; worst-case (Citadel
converts at 0.5x) ~430k.  Either way, ship as session-8 final aggressive.
