...
Idea:
- Build a real-time residual surface factor from `VEV_5000..VEV_5500`
  against `VELVETFRUIT_EXTRACT`.
- Interpret that factor as chain richness / cheapness versus the
  underlying, not as a standalone ATM trading signal.
- Feed the factor into the proven deep-ITM sleeve by biasing fair
  lower when the surface is rich and higher when the surface is cheap.
- Make buying harder when the chain is rich and selling harder when the
  chain is cheap.

This directly targets the voucher/extract relationship without forcing a
spot-vs-voucher arb or an ATM residual mean-reversion trade.
"""
...
_SPEC = spec_from_file_location("_fundamental_v1_base_surface_level", _BASE_PATH)
...
class Trader(BaseTrader):
    ...
    # Positive factor = calls rich vs extract -> lean short vouchers.
    SURF_FAIR_BETA = 0.60
    SURF_EDGE_WIDEN = 0.35
    ...
    def _update_surface_level(
        ...
        for name, strike in self.SURF_STRIKES.items():
            ...
            mean_key = f"surf_mean_{name}"
            scale_key = f"surf_scale_{name}"
    ...
    def _trade_deep_itm(
        ...
        fair -= self.SURF_FAIR_BETA * surface_level

        buy_extra = base_extra_edge + self.SURF_EDGE_WIDEN * max(surface_level, 0.0)
        sell_extra = base_extra_edge + self.SURF_EDGE_WIDEN * max(-surface_level, 0.0)
