...
class Trader:
    ...
    VFE_RESET_INTERVAL = 1000
    VFE_RESET_KEEP_SIGMA = True  # keep sigma_var/ticks; only reset ema+side
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        # Anchor-only reset: re-derive EMA from local mid, drop sticky side,
        # but keep sigma_var (avoid post-reset z-explosion from tiny sigma).
        anchor_reset_now = (
            self.VFE_RESET_INTERVAL > 0 and ticks >= self.VFE_RESET_INTERVAL
        )
        if anchor_reset_now:
            ema = None
            side = 0
            ticks = 0  # always reset ticks so the reset doesn't fire every call
            if not self.VFE_RESET_KEEP_SIGMA:
                sigma_var = 0.0
