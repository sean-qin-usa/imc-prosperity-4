"""
Round 3 — Timo-style clone v3.

Track record:
  v1 (THR=0.5, strikes 5000-5300, no per-tick cap): 198,652
  v2 (THR=0.3, strikes 5000-5300, per-tick=30):    206,240
  v3 ↓                                              [target]

v3 changes:
  - Drop strikes 5200/5300 (per-strike PnL was net negative in v2).
  - Keep just K=5000, 5100 — both clearly profitable per-day.
  - Add switch_mean gate (Timo's): SCALP_SW_THR = 0.15 (calibrated to
    our residual std ~0.4, vs Timo's 0.7 calibrated to P3 VR options).
  - Pre-flatten existing positions when signal regime is calm
    (switch_mean below threshold) — helps avoid carrying a position
    into a non-tradable regime.
"""
...
SCALP_SW_WINDOW = 100      # rolling vol of |residual|
SCALP_SW_THR = 0.15        # "regime active" gate; below = don't trade
...
class Trader:
    ...
    def _trade_iv_scalp(self, name, K, od, pos, S, T, saved):
        ...
        sw = self._ema(saved, f"_iv_sw_{name}", SCALP_SW_WINDOW, abs(diff - mdiff))

        # Calm-regime gate: skip new trades, but flatten any held position
        # (regime change usually means we were caught wrong-side on signal).
        if sw < SCALP_SW_THR:
            orders: List[Order] = []
            if pos > 0:
