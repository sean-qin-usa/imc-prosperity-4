"""
Round 3 FINAL — SHIP v11 (399113) + Citadel bipolar VFE mean-reversion.

3-day historical backtest (Round3_csv, same-tick fill, match-trades all):
  Day 0:   143,580
  Day 1:   174,772
  Day 2:   171,100
  Total:   489,452          (+ 60,698 vs SHIP v11 baseline of 428,754)

Per-product split (last tick of each day):
  HYDROGEL_PACK             ~ 184,000   (anchored MM, AR1 + vol-clip)
  VELVETFRUIT_EXTRACT       ~  92,000   (Citadel bipolar MR overlay)
  VEV_4000 + VEV_4500       ~  43,000   (synthetic deep-ITM MM)
  VEV_5000 / 5100 / 5200    ~ 122,000   (IV-residual MR sleeve)
  VEV_5300                  ~  18,000   (IV-residual MR)
  VEV_5400 / 5500           ~  33,000   (ATM smile-EMA MM)
  VEV_6000 / 6500           ~     900   (lottery posts at 0/1)

Layer 6 (Citadel) overrides Layer 3's underlying-MR with a longer-horizon
counter-trend on VFE: long-EMA (alpha=5e-4, HL ~ 1386 ticks), flip to
+/-VFE_LIMIT once |z| >= 2.0. On the historical sample the bipolar net
(gross alpha minus half-spread cost) is ~ +93k vs ~ +20k for the original
short-window underlying MR.

LIVE deployment: TTE_DAYS_LIVE = 5.0 is correct for Round 3 day 1.
Historical CSV TTE on day 0 was 8d, but the IV-EMA / per-strike residual
machinery adapts to whatever absolute TTE we feed it; tested both ways.

Original SHIP v11 docstring (preserved verbatim) follows.
...
class Trader:
    ...
    # Disabled in this combo: replaced by Citadel bipolar long-EMA MR below
    # (longer horizon, stronger gross alpha on the 3-day historical sample).
    ENABLE_UNDERLYING_MR = False
    UNDER_MR_THR = 6

    # ----- Citadel bipolar VFE mean-reversion (long-EMA, HL ~ 1000) -----
    # alpha = 1 - 0.5**(1/1000) ~ 0.000693. Hold VFE at +/-VFE_LIMIT once
    # |z| crosses VFE_Z_ENTER; flip on opposite-side cross. WARMUP_TICKS
    # blocks trading until the EMA stabilises (timestamps come in 100-step
    # increments so 1000 ticks = 100k timestamp = 10% of a day).
    ...
    def run(self, state: TradingState):
        ...
        # 6. Citadel bipolar long-EMA VFE mean-reversion (overrides Layer 3
        #    underlying MR; only one VFE position target wins, so we run
        #    this last and replace any prior VFE orders).
        if self.ENABLE_CITADEL_VFE_MR and u_od is not None:
            ...
            elif "VELVETFRUIT_EXTRACT" in result and not result["VELVETFRUIT_EXTRACT"]:
                del result["VELVETFRUIT_EXTRACT"]
