"""
Round 3 combined SHIP v22 — 2026-04-25 (session 8 + hdrift stack).

DELTA vs v15_hdrift (445,448 bt) — adds VEV drift carry on V5000-5300
(target=300, multi-level bb / bb-1, gated by no MR signal).  Same
mechanism as v21 over v20.

3-day backtest match-trades all:  TBD
3-day backtest match-trades none: TBD
vs v15_hdrift: TBD

Stacks two independent improvements:
  - HYDROGEL drift-regime gate (v15_hdrift): when 200-tick touch_mid
    range > 53, multiply CLIP_VOL_K by 2.5. +1,964 bt.
  - VEV passive drift carry (v21): post bid stack on V5000-5300 in
    no-MR ticks, target 300 long. +1,594 bt over the v20 chassis.

Both pass --match-trades none alpha-floor unchanged → both are
matching-engine fill mechanics that should convert positively in live.
