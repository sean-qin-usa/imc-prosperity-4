"""
Round 3 combined SHIP v4 — 2026-04-25 (session 4).
...
class Trader:
    ...
    H_WIDE_SPREAD = 8
    # Layer-2 deeper passive quotes (v62 experiment for live-fill upside)
    H_LAYER2_OFFSET = 5  # ticks deeper than primary layer
    H_LAYER2_SIZE = 30
    AR1_BETA = 0.18
    TYPICAL_SPREAD = 16
    CLIP_VOL_K = 0.3
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist):
        ...
        if bid_price < ask_price:
            ...
            # v62: layer-2 deeper passive quotes. Costs nothing in
            # backtester (visible book doesn't reach that depth) but may
            # capture extra fills in live where the matching engine
            # gives passive quotes more flow than backtester models.
            if self.H_LAYER2_SIZE > 0:
                deep_bid = bid_price - self.H_LAYER2_OFFSET
                deep_ask = ask_price + self.H_LAYER2_OFFSET
                deep_buy_cap = max(0, buy_cap - bid_size)
                deep_sell_cap = max(0, sell_cap - ask_size)
                deep_bid_size = min(self.H_LAYER2_SIZE, deep_buy_cap)
                deep_ask_size = min(self.H_LAYER2_SIZE, deep_sell_cap)
                if deep_bid_size > 0:
                    orders.append(Order(prod, int(deep_bid), deep_bid_size))
                if deep_ask_size > 0:
                    orders.append(Order(prod, int(deep_ask), -deep_ask_size))
        return orders, tm
