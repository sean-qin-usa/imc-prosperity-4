"""
Round 3 combined SHIP v6 — 2026-04-25 (session 6).

DELTA vs ship_v4 — HYDROGEL chassis retune (4 knobs from h_only_v15
sweep, applied to the combined ship for the first time):

  CLIP_VOL_K   0.30 → 0.795   (+~4.1k single-knob; cliff at 0.80)
  DMID_HISTORY 20   → 50      (+~0.5k joint; cliff at ≥60)
  H_PENNY_EDGE 2.0  → 3.0     (+~0.1k joint)
  AR1_BETA     0.18 → 0.25    (+~0.1k joint with the rest)

Total bt: +5,236 over ship_v4 (312,124 → 317,360).

Audit notes (session 6):
  - prior recipe asserted "CLIP_VOL_K > 0.3 didn't help in combined".
    Re-tested 2026-04-25 — IT DOES help up to 0.798 (-0.05 buffer
    from 0.80 cliff).  Earlier negative result was likely from
    sweeping with old CLIP and not re-sweeping the joint with DH/PE.
  - cliff structure unchanged from h_only_v15: CLIP=0.80, DH=60,
    H_MAX_POST_SIZE=19 all crater day-2 (-25 to -35k).
  - SIGMA sensitivity is HIGH in P4 R3 (peak 0.23, sharp drop both
    sides) — different from P3 where smile was decorative.  P4 ATM-EMA
    sleeve and synth deep-ITM MM both depend on SIGMA, while P3 only
    used it for residual EMAs which absorbed any bias.
  - P3 session-4.5 "move ITM strikes from IV-scalp to MR" → REGRESSED
    in P4 R3 by -25k (best variant) to -485k (worst).  P4 wide spreads
    on V4000 (~21 ticks) make aggressive MR-style market-take too
    expensive for deep ITM, and OTM strikes lack vega for clean
    IV-residual signal.  Microstructure differs.

Live ladder (from prior sessions):
  h_only_v8: 171,890 bt → 391,745 live (2.27x)
  ship_v1:   268,008 bt → 393,037 live (1.47x)
  ship_v2:   279,355 bt → 395,505 live (1.42x)
  ship_v4:   312,124 bt → 395,880 live (1.27x)
  ship_v6:   317,360 bt → ??? live (HYDROGEL retune; expect ~12-15k
                                    extra live since HYDROGEL converts
                                    at 2.27x and this is a HYDROGEL
                                    delta).

Sleeves (unchanged from v4):
  1. HYDROGEL_PACK: own-microstructure MM with vol-adaptive CLIP, AR(1)
     mean-reversion lean, anchor 9983.
