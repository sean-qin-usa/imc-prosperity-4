"""
Round 3 combined SHIP v15_hdrift — 2026-04-25.

v15_hdrift = v15 + HYDROGEL drift-regime gate (h_day1_drift_v2.py port).
3-day backtest: 443,484 → 445,448 (+1,964) — all 3 days improved
  (d0 +629 / d1 +660 / d2 +676).

Mechanism: when 200-tick touch_mid range > 53, multiply HYDROGEL's
CLIP_VOL_K by 2.5. Detects drift bursts (e.g., day-1 ticks 6000-9999
where mid jumps 9976→10079). Without the gate, HYDROGEL fights drift
via aggressive CLIP-capped fair, accumulates adverse-fill shorts
during drift bursts.

Cliffs (sweep_drift_v15.py):
  R <= 51 → spurious gating on a recurring d2 episode = -28k day 2
  B >= 1.7 → over-widening fades real MR = -456 day 1
...
class Trader:
    ...
    # ----- VFE aggressor-flow signal (v15_aggr add) ----------------------
    # Per-tick: classify state.market_trades[VFE] as buyer-aggr (price≥ask)
    # → +qty, seller-aggr (price≤bid) → -qty, sum into AGG_SIGNAL.
    # Empirical: corr(AGG, next_dmid) +0.28/+0.22/+0.34 across 3 days.
    # Sweep peak at THR=3, SHRINK=0.3, INSIDE=0, BOOST=1.0 on v15 chassis:
    # +50 jmerle bt; bundle-cal cal_minus_official -360 → -0.3 (+360).
    # Only bid-size SHRINK on seller flow paid (bb+1 step lost -11k from
    # queue position; size BOOST has no effect, matcher fill cap binds).
    VFE_AGGR_THR = 3
            ...
            if ema_o_dev > self.UNDER_MR_THR and u_max_sell > 0:
                ...
                if u_book is not None and u_pos < self.UNDER_DRIFT_TARGET:
                    ...
                    if cap > 0:
                        ...
                        # Aggressor overlay (v15_aggr): boost on buyer flow,
                        # shrink on seller flow.
    ...
    def run(self, state: TradingState):
        ...
        vfe_agg = 0
        u_od_for_agg = state.order_depths.get("VELVETFRUIT_EXTRACT")
        vfe_book_for_agg = self._book(u_od_for_agg) if u_od_for_agg is not None else None
        if vfe_book_for_agg is not None:
            ...
            for tr in mt:
                if tr.price >= ba_a - 0.5:
                    vfe_agg += tr.quantity
                elif tr.price <= bb_a + 0.5:
                    vfe_agg -= tr.quantity
