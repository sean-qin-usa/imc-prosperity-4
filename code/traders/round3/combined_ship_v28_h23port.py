"""
Round 3 combined SHIP v25 — 2026-04-25 (session 8 close, cap-tuned).

DELTA vs v24 (493,785 bt) — same chassis as v24, with one tuned knob:
VFE_MAX_TAKE_PER_TICK lowered from 80 → 15.  Slower per-tick rebalancing
gives Citadel time to confirm the side before committing the full 200,
reducing whipsaw on local z-score crossings.

3-day backtest match-trades all:  +497,324 (148,076 / 174,920 / 174,328)
3-day backtest match-trades none: +341,134 (79,052 / 130,158 / 131,924)
                                   alpha floor jumped +60,298 vs v23's 280,836.

The +60k alpha-floor jump is the headline result: the Citadel layer is
NOT matching-engine wallpaper — it's real take-side directional alpha
that the bots can't avoid.  Per memory `feedback_match_trades_none_for_alpha.md`,
match-trades none is a live-predictive lower bound; this means
the Citadel sleeve should convert at well above the 1-3% rate seen
on voucher MR.  Estimated live: ≥341k (alpha-floor lower bound).

Cap sweep on v23+Citadel chassis (Z=2.0, ALPHA=5e-4 fixed):
  cap=10  → 496,805
  cap=15  → 497,324  ← peak
  cap=20  → 496,967
  cap=25  → 496,942
  ...
  cap=50  → 494,420
  cap=60  → 493,918
  cap=80  → 493,785  (dad.py default; clean port = v24)
  cap=200 → 493,785

Z sweep on cap=80 chassis (default cap):
  Z=1.5  → 463,341
  Z=1.75 → 480,262
  Z=2.0  → 493,785  ← peak (matches dad.py default)
  Z=2.25 → 438,041
  Z=2.5  → 449,002

EMA_ALPHA sweep on cap=15 chassis (Z=2.0 fixed):
  alpha=2e-4 → 466,156
  alpha=5e-4 → 497,324  ← peak (matches dad.py default)
  alpha=1e-3 → 487,467
  alpha=2e-3 → 478,922

Cumulative session-8 stack:
  v15            (baseline ship):                    443,484
  v15_hdrift     (drift-regime CLIP gate):           +1,964 → 445,448
  v22            (+VEV carry V5000-5300):            +1,594 → 447,042
  v23            (+asym INV_SKEW LONG/SHORT):        +4,289 → 451,331
  v24            (Citadel VFE MR replaces Timo):    +42,454 → 493,785
  v25            (cap=15 instead of 80):             +3,539 → 497,324
  Total v15→v25:  +53,840 bt (+12.1%).

Citadel VFE MR (overrides Timo underlying MR + VFE drift carry):
  VFE_EMA_ALPHA         = 5e-4   half-life ~1386 ticks (~14% of a day)
  VFE_SIGMA_ALPHA       = 5e-4
  VFE_SIGMA_FLOOR       = 1.0
  VFE_WARMUP_TICKS      = 500    no trades until EMA stabilises
  VFE_Z_ENTER           = 2.0    enter on |z| >= 2; flip on opposite cross
  VFE_MAX_TAKE_PER_TICK = 15     v25 tuned (down from 80); plateau 10-30
  ENABLE_UNDERLYING_MR  = False  (Timo short-window MR disabled)

Live conversion update (post alpha-floor check, lower uncertainty):
  - The +60k alpha-floor jump confirms the Citadel sleeve is take-side
    directional alpha — bots cross the spread to fill our orders, so
    the gross PnL converts close to 1.0x rather than the 1-3% voucher-MR rate.
  - Live PnL forecast: ≥341k (match-trades none lower bound) and
    ≤497k (match-trades all upper bound).  Mid-point ~419k as a
    reasonable point estimate; assumes voucher-MR 1-3% conversion on
    the remaining 156k matching-engine fraction → ~341 + 4 = ~345k
    conservative, or ~341 + 156 = ~497k optimistic.
  - cap=15 vs cap=80 plateau is only +3.5k wide; the cap-tuning may
    not transfer to live as cleanly as the Citadel structural alpha.
  - Sample-path risk on the 3-day historical sample remains, but the
    alpha-floor result rules out the "all matching-engine sample fluke"
    failure mode.
...
class Trader:
    ...
    # ----- HYDROGEL (v28: h_only_v23 chassis port) -----
    # h_only_v23 standalone bt = 196,485; v25 in-ship contribution =
    # 188,202.  Ports the asymmetric pos-conditional vol-adaptive CLIP
    # plus the h_only_v23 retunes (TE 0.6→0.3, PE 3.5→4.0, INV_SKEW_LONG
    # -0.004→-0.015, AR1 0.25→0.20, CLIP_VOL_K 0.795→0.78,
    # H_POST_VOL_K 0.0→1.0).  Drift-gate dropped per memory note that
    # it regresses on this chassis.
    H_ANCHOR = 9983.0
    H_CLIP = 33.0
    H_TAKE_EDGE = 0.3
    H_REDUCE_EDGE = 0.0
    ...
    AR1_BETA = 0.20
    TYPICAL_SPREAD = 16
    CLIP_VOL_K = 0.78        # legacy ref; superseded by H_VK_UP/H_VK_DN_*
    # Asymmetric pos-conditional vol-adaptive CLIP (h_only_v21/v22/v23)
    H_VK_UP = 0.78
    H_VK_DN = 0.87           # zero-pos fallback
    H_VK_DN_LONG = 2.7       # legacy (v23 splits LONG by magnitude)
    H_VK_DN_SHORT = 0.85
    H_VK_DN_HIGH = 12.0      # when pos > H_POS_THR (extreme brake)
    H_VK_DN_LOW = 2.7        # when 0 < pos <= H_POS_THR
    H_POS_THR = 120
    DMID_HISTORY = 150
    H_BASE_POST_SIZE = 18
    H_POST_VOL_K = 1.0       # h_only_v23: vol-adaptive post size enabled
    H_POST_MIN = 12
    H_POST_MAX = 18

    # Drift-gate dropped (regresses on the h_only_v23 chassis).
    MID_RANGE_HISTORY = 200
    DRIFT_GATE_RANGE = 0     # 0 disables; set 53 to re-enable
    DRIFT_CLIP_BOOST = 0.0

    # ----- Underlying MR (VFE) — Timo P3R3 port -----
    # v24: DISABLED.  Replaced by Citadel bipolar long-EMA MR below
    # (longer horizon, +42,454 in dad.py vs Timo short-window MR).
    ...
    # v24: drift-carry branch was inside the Timo MR else-clause; with
    # ENABLE_UNDERLYING_MR=False that branch never fires.  Citadel layer
    # subsumes the long-bias mechanism by holding +LIMIT during low-z regimes.
    ...
    # ----- Citadel bipolar VFE long-EMA MR (port from dad.py) -----
    # alpha = 5e-4 → half-life log(0.5) / log(1-5e-4) ~ 1386 ticks (~14% of day).
    # Hold VFE at +/-LIMIT once |z| crosses VFE_Z_ENTER; flip on opposite cross.
    # WARMUP blocks trading until the EMA stabilises.
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist, mid_hist):
        ...
        # Asymmetric pos-conditional vol-adaptive CLIP (h_only_v23 chassis).
        # CLIP_DOWN is wider than CLIP_UP so fair can drift further below
        # anchor in vol bursts, preventing over-buying on persistent down-
        # moves.  When LONG and pos > H_POS_THR, vk_dn jumps to HIGH (12).
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        if self.ENABLE_UNDERLYING_MR:
            ...
            if ema_o_dev > self.UNDER_MR_THR and u_max_sell > 0:
                ...
                # No MR signal — pursue passive long-bias for drift carry.
                u_book = self._book(u_od)
                if u_book is not None and u_pos < self.UNDER_DRIFT_TARGET:
                    cap = min(u_max_buy, self.UNDER_DRIFT_TARGET - u_pos)
                    if cap > 0:
                        bid_price = u_book["bb"]
                        sz = min(cap, self.UNDER_CARRY_BID_SIZE)
                        u_orders.append(Order("VELVETFRUIT_EXTRACT", bid_price, sz))

        return {k: v for k, v in out.items() if v}

    # ---------- Citadel bipolar long-EMA VFE mean-reversion (port from dad.py) ----------
    ...
    def run(self, state: TradingState):
        ...
        # 6. Citadel bipolar long-EMA VFE MR (overrides any prior VFE orders).
        # When ENABLE_UNDERLYING_MR=False, _trade_iv_residual emits no VFE
        # orders, so this is the sole VFE source.
