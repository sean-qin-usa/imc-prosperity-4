...
class Trader:
    ...
    # Layer-2 passive close: flatten MR position when signal goes past 0
    # toward the unfavorable side. THR is the cushion before flattening.
    MR_CLOSE_THR = 1.0
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # OPEN: take-side aggressive on |combined| > OPT_MR_THR.
        # CLOSE-LAYER-2: when sign(combined) flipped relative to position
        # (signal decayed past 0), post inside-touch passive flatten
        # instead of waiting for the opposite take. Captures spread on
        # exit (bb+1 instead of bb, ba-1 instead of ba).
        for name, K in MR_STRIKES.items():
            ...
            if combined > OPT_MR_THR and max_sell > 0:
                ...
                # Layer-2 close: signal reverted past target.  We OPENED
                # long at combined<-5; close once combined > MR_CLOSE_THR
                # (passed near-zero target). Captures spread on exit
                # instead of waiting for opposite take to fire.
                spread = aw - bw
                if spread >= 2 and p != 0:
                    if p > 0 and combined > self.MR_CLOSE_THR:
                        # long; reverted up — close passively at ba-1
                        orders.append(Order(name, int(aw - 1), -p))
                    elif p < 0 and combined < -self.MR_CLOSE_THR:
                        # short; reverted down — close passively at bb+1
                        orders.append(Order(name, int(bw + 1), -p))
