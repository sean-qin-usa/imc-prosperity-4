"""
Round 3 baseline v6 — 2026-04-24 session 2.

v5 pathology (live day-2 submission log 374582.log): HYDROGEL touch_mid
dropped 55.5 ticks in one step (9984 -> 9928.5) at t=80300. The
anchor-clip held fair at 9960 (lower-clip floor) while the market was
at 9928.5, so every live ask <= 9960 looked like a fat take. The bot
loaded to the 200-long limit and took a -9,296 one-tick P&L hit as
the entire inventory mark-dropped.

Historical 3-day context (from stats run):
  day 0: worst single-tick mid drop = -11.5
  day 1: worst single-tick mid drop = -10.5
  day 2: worst single-tick mid drop = -8.0
Live shock was 5x the worst historical precedent. Zero training-data
signal that such a shock was possible.

v6 fix: detect shock via `abs(touch_mid - prev_touch_mid) > SHOCK_MOVE`.
When shock fires:
  * Set fair = touch_mid for this tick (no anchor pull; prevents
    aggressive takes into a falling market).
  * Skip new-post logic entirely (no quotes posted this tick).
  * Keep take-loops structurally intact, but with fair = touch_mid the
    inventory-skewed threshold evaluates correctly against current mid
    (we stop buying asks below stale-fair).

Historical backtest: zero shock events (worst = 11.5 < 15), so v6
behaves identically to v5 on all 3 historical days. Confirmed below.

Vouchers (VEV_4000/4500) unchanged from v5 - per-tick live performance
was within 54-85% of backtest which is consistent with the known
flat-sigma bias flagged in session 1. That's a separate v7 work item
(port Timo's IV-residual EMA).
"""
...
class Trader:
    ...
    # HYDROGEL_PACK peak (unchanged from v5)
    H_ANCHOR = 9990.0
    H_CLIP = 30.0
    H_TAKE_EDGE = 0.0
    H_REDUCE_EDGE = 1.0
    ...
    # v6: single-tick shock detector. 15 is ~3x the historical max
    # (-11.5) and ~27% of the observed live shock (-55.5). Catches the
    # live event with margin; never fires on historical 3-day.
    H_SHOCK_MOVE = 15.0
    ...
    def _trade_hydrogel(self, od: OrderDepth, pos: int, prev_mid) -> (List[Order], float):
        prod = "HYDROGEL_PACK"
        ...
        # v6: shock detection. If prev_mid exists and move exceeds
        # H_SHOCK_MOVE, set fair = touch_mid (no anchor trust) and
        # suppress new posts. Take-loops still run but edge collapses
        # naturally when fair = touch_mid.
        ...
        # v6: on shock, skip posting. Wait a tick for the book to
        # re-equilibrate before re-engaging as MM.
        if shock:
