# HYDROGEL_PACK Stand-Alone — Recipe (v25, 2026-04-25)

## TL;DR

`traders/round3/h_only_v25.py` ships at **197,509** over 3 historical
days (69,480 / 57,590 / 70,439). That is **+25,619 (+14.91 %)** over
v8 (171,890), and +656 over v24 (196,853). Day 2 +554 from a 4-tier
vk_dn LONG side (super-extreme brake at pos > 165).

```
cd /Users/sean_tsu_/Downloads/prosperity/IMCP2026
python3 tools/jmerle_backtester.py traders/round3/h_only_v25.py 3 --merge-pnl --no-out
```

The hidden submission day (file `391745`) is the first 100k of day 2's
1M ticks. Day 2 gain on local (+12,805 vs v8) should carry over.

## What changed vs v8

Seventeen-knob retune (16 from v24 + 1 new in v25: vk_dn LONG now 4-tier).

| Param | v8 | v22 | v23 | v24 | v25 | Notes |
|---|---|---|---|---|---|---|
...
| `H_VK_UP` | (=0.3) | 0.78 | 0.78 | 0.78 | **0.78** | upper CLIP vol scaling |
| `H_VK_DN_LOW` | — | (=2.7) | 2.7 | 2.7 | **2.7** | vk when 0 < pos ≤ 130 (= v22) |
| `H_VK_DN_HIGH` | — | — | 12.0 | 12.0 | **12.0** | vk when 130 < pos ≤ 165 (v23 brake) |
| `H_VK_DN_XTREME` | — | — | — | — | **16.0** | **v25 NEW** (+656): vk when pos > 165 (super-brake) |
| `H_POS_THR` | — | — | 120 | 120 | **130** | v25: 120 → 130 |
| `H_POS_THR_2` | — | — | — | — | **165** | v25 NEW second threshold |
| `H_VK_DN_SHORT` | — | 0.85 | 0.85 | 0.85 | **0.85** | when pos < 0 |
| `H_POST_VK_HIGH` | — | — | — | 1.0 | **1.0** | v24 lever |
| `H_POST_VK_LOW` | — | — | — | 0.0 | **0.0** | v24 lever |
| `H_POST_ABS_POS_THR` | — | — | — | 160 | **160** | v24 lever |
...
| `_cap_size` linear | 0.7 | 0.5 | 0.5 | 0.5 | **0.5** | v20 lever |

## Why 4-tier vk_dn LONG is the v25 lever

v23 introduced the 2-tier mag-thresh on vk_dn LONG (vk=12 when
pos>120, vk=2.7 otherwise). This braked over-buying at extreme long
positions, but the brake at the 12 level was still firing modestly
relative to the risk at very-extreme positions.

v25 splits the LONG side into 3 tiers (instead of 2):

  pos > 165       → vk_dn = 16.0  (super-extreme brake, NEW)
  130 < pos ≤ 165 → vk_dn = 12.0  (= v23 brake)
  0 < pos ≤ 130   → vk_dn = 2.7   (= v22 baseline)

At pos > 165 (close to the +200 limit), the strategy is in the
highest-risk over-accumulation zone. The vk=16 super-brake widens
the lower CLIP further than vk=12, pushing bid posts even deeper
and effectively halting new long fills.

Sweep:
  vk_xtreme = 14, 15 → +346, +205 (both wins, 16 is peak)
  vk_xtreme = 16    → 197,509 (peak, +656)
  vk_xtreme = 17    → +462 (slight regression)
  vk_xtreme = 20    → +57
  vk_xtreme = 30+   → -150
  thr2 ∈ {165, 168} → identical (both 197,509 with vk=16)
  thr2 ≥ 175      → marginal regression
  thr1 = 130 (was 120) → small additional gain over thr1=120
...
22. **v24: H_POST_ABS_POS_THR ≤ 140**: collapses to baseline.
23. **v24: H_POST_ABS_POS_THR > 200**: never triggers.
24. **v25: H_VK_DN_XTREME ≥ 30**: -150 (over-brakes, kills fills entirely).
25. **v25: H_VK_DN_XTREME = 12 (= v23 brake) at thr2=165**: collapses to v23 (no XTREME tier benefit).
26. **v25: H_POS_THR_2 ≥ 175 with vk=16**: marginal regression.
...
| **mag-thresh INV_SKEW_LONG** (different skew at extreme pos) | every variant ≤ baseline | (v25 sweep) |
| **drift × pos joint gate on v23 brake** (only fire when drift adverse too) | dg=0-30 all = baseline (drift always ≤-30 when brake fires); dg≥40 regresses | (v25 sweep) |
| **bid-only PE mag-thresh** (deeper bid when long > THR) | +9 noise (peak 196,862) | (v25 sweep) |
| **mag-thresh TAKE_EDGE** (different cushion at extreme \|pos\|) | every variant ≤ baseline | (v25 sweep) |
| **vol-conditional INV_SKEW_LONG** (different skew at high vs low std) | zero cells beat baseline | (v25 sweep) |
| **mag-thresh REDUCE_EDGE** (allow RE>0 only at extreme \|pos\|) | every RE>0 variant regresses | (v25 sweep) |
...
| File | Status |
|---|---|
| `h_only_v25.py` | **SHIP** — 197,509 (+25,619 vs v8, +14.91%) |
| `h_only_v24.py` | prev ship — 196,853 |
| `h_only_v23.py` | older — 196,485 |
...
| `HYDROGEL_ONLY_RECIPE_v24.md` | v24 recipe |
| `HYDROGEL_ONLY_RECIPE_v25.md` | this file |
...
# Final ship
python3 tools/jmerle_backtester.py traders/round3/h_only_v25.py 3 --merge-pnl --no-out
# Expected: total 197,509  (69,480 / 57,590 / 70,439)

# Verify forbidden-imports check
grep -nE "^import os|^from os|import subprocess|exec\(|eval\(" traders/round3/h_only_v25.py
# Expected: empty
```
