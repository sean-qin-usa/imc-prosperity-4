"""
HYDROGEL-only v14 — v8 retune (2026-04-25).

Two-knob change vs v8:
  H_ANCHOR    9985 → 9983   (+2.8k joint, full anchor sweep)
  H_INV_SKEW  0.015 → 0.014 (+1.5k joint, full skew sweep)
Local 3-day total: 175,497  (+3,607 vs v8 171,890), per-day
[61,366 / 52,574 / 61,557].

Day 2 alone gains +3.9k (57,634 → 61,557). Days 0/1 unchanged.
Day 2's the same data as the hidden submission window (first 100k
ticks of day 2), so the retune most likely improves hidden-day too.

Other dims confirmed at peak by individual sweep (no incremental gain
from H_PENNY_EDGE, AR1_BETA, H_CLIP, H_MAX_POST_SIZE, CLIP_VOL_K,
TYPICAL_SPREAD, H_REDUCE_EDGE).
...
class Trader:
    ...
    # v14 retune (best joint sweep result):
    H_ANCHOR = 9983.0
    H_CLIP = 33.0
    H_TAKE_EDGE = 0.0
    H_REDUCE_EDGE = 0.0
