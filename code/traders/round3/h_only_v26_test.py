"""
HYDROGEL-only v25 — 4-TIER vk_dn LONG side (2026-04-25).

New lever vs v24: split the v23 LONG-side vk_dn into THREE tiers
(plus SHORT and zero), instead of two:

  pos > 165       → H_VK_DN_XTREME = 16.0  (super-extreme brake)
  130 < pos ≤ 165 → H_VK_DN_HIGH   = 12.0  (= v23 brake)
  0 < pos ≤ 130   → H_VK_DN_LOW    = 2.7   (= v22 vk_long)
  pos < 0         → H_VK_DN_SHORT  = 0.85
  pos = 0         → H_VK_DN        = 0.87

Why it works: at very extreme positions (>165), the v23 brake (12.0)
is still firing too modestly — accumulated longs at >165 are already
in the highest-risk regime where adverse selection during persistent
down-trends is most acute. The XTREME tier (16.0) widens the lower
CLIP further, pushing bid posts even deeper and effectively halting
new long accumulation.

Local 3-day total: 197,509 (+656 vs v24 196,853; +25,619 vs v8 171,890;
+14.91%). Per-day: [69,480 / 57,590 / 70,439]
  day 0: 0 vs v24    day 1: +102 vs v24    day 2: +554 vs v24

Day 2 is the big winner (+554) — the day where extreme long-side
inventory builds up most in a down-trend.

Sweep cliffs:
  H_VK_DN_XTREME ≠ 16.0 → small loss (16.0 is sweet spot)
  H_POS_THR_2 ≠ 165       → marginal regression
  H_POS_THR ∉ [120, 130]  → marginal regression

Also bumped H_POS_THR 120 → 130 for slight additional gain.

------ v24 history below ------

HYDROGEL-only v24 — MAGNITUDE-thresholded POST_VOL_K (2026-04-25).

New lever vs v23: split the v17 vol-adaptive post-size scaling
(H_POST_VOL_K=1.0, sym across position) into a magnitude-conditional
two-tier function:

  |pos| > 160 → POST_VK_HIGH = 1.0 (= v23; vol-shrink to ~16-17)
  |pos| ≤ 160 → POST_VK_LOW  = 0.0 (no shrink, post full size 18)

Why it works: at moderate position, the strategy can post LARGER size
(=18) without the vol-shrink penalty. At extreme position (|pos|>160),
keep the v23 vol-shrink behaviour because day 0 critically needs it
(sym POST_VK<1.0 craters day 0 by -30k!).

Local 3-day total: 196,853 (+368 vs v23 196,485; +24,963 vs v8 171,890;
+14.52%). Per-day: [69,480 / 57,488 / 69,885]
  day 0: 0 vs v23     day 1: +368 vs v23     day 2: 0 vs v23

Marginal +368 (0.19%); single-knob day 1 gain. Stable across many
parameter values (pl ∈ {0.0, 0.3, 0.5, 0.8} and thr ∈ {160, 170, 180}
all give the exact same 196,853 — likely a single-tick fill effect).

Sweep cliffs:
  POST_VK_HIGH < 1.0       → day 0 craters from 69k → 39k (-30k!)
  POST_ABS_POS_THR ≤ 140   → no benefit (collapses to baseline)
  POST_ABS_POS_THR > 200   → no benefit (never triggers; |pos|≤200)

Search exhausted (this round): mag-thresh VK_UP SHORT side (+27 noise,
dead end), mag-thresh PE (deeper at extreme — every variant ≤ baseline).
...
class Trader:
    ...
    H_POS_THR = 130         # v25: 120 → 130
    # v25: 4-tier vk_dn — super-extreme brake at very high pos
    H_VK_DN_XTREME = 16.0   # when pos > H_POS_THR_2
    H_POS_THR_2 = 165
    H_CAP_LIN_HIGH = 0.7
    H_CAP_LIN_LOW = 0.6
    H_CAP_ABS_POS_THR = 180
    DMID_HISTORY = 150

    H_BASE_POST_SIZE = 18
    H_POST_VOL_K = 1.0       # legacy fallback
    H_POST_MIN = 12
    H_POST_MAX = 18
    # v24: mag-thresholded post-vol-K — at moderate |pos|, no shrink (LARGER posts)
    H_POST_VK_HIGH = 1.0     # = v23 (preserve when |pos| > thr; day 0 NEEDS this)
    H_POST_VK_LOW  = 0.0     # no shrink at moderate |pos|
    ...
    def _cap_size(self, max_size, pos, side, cap, limit):
        if cap <= 0: return 0
        _cap_lin = self.H_CAP_LIN_HIGH if abs(pos) > self.H_CAP_ABS_POS_THR else self.H_CAP_LIN_LOW
        ratio = 1.0 - min(_cap_lin, abs(pos) / limit)  # v20: 0.7 → 0.5 (softer linear cap)
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist):
        ...
        if self.CLIP_VOL_K > 0 and len(dmid_hist) >= 3:
            ...
            if pos > self.H_POS_THR_2:
                _vkd = self.H_VK_DN_XTREME
            elif pos > self.H_POS_THR:
                _vkd = self.H_VK_DN_HIGH
            elif pos > 0:
                _vkd = self.H_VK_DN_LOW
            elif pos < 0:
                _vkd = self.H_VK_DN_SHORT
            else:
                _vkd = self.H_VK_DN
            clip_dn = self.H_CLIP + _vkd * std_d
        ...
        if self.H_POST_VOL_K != 0 and len(dmid_hist) >= 3:
            ...
            adaptive_size = self.H_BASE_POST_SIZE - effective_pvk * sd
