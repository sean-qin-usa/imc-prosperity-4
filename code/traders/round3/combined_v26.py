    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        for name, K in MR_STRIKES.items():
            ...
            if combined > OPT_MR_THR and max_sell > 0:
                ...
                # Layer-2 close: signal decayed AND we have position
                # opposite to its current sign → flatten passive inside.
                spread = aw - bw
                if spread >= 2 and p != 0:
                    if p > 0 and combined < self.MR_CLOSE_THR:
                        # long, signal weakened → close at ba-1 (passive ask)
                        orders.append(Order(name, int(aw - 1), -p))
                    elif p < 0 and combined > -self.MR_CLOSE_THR:
                        # short, signal weakened → close at bb+1 (passive bid)
