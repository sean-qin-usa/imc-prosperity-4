...
class Trader:
    ...
    # v29 stat-arb: Δresidual AR1 = -0.39/-0.27 on V_5400/5500.  Lean fair
    # toward partial reversion of last residual.  Smaller damping than
    # synth because (a) ATM EMA-fair already captures slow drift, and
    # (b) Δresidual AR1 is weaker for ATM strikes.
    ATM_RESID_DAMP = 0.30
    ...
    VS_PENNY_EDGE = 1.0
    VS_INV_SKEW = 0.0
    # v29 stat-arb: Δresidual AR1 = -0.50 cross-day stable on V_4000/4500
    # (residuals ~ N(0, 0.8); raw Δmid AR1 captures partly spurious AR1
    # from VFE delta=1).  Fair shifts toward 0.5*(last_residual+prev_residual)
    # per the AR1(Δresidual)=-0.5 prediction.
    VS_RESID_DAMP = 0.50
    VS_MAX_POST_SIZE = 40
    VS_WIDE_SPREAD = 3
    ...
    def _trade_synth_voucher(self, name, K, od, pos, S, T, last_residual=0.0):
        ...
        # v29 residual-AR1 lean: fair predicts mid by (1+ρ_dr)*last_residual
        # where ρ_dr=-0.5; with prev_resid≈last_resid this collapses to
        # 0.5*last_residual.
        fair = bs_theo + self.VS_RESID_DAMP * last_residual
        working = pos
        orders: List[Order] = []
        for ap, av in book["sells"].items():
            cap = limit - working
    ...
    def run(self, state: TradingState):
        ...
        if u_od is not None:
            ...
            if u_book is not None:
                ...
                last_synth_residual = saved.get("last_synth_residual", {})
                for name, K in self.SYNTH_STRIKES.items():
                    ...
                    if od is not None:
                        ...
                        cur_resid = None
                        if b is not None:
                            bs_theo, _, _ = self._opt_bs(S_synth, K, T_synth, self.SIGMA)
                            cur_resid = b["touch_mid"] - bs_theo
                        last_resid = last_synth_residual.get(name, 0.0)
                        result[name] = self._trade_synth_voucher(
                            name, K, od, pos.get(name, 0), S_synth, T_synth,
                            last_resid,
                        )
                        if cur_resid is not None:
                            last_synth_residual[name] = cur_resid
                saved["last_synth_residual"] = last_synth_residual
