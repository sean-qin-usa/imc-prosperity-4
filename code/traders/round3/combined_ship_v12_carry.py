"""
Round 3 combined SHIP v12_carry — 2026-04-25 (session 5, VFE drift carry).

** DO NOT SHIP **  -- this is a *negative* result kept for reference.
The default ENABLE_UNDERLYING_CARRY=False makes this functionally
identical to combined_ship_v11.py.

Hypothesis: VFE has +0.002/ts drift on days 1-2 (+20.5 / +28) and ~0 on
day 0 (-6). Static long-bias should capture ~drift * target.

Variant A — passive penny-inside layer (this file). Posts bid at bb+1 if
below target, ask at ba-1 if above target, sized to deficit. The layer
*adversely fills*: at bb+1 we're inside the L1 spread, so any seller
crossing to bb hits us first. We acquire long right before further drops,
dump it via passive ask right before further rises.

Result vs v11 baseline 428,754:
  target=0:   421,003 (Δ -7,751)  -- pure churn around 0
  target=30:  423,237 (Δ -5,517)
  target=80:  428,534 (Δ -220)    -- layer goes idle once filled

Variant B (combined_ship_v12_carry_b.py) was cleaner (asymmetric MR cap)
but still negative-slope (≈ −6/unit; -189 at target=30). Both variants
fail because VFE's MR sleeve uses 100% of position capacity per fire
and steals more alpha per reserved unit than drift gains.

Detailed writeup: `VFE_DRIFT_CARRY_2026_04_25.md` in this directory.

(Branched off v11 because a parallel agent is independently adding
imbalance-aware quote sizing to combined_ship_v12.py for the synth voucher.)

Underlying base: v11.

== v11 history (kept for reference) ==
Round 3 combined SHIP v11 — 2026-04-25 (session 4, aggressive ceiling).

3-day backtest match-trades all:  +428,754 (122,899 / 150,258 / 155,597).
3-day backtest match-trades none: +280,600 (alpha floor).
vs combined_ship_v4 (312,124 / 144,856; live 395,880): +116,630 / +135,744.
vs combined_ship_v9 (399,592):                          +29,162.

Per-day stability:
  Day 0: 122,899   Day 1: 150,258   Day 2: 155,597
  μ = 142,918, σ = 17,425, CV = 12.2% (v4 CV 18.7%, v1 14.1%).

This file is v4's structure (ALL passive-make sleeves preserved) +
the most-defensible own-microstructure / make-side wins from the
session-4 sweeps:

Stack of changes vs ship_v4:
  HYDROGEL (h_only_v14 → h_only_v16 multi-knob retune):
    H_PENNY_EDGE  2.0  → 3.0    (avoids pe=2/dh≥60 cliff)
    H_TAKE_EDGE   0.0  → 0.5    (cushion for marginal crosses)
    CLIP_VOL_K    0.3  → 0.76   (volatility-adaptive CLIP scales further)
    DMID_HISTORY   20  → 150    (smoother std for vol-CLIP)
    AR1_BETA      0.18 → 0.17

  Synthetic-voucher MM (V4000/V4500):
    VS_INV_SKEW   0.005 → 0.0   (the single biggest free lunch this
                                  session: 286k → 311k at v3 base.
                                  Clean alpha was being skewed away.)

  Voucher IV-residual MR (5000/5100/5200/5300):
    THEO_NORM_WINDOW   20 → 25  (smoother per-strike residual)
    iv_dev weight    2.25 → 1.75 (per-strike alpha 1.75x ema_o)
    OPT_MR_WINDOW      30 → 44  (slower underlying EMA, less choppy)
    OPT_MR_THR          5 → 6.3 (only large deviations, less noise trade)

  VELVETFRUIT underlying MR:
    UNDER_MR_THR        5 → 6   (matches the new EMA window scale)

LIVE-vs-BACKTEST WARNING (from session-4 ladder analysis):
- Voucher MR alpha generalises POORLY to live (~1-3% of backtest PnL
  in test_results/ ladder). The aggressive (44, 6.3) MR tuning here
  contributes ~+87k of the +116k backtest delta — most of which will
  NOT realise live. Expect realised live gain of ~+15-30k vs ship_v4
  (HYDROGEL retune + VS_INV_SKEW + UNDER MR retune are the convertible
  pieces; the ATM-EMA passive-make on 5400/5500 already shipped in v4).
- For a more conservative variant that strips the voucher MR retune
  but keeps all the make-side improvements, see combined_ship_v10.py
  (341,808 backtest; ~425k expected live).

Critical sweep cliffs (do NOT casually retune):
- VS_INV_SKEW=0 peak. 0 → 311k at v3 base, 0.005 (was v1) → 286k.
  The +25k at this single knob is the single biggest win of the session.
- OPT_MR_THR plateau 6.3-6.5 at WIN=44. THR≤5.5 craters; THR≥7 craters.
- OPT_MR_WINDOW=44 peak (with THR=6.3). 30→312k, 44→399k, 48→384k.
  Cliff on either side; correlated with THR.
- UNDER_MR_THR=6 peak (was 5 at v1). 5→371k, 6→398k, 7→377k at v9 base.
- iv_dev weight wide plateau [1.7, 2.0]. 1.5 → 390k, 1.75 → 398k peak,
  2.0 → 396k, 2.25 → 392k.

What was DEPRIORITIZED (verified bad, do not redo):
- Layered Timo IV-scalp on 5000/5100 alongside MR: -46k catastrophic
  (the calm-regime flatten branch fights the MR position).
- Add VEV_5400 to MR strike set: -8k (vega < 1, residual noise-dominated
  even with new params). 5400/5500 are better-handled by ATM-EMA MM.
- Drop VEV_5300 from MR: -3.4k. Small but real contributor.
- Drop VFE underlying MR: -38k. Keep it.
- VS_PENNY_EDGE sweep [0,5]: completely insensitive (synth book has
  21-tick spread; we never cross at any post level).
- 3-way basis arb (V4000+4000 ↔ V4500+4500 ↔ S): basis std 0.83 ticks
  but round-trip cost ≥4 ticks > available edge.

ATM smile-EMA MM (5400/5500) and lottery posts (6000/6500) are
inherited from ship_v4 — see v4 history block in those files for
the ATM and lottery rationale.

Underlying = VELVETFRUIT_EXTRACT (per-tick wall_mid for IV machinery).
Upload-safe: only math + statistics + json + datamodel.
"""
...
class Trader:
    ...
    # ----- Underlying drift carry (VFE) — v12 add -----
    # VFE has +0.002/ts drift days 1-2, ~0 day 0. Maintaining a base
    # long captures drift on top of MR. Passive layer at bb+PENNY/ba-PENNY
    # (penny inside the L1 spread) with deficit-scaled size.
    ENABLE_UNDERLYING_CARRY = False  # disabled by default: see header sweep
    UNDER_TARGET_LONG = 30
    UNDER_CARRY_MAX_SIZE = 30
    UNDER_CARRY_PENNY = 1
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # ---- Underlying drift carry (passive penny-inside layer) ----
        # Maintains a base long to capture VFE's +0.002/ts drift on
        # days 1-2. Skips when MR is firing same-side (MR has priority).
        if self.ENABLE_UNDERLYING_CARRY:
            u_book = self._book(ods["VELVETFRUIT_EXTRACT"])
            if u_book is not None:
                u_pos = pos.get("VELVETFRUIT_EXTRACT", 0)
                u_limit = self.LIMITS["VELVETFRUIT_EXTRACT"]
                u_orders = out.setdefault("VELVETFRUIT_EXTRACT", [])
                bb = u_book["bb"]; ba = u_book["ba"]
                deficit = self.UNDER_TARGET_LONG - u_pos
                # Skip the passive buy when MR is firing (either side):
                #   MR-sell crosses our own bid (bw+1 = best_bid; our bid bb+1 = best_bid+1 > MR sell px).
                #   MR-buy already uses full max_buy headroom on aggressive lift.
                if deficit > 0 and not u_mr_sell and not u_mr_buy:
                    bid_p = bb + self.UNDER_CARRY_PENNY
                    if bid_p < ba:
                        room = u_limit - u_pos
                        size = min(deficit, room, self.UNDER_CARRY_MAX_SIZE)
                        if size > 0:
                            u_orders.append(Order("VELVETFRUIT_EXTRACT", int(bid_p), size))
                # Skip the passive sell when MR is firing (either side):
                #   MR-buy crosses our own ask (aw-1 = best_ask; our ask ba-1 = best_ask-1 < MR buy px).
                #   MR-sell already uses full max_sell headroom on aggressive cross.
                elif deficit < 0 and not u_mr_buy and not u_mr_sell:
                    ask_p = ba - self.UNDER_CARRY_PENNY
                    if ask_p > bb:
                        room = u_limit + u_pos
                        size = min(-deficit, room, self.UNDER_CARRY_MAX_SIZE)
                        if size > 0:
                            u_orders.append(Order("VELVETFRUIT_EXTRACT", int(ask_p), -size))
