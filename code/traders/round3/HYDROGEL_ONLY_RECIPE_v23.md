# HYDROGEL_PACK Stand-Alone — Recipe (v23, 2026-04-25)

## TL;DR

`traders/round3/h_only_v23.py` ships at **196,485** over 3 historical
days (69,480 / 57,120 / 69,885). That is **+24,595 (+14.31 %)** over
v8 (171,890), and +1,489 over v22 (194,996). Day 0 -1,378 (trade-off)
but Day 1 +1,043 and Day 2 +1,824 more than make up for it.

```
cd /Users/sean_tsu_/Downloads/prosperity/IMCP2026
python3 tools/jmerle_backtester.py traders/round3/h_only_v23.py 3 --merge-pnl --no-out
```
...
Fifteen-knob retune (14 from v22 + 1 new in v23: vk_dn LONG side now
3-tier by magnitude).

| Param | v8 | v19 | v22 | v23 | Notes |
|---|---|---|---|---|---|
| `H_ANCHOR` | 9985 | 9983 | 9983 | **9983** | cliffs at 9982 / 9986 |
| `H_INV_SKEW_LONG` | (=0.015) | −0.015 | −0.015 | **−0.015** | v19 lever |
| `H_INV_SKEW_SHORT` | (=0.015) | +0.014 | +0.014 | **+0.014** | v19 lever |
| `H_VK_UP` | (=0.3) | (=0.76) | 0.78 | **0.78** | upper CLIP vol scaling |
| `H_VK_DN_HIGH` | — | — | — | **12.0** | **v23 NEW** (+1,489): vk when pos > 120 (extreme brake) |
| `H_VK_DN_LOW` | — | — | (=2.7) | **2.7** | vk when 0 < pos ≤ 120 (= v22 vk_long) |
| `H_POS_THR` | — | — | — | **120** | v23 NEW magnitude threshold |
| `H_VK_DN_SHORT` | — | — | 0.85 | **0.85** | when pos < 0 |
| `DMID_HISTORY` | 20 | 150 | 150 | **150** | cliff at 200 |
| `H_PENNY_EDGE` | 2.0 | 4.0 | 4.0 | **4.0** | cliff at 2 / 4.5+ |
| `H_TAKE_EDGE` | 0.0 | 0.3 | 0.3 | **0.3** | plateau 0.3–0.35 |
| `H_REDUCE_EDGE` | 0.0 | 0.0 | 0.0 | **0.0** | MUST stay 0 |
| `AR1_BETA` | 0.18 | 0.20 | 0.20 | **0.20** | cliff at 0.25 |
| `H_POST_VOL_K` | 0 | 1.0 | 1.0 | **1.0** | clamp [12, 18] |
| `_cap_size` linear | 0.7 | 0.7 | 0.5 | **0.5** | v20 lever |
...
| File | Status |
|---|---|
| `h_only_v23.py` | **SHIP** — 196,485 (+24,595 vs v8, +14.31%) |
| `h_only_v22.py` | prev ship — 194,996 |
...
| `HYDROGEL_ONLY_RECIPE_v23.md` | this file |
...
# Final ship
python3 tools/jmerle_backtester.py traders/round3/h_only_v23.py 3 --merge-pnl --no-out
# Expected: total 196,485  (69,480 / 57,120 / 69,885)

# Verify forbidden-imports check
grep -nE "^import os|^from os|import subprocess|exec\(|eval\(" traders/round3/h_only_v23.py
# Expected: empty
```
