...
class Trader:
    ...
    VFE_DECAY_RAMP = 3000
    # Drift-aware entry: block entry when mid velocity is strongly adverse.
    VFE_VEL_WINDOW = 200
    VFE_VEL_BLOCK = 0.025
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        mid_hist.append(mid)
        if len(mid_hist) > self.VFE_VEL_WINDOW:
            mid_hist = mid_hist[-self.VFE_VEL_WINDOW:]
        velocity = 0.0
        if len(mid_hist) >= 50:
            velocity = (mid_hist[-1] - mid_hist[0]) / len(mid_hist)

        if ticks >= self.VFE_WARMUP_TICKS:
            if z >= self.VFE_Z_ENTER and velocity <= self.VFE_VEL_BLOCK:
                side = -1
            elif z <= -self.VFE_Z_ENTER and velocity >= -self.VFE_VEL_BLOCK:
                side = 1
