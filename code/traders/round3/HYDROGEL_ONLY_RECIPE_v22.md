# HYDROGEL_PACK Stand-Alone — Recipe (v22, 2026-04-25)

## TL;DR

`traders/round3/h_only_v22.py` ships at **194,996** over 3 historical
days (70,858 / 56,077 / 68,061). That is **+23,106 (+13.44 %)** over
v8 (171,890), and +2,425 over v21 (192,571). Day 0 +1,088 / day 1
+1,084 / day 2 +253 — first iteration where day 1 gains big (+1k+).

```
cd /Users/sean_tsu_/Downloads/prosperity/IMCP2026
python3 tools/jmerle_backtester.py traders/round3/h_only_v22.py 3 --merge-pnl --no-out
```

The hidden submission day (file `391745`) is the first 100k of day 2's
1M ticks. Day 2 gain on local (+10,427 vs v8) should carry over.

## What changed vs v8

Fourteen-knob retune (13 from v21 + 1 new in v22: vk_dn now POS-conditional).

| Param | v8 | v19 | v20 | v21 | v22 | Notes |
|---|---|---|---|---|---|---|
| `H_ANCHOR` | 9985 | 9983 | 9983 | 9983 | **9983** | cliffs at 9982 / 9986 |
| `H_INV_SKEW_LONG` | (=0.015) | −0.015 | −0.015 | −0.015 | **−0.015** | v19 lever; cliff at ≤−0.017 |
| `H_INV_SKEW_SHORT` | (=0.015) | +0.014 | +0.014 | +0.014 | **+0.014** | v19 lever |
| `H_VK_UP` | (=0.3) | (=0.76) | (=0.78) | 0.78 | **0.78** | upper CLIP vol scaling |
| `H_VK_DN` (sym) | (=0.3) | (=0.76) | (=0.78) | 0.87 | (legacy) | now split (next 2 rows) |
| `H_VK_DN_LONG` | — | — | — | — | **2.7** | **v22 NEW** (+2,425): lower CLIP scaling when long; cliff at 2.8 (-3k day 1) |
| `H_VK_DN_SHORT` | — | — | — | — | **0.85** | when short; cliff at 0.90 (-18k day 0!) |
...
| File | Status |
|---|---|
| `h_only_v22.py` | **SHIP** — 194,996 (+23,106 vs v8, +13.44%) |
| `h_only_v21.py` | prev ship — 192,571 |
...
| `HYDROGEL_ONLY_RECIPE_v22.md` | this file |
...
# Final ship
python3 tools/jmerle_backtester.py traders/round3/h_only_v22.py 3 --merge-pnl --no-out
# Expected: total 194,996  (70,858 / 56,077 / 68,061)

# Verify forbidden-imports check
grep -nE "^import os|^from os|import subprocess|exec\(|eval\(" traders/round3/h_only_v22.py
# Expected: empty
```
