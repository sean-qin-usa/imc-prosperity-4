...
class Trader:
    ...
    VFE_Z_ENTER = 2.0
    VFE_MAX_TAKE_PER_TICK = 8
    # Composite gate: reset Citadel state if 2+ independent signals agree the
    # current position is in a stuck/regime-shift state, not a patient MR hold.
    VFE_COMP_MIN_HOLD = 500       # ticks held before any gate signal can fire
    VFE_COMP_Z_STUCK_TICKS = 2000 # |z|>1 for this many ticks => signal 1
    VFE_COMP_SHORT_EMA_ALPHA = 0.01
    VFE_COMP_EMA_CROSS_SIGMAS = 1.0
    VFE_COMP_VEL_WINDOW = 200
    VFE_COMP_VEL_THRESH = 0.005
    VFE_COMP_PNL_WINDOW = 500
    VFE_COMP_PNL_SLOPE_THRESH = -0.5  # PnL falling >0.5 per tick
    VFE_COMP_FIRE_COUNT = 2
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        short_ema = saved.get("vfe_short_ema")
        z_stuck_count = int(saved.get("vfe_z_stuck", 0))
        mid_hist = saved.get("vfe_mid_hist", [])
        pnl_hist = saved.get("vfe_pnl_hist", [])
        ...
        if short_ema is None:
            ...
            sa_short = self.VFE_COMP_SHORT_EMA_ALPHA
            short_ema = (1.0 - sa_short) * short_ema + sa_short * mid
        dev = mid - ema
        sa = self.VFE_SIGMA_ALPHA
        sigma_var = (1.0 - sa) * sigma_var + sa * (dev * dev)
        sigma = max(math.sqrt(sigma_var), self.VFE_SIGMA_FLOOR)
        ticks += 1
        z_now = dev / sigma if ticks >= self.VFE_WARMUP_TICKS else 0.0

        # Maintain entry anchor / pnl history for the gate
        if cur_pos == 0:
            entry_mid = None
            pnl_hist = []
            ...
            pnl_hist = []
        cur_pnl = (cur_pos * (mid - entry_mid)) if entry_mid is not None else 0.0
        pnl_hist.append(cur_pnl)
        if len(pnl_hist) > self.VFE_COMP_PNL_WINDOW:
            pnl_hist = pnl_hist[-self.VFE_COMP_PNL_WINDOW:]
        mid_hist.append(mid)
        if len(mid_hist) > self.VFE_COMP_VEL_WINDOW:
            mid_hist = mid_hist[-self.VFE_COMP_VEL_WINDOW:]

        # Composite signal counters
        if abs(z_now) > 1.0:
            z_stuck_count += 1
        else:
            z_stuck_count = 0
        sig_z_stuck = z_stuck_count >= self.VFE_COMP_Z_STUCK_TICKS
        sig_ema_cross = (
            cur_pos != 0
            and (
                (cur_pos > 0 and (short_ema - ema) <= -self.VFE_COMP_EMA_CROSS_SIGMAS * sigma)
                or (cur_pos < 0 and (short_ema - ema) >= self.VFE_COMP_EMA_CROSS_SIGMAS * sigma)
            )
        )
        velocity = 0.0
        if len(mid_hist) >= 50:
            velocity = (mid_hist[-1] - mid_hist[0]) / len(mid_hist)
        sig_vel_adverse = cur_pos != 0 and (
            (cur_pos > 0 and velocity <= -self.VFE_COMP_VEL_THRESH)
            or (cur_pos < 0 and velocity >= self.VFE_COMP_VEL_THRESH)
        )
        sig_pnl_falling = False
        if len(pnl_hist) >= 100:
            n = len(pnl_hist)
            slope = (pnl_hist[-1] - pnl_hist[0]) / n
            sig_pnl_falling = slope <= self.VFE_COMP_PNL_SLOPE_THRESH

        fire_count = sum([sig_z_stuck, sig_ema_cross, sig_vel_adverse, sig_pnl_falling])
        held_long_enough = (
            entry_mid is not None
            and (total_ticks - entry_tick) >= self.VFE_COMP_MIN_HOLD
        )
        if held_long_enough and fire_count >= self.VFE_COMP_FIRE_COUNT:
            ema = mid
            short_ema = mid
            sigma_var = 0.0
            ticks = 0
        ...
        saved["vfe_side"] = side
        saved["vfe_short_ema"] = short_ema
        saved["vfe_z_stuck"] = z_stuck_count
        saved["vfe_mid_hist"] = mid_hist
        saved["vfe_pnl_hist"] = pnl_hist
