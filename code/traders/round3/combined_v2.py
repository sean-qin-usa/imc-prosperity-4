"""
Round 3 combined v1 — 2026-04-24.

Layers three sleeves verified independently:
  1. HYDROGEL_PACK: h_only_v8 handler (3-day +171,890 alone).
  2. VEV_4000 / VEV_4500: baseline_v5 synthetic-underlying MM at flat
     sigma=0.23 (3-day ~+8k combined).
  3. Strikes 5000-5500: Timo-P3R3-style IV-residual (port of
     `traders/p3_fresh_claude/p3_combined_v1.py:296-477`).
       - EMA(theo_diff) and EMA(|theo_diff - mean|) per strike.
       - IV scalping (take-side aggressive) on OTM strikes 5300/5400/5500.
       - MR (combined ema_o_dev + iv_dev) on near-ATM strikes
         5000/5100/5200.
     This sidesteps the smile_quadratic dead end: that strategy posted
     PASSIVE quotes inside touch and got zero fills on tight ATM books.
     Timo's IV scalping crosses the touch when residual exceeds the
     half-spread plus THR_OPEN, so fills are not gated by passive flow.
...
# Strike bucketing — v2: scalping bucket fired 0 in v1; move all strikes
# 5000-5500 to MR bucket (combined ema_o_dev + iv_dev signal).
...
class Trader:
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # ---- MR: near-ATM strikes 5000/5100/5200 ----
