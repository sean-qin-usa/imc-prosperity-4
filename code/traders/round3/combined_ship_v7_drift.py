...
class Trader:
    ...
    # Drift-carry overlay: bias inventory long to capture VFE positive drift.
    # Shift effective u_pos by -VFE_LONG_BIAS so the MR sleeve buys (+bias)
    # extra and stops selling once at -bias, leaning long by ~bias on average.
    VFE_LONG_BIAS = 0
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        if self.ENABLE_UNDERLYING_MR:
            ...
            # Drift-carry: shift effective inventory long by VFE_LONG_BIAS
            u_pos_eff = u_pos - self.VFE_LONG_BIAS
            u_max_sell = u_limit + u_pos_eff
            u_max_buy = u_limit - u_pos_eff
