"""
Round 3 — Timo-style clone, FINAL ship (2026-04-24).

Lineage:
  Followed Timo Diehm's Prosperity-3 Round 3 method end-to-end:
  fit a smile, BS-theo, EMA(theo_diff) per strike, signed
  (best_bid|best_ask) - theo - mean_diff scalping, no passive MM.
  Adapted to P4 R3:
    - Smile is FLAT (sigma=0.23 across strikes 5000-5500), no quadratic
      fit needed.
    - Residual std ~0.4 vs Timo's calibration on P3 VR options ~0.7;
      so SCALP_THR_OPEN tuned down from 0.5 → 0.10 (sweep peak).
    - Switch_mean gate (Timo's THR=0.7) is dead at our residual scale;
      removed it.
    - OTM strikes (5400+) showed positive 1-lag autocorrelation
      (momentum, NOT mean-reversion) — excluded from scalp universe.
    - K=5200/5300 net-flat to slightly negative in jmerle backtest
      due to position-build during long-residual regimes; excluded.

3-day backtest result (jmerle, --merge-pnl):
  baseline_v5 (HYDROGEL + VEV_4000/4500 only): 168,031
  h_only_v8   (HYDROGEL alone, peak):           171,890
  THIS FILE (HYDROGEL + synth + IV-scalp):      213,927  (+45,896 vs baseline_v5)

Per-product 3-day total (THIS FILE):
  HYDROGEL_PACK:     ~171,890   (h_only_v8 logic verbatim)
  VEV_4000+4500:      ~13,400   (synthetic-underlying MM, baseline_v5)
  VEV_5000:           ~15,000   (Timo IV-scalp, THR=0.03)
  VEV_5100:           ~13,500   (Timo IV-scalp, THR=0.03)
  Other vouchers:           0   (intentionally untraded; verified -EV)

THR sweep (3-day total, strikes=(5000,5100), per_tick=30):
  THR=0.50: 199,301
  THR=0.40: 202,829
  THR=0.30: 206,698
  THR=0.20: 209,328
  ...
  THR=0.05: 213,778
  THR=0.04: 213,886
  THR=0.03: 213,927  <-- ship (peak)
  THR=0.02: 213,810
  THR=0.01: 213,843
  Plateau across [0.01-0.05] is ~150 PnL wide; ship sits in plateau center.

DON'T:
  1. Add VEV_5200/5300 to scalp — bled in v1; small +EV in mark-to-mid
     but jmerle penalises position-build holding cost.
  2. Add OTM (K=5400+) — positive 1-lag AC = momentum; spread cost
     swamps any chase strategy.
  3. Trade VFE on EMA-MR — alpha 0.12/tick < spread cost 2.5 tick;
     cost us -60k/day in v1.
  4. Add basis arb (S vs vouchers) — paying spread on both legs
     destroys the 1-tick mean-reversion alpha.
  5. Reuse Timo's switch_mean gate at THR=0.7 — never fires here
     (our residual std ~0.4 vs his ~0.7).
  6. Add quadratic smile fit — surplus complexity since the smile
     IS flat enough that a constant sigma absorbs all bias via the
     EMA(theo_diff) residual.

Backtest reproduction:
  cd /Users/sean_tsu_/Downloads/prosperity/IMCP2026
  python3 tools/jmerle_backtester.py traders/round3/timo_clone_FINAL.py 3 --merge-pnl --no-out
"""
...
# HYDROGEL params (h_only_v8 — proven peak 171,890 standalone)
H_ANCHOR = 9985.0
H_CLIP = 33.0
H_TAKE_EDGE = 0.0
H_REDUCE_EDGE = 0.0
...
# VEV_4000/4500 synth-MM (baseline_v5 — proven +13k)
SIGMA = 0.23
VS_TAKE_EDGE = 0.0
VS_PENNY_EDGE = 1.0
VS_INV_SKEW = 0.005
...
# IV scalp (Timo formula, THR sweep peak)
IV_SCALP_STRIKES = (5000, 5100)
THEO_NORM_WINDOW = 20
SCALP_THR_OPEN = 0.03
SCALP_THR_CLOSE = 0.0
...
class Trader:
    ...
    def _trade_iv_scalp(self, name, K, od, pos, S, T, saved):
        # Timo's IV scalp: take side only when (best_touch - bs_theo) crosses
        # threshold relative to its own EMA (residual mean-revert).
        book = self._book(od)
        if not book:
            return []
        ...
        # Scores already include the cross-spread cost (bb < tm < ba):
        #   sell_score = bb - theo - mdiff  (sell at best_bid, i.e. cross down)
        #   buy_score  = ba - theo - mdiff  (buy at best_ask, i.e. cross up)
        # When residual is rich enough that even after the cross we still
        # have edge ≥ THR, we trade.
