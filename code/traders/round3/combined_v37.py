...
class Trader:
    ...
    UNDER_MR_THR = 4  # VFE day-std=15, σ(Δ)=1.13. Start mid-range, sweep.
    UNDER_MAX_CHUNK = 80
    UNDER_MIN_CHUNK = 30
    UNDER_SIZE_SCALE = 50
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # Chunked size to prevent inventory pin (limit 200 → 400 max
        # naïvely, way too aggressive on a 5-tick spread).
        if self.ENABLE_UNDERLYING_MR:
            ...
            mag = abs(ema_o_dev) - self.UNDER_MR_THR
            if mag > 0:
                u_chunk = int(round(min(self.UNDER_MAX_CHUNK,
                                       max(self.UNDER_MIN_CHUNK,
                                           mag * self.UNDER_SIZE_SCALE))))
                if ema_o_dev > 0 and u_max_sell > 0:
                    q = min(u_chunk, u_max_sell)
                    if q > 0:
                        u_orders.append(Order("VELVETFRUIT_EXTRACT", int(u_bw + 1), -q))
                elif ema_o_dev < 0 and u_max_buy > 0:
                    q = min(u_chunk, u_max_buy)
                    if q > 0:
                        u_orders.append(Order("VELVETFRUIT_EXTRACT", int(u_aw - 1), q))
