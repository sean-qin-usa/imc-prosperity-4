...
# Variant source: `fundamental_spot_anchor_v1.py`
# Embedded base: `fundamental_v1.py`
...
class BaseTrader:
    ...
    H_CRASH_BID_SIZE_MULT = 2.0
    H_CRASH_ASK_SIZE_MULT = 0.25
    ...
    def _trade_hydrogel(
        ...
        crash_bid_mult = 1.0
        crash_ask_mult = 1.0
        if touch_mid <= self.H_ANCHOR - self.H_CRASH_SIZEUP_TRIGGER and spread >= self.H_CRASH_MIN_SPREAD:
            crash_bid_mult = self.H_CRASH_BID_SIZE_MULT
            crash_ask_mult = self.H_CRASH_ASK_SIZE_MULT

        bid_size = self._cap_size(
            self.H_MAX_POST_SIZE, working, "buy", buy_cap, limit, crash_bid_mult
        )
        ask_size = self._cap_size(
            self.H_MAX_POST_SIZE, working, "sell", sell_cap, limit, crash_ask_mult
        )
