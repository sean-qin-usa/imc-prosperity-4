"""
Round 3 baseline v19 = v17 with HYDROGEL upgraded to h_only_v9 logic.

Adopts the parallel-chat HYDROGEL deep dive (HYDROGEL_ONLY_RECIPE.md) which
proves HYDROGEL is statistically independent of every other product
(cross-product OOS R² < 0 across all rotations). Replaces the v13c drift-
handler-based HYDROGEL with v9's own-microstructure tuned MM:

  H_ANCHOR     9990 → 9985    (-5 from true mean → biases shorts)
  H_CLIP        30 → 33
  H_REDUCE_EDGE 1.0 → 0.0     (cover at fair, no premium)
  H_PENNY_EDGE  1.5 → 2.0
  H_MAX_POST_SIZE 20 → 18
  H_PASSIVE_OFFSET 12 → 8
  + AR1_BETA = 0.18           (lean against last Δmid; cliff <0.15)
  + TYPICAL_SPREAD = 16       (micro-price gate at narrow spread)
  + CLIP_VOL_K = 0.3          (CLIP grows with realized vol)
  + L3_BETA = -4.0            (deep-book imbalance lean at narrow spread)

The v15 drift handler + v17 cap-flattener are KEPT as out-of-distribution
defenses; they remain dormant on stationary days. Other sleeves unchanged
from v17 (deep-ITM voucher, ATM smile-EMA tweaks, lottery).
...
class Trader:
    ...
    # HYDROGEL — v19 = h_only_v9 params (parallel chat)
    H_ANCHOR = 9985.0           # v9: -5 from true mean → biases shorts (+6.2k joint)
    H_CLIP = 33.0               # v9: 30→33 (joint with anchor)
    H_TAKE_EDGE = 0.0
    H_REDUCE_EDGE = 0.0         # v9: 1.0→0.0 (cover at fair, no premium; +3.0k)
    H_PENNY_EDGE = 2.0          # v9: 1.5→2.0
    H_INV_SKEW = 0.015          # plateau 0.014-0.016
    H_MAX_POST_SIZE = 18        # v9: 20→18 (cliff at 30 = -16k day 1)
    H_PASSIVE_OFFSET = 8.0      # v9: 12→8
    H_WIDE_SPREAD = 8
    H_SHOCK_MOVE = 15.0
    # v9 NEW levers
    AR1_BETA = 0.18             # v9: lean against last Δmid (+1.5k; cliff <0.15)
    TYPICAL_SPREAD = 16         # v9: micro-price gate (+1.9k)
    CLIP_VOL_K = 0.3            # v9: CLIP grows with realized vol (+1.5k)
    DMID_HISTORY = 20
    L3_BETA = -4.0              # v9: L3 imbalance lean at narrow spread
    # v17: cap-flattener supplement. Fires only at-limit with adverse drift.
    H_CAP_NEAR = 1.0
    H_CAP_DRIFT = 35.0
    H_CAP_FLATTEN_QTY = 30
    # v15 drift handler (kept as fallback for trap days; gates rarely fire)
    H_INV_SKEW_DRIFT = 0.045
    H_DRIFT_BLEND = 0.6
    ...
    def _fair_input_h(self, book):
        # h_only_v9: at narrow spread, L3 imbalance carries the signal.
    ...
    def _trade_hydrogel(self, od: OrderDepth, pos: int,
                        ...
                        last_dmid: float, dmid_hist: List[float],
        ...
        # h_only_v9: vol-adaptive CLIP
        if self.CLIP_VOL_K > 0 and len(dmid_hist) >= 3:
            n = len(dmid_hist)
            mean_d = sum(dmid_hist) / n
            std_d = math.sqrt(sum((d - mean_d) ** 2 for d in dmid_hist) / n)
            clip_eff = self.H_CLIP + self.CLIP_VOL_K * std_d
        else:
            clip_eff = self.H_CLIP

        # h_only_v9: micro-price + L3 imbalance fair input
        ...
        if shock:
            ...
            clipped = max(-clip_eff, min(clip_eff, drift_raw))
            fair = self.H_ANCHOR + clipped + self.H_DRIFT_BLEND * (drift_raw - clipped)
            inv_skew = self.H_INV_SKEW_DRIFT
            regime = "drift"
        else:
            fair_adj = max(-clip_eff, min(clip_eff, fair_input - self.H_ANCHOR))
            fair = self.H_ANCHOR + fair_adj
            inv_skew = self.H_INV_SKEW
            regime = "normal"

        # h_only_v9: AR(1) lean against last Δmid
        if last_dmid and not shock:
    ...
    def run(self, state: TradingState):
        ...
        if "HYDROGEL_PACK" in state.order_depths:
            ...
            last_dmid = float(saved.get("h_last_dmid", 0.0))
            dmid_hist = list(saved.get("h_dmid_hist", []))
            h_orders, new_mid, h_state = self._trade_hydrogel(
                state.order_depths["HYDROGEL_PACK"],
                pos.get("HYDROGEL_PACK", 0),
                prev_mid, last_dmid, dmid_hist,
            )
            result["HYDROGEL_PACK"] = h_orders
            # Update Δmid history
            if prev_mid is not None and new_mid is not None:
                d = new_mid - prev_mid
                saved["h_last_dmid"] = d
                dmid_hist.append(d)
                ...
                saved["h_last_dmid"] = 0.0
            saved["h_dmid_hist"] = dmid_hist
