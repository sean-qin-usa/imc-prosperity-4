"""
Round 3 combined SHIP v27 — 2026-04-25 (session 8 close, after v25 live=525,560).
...
class Trader:
    ...
    ATM_PENNY_EDGE = 0.0
    ATM_INV_SKEW = 0.0
    # v28 stat-arb finding: VEV_5400/5500 Δmid AR1 = -0.25/-0.24 (cross-day stable).
    # Lean fair against last move; same mechanism that pays in HYDROGEL.
    ATM_AR1_BETA = 0.20
    ATM_WIDE_SPREAD = 1
    ...
    VS_PENNY_EDGE = 1.0
    VS_INV_SKEW = 0.0
    # v28 stat-arb finding: VEV_4000/4500 Δmid AR1 = -0.28/-0.22 (cross-day stable).
    # Lean fair against last move on the synth-MM strikes.
    VS_AR1_BETA = 0.20
    VS_MAX_POST_SIZE = 40
    VS_WIDE_SPREAD = 3
    ...
    def _trade_synth_voucher(self, name, K, od, pos, S, T, last_dmid=0.0):
        ...
        # v28: AR1 lean against last own-mid move (Δmid AR1 = -0.28 on V4000,
        # -0.22 on V4500, cross-day stable; lean is monetized via tighter
        # reversion-aligned quotes).
        if last_dmid:
            fair -= self.VS_AR1_BETA * last_dmid
        working = pos
        orders: List[Order] = []
        for ap, av in book["sells"].items():
            cap = limit - working
    ...
    def _trade_atm(self, name, K, od, pos, S, T, residual, last_dmid=0.0):
        limit = self.LIMITS[name]
        ...
        # v28: AR1 lean against last own-mid move (Δmid AR1 = -0.25 on V5400,
        # -0.24 on V5500, cross-day stable).
        if last_dmid:
            bs_theo -= self.ATM_AR1_BETA * last_dmid
    ...
    def run(self, state: TradingState):
        ...
        if u_od is not None:
            ...
            if u_book is not None:
                ...
                last_synth_mid = saved.get("last_synth_mid", {})
                for name, K in self.SYNTH_STRIKES.items():
                    od = state.order_depths.get(name)
                    if od is not None:
                        b = self._book(od)
                        cur_mid = b["touch_mid"] if b else None
                        prev = last_synth_mid.get(name)
                        last_dmid = (cur_mid - prev) if (cur_mid is not None and prev is not None) else 0.0
                        result[name] = self._trade_synth_voucher(
                            name, K, od, pos.get(name, 0), S_synth, T_synth,
                            last_dmid,
                        )
                        if cur_mid is not None:
                            last_synth_mid[name] = cur_mid
                saved["last_synth_mid"] = last_synth_mid
        ...
        if u_od is not None:
            ...
            if u_book is not None:
                ...
                saved["last_atm_mid"] = last_atm_mid
