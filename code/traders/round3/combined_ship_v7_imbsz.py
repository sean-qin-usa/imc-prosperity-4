"""
Round 3 combined SHIP v7 — 2026-04-25 (session 6, late).

DELTA vs ship_v6 — three additional knobs from h_only_v16 sweep
(parallel HYDROGEL-only track) re-tested in the combined chassis:

  DMID_HISTORY   50 → 150   (+128 single-knob; plateau 100-200)
  H_PENNY_EDGE   3.0 → 3.5  (+50 single-knob; plateau 3.0-4.0)
  H_TAKE_EDGE    0.0 → 0.6  (NEW lever; +1.2k joint with above; cliff
                             at 0.65+ where regime crosses lose money;
                             RE must stay 0 — RE>0 with TE>0 leaks -5k)

(Retained from v6: CLIP_VOL_K 0.795, AR1_BETA 0.25.)

3-day bt: **318,576** (+6,452 over ship_v4 baseline 312,124,
+1,216 over ship_v6 317,360).

Discovery process (session 6):
  1. Re-tested CLIP_VOL_K joint with DMID_HISTORY (prior memo had
     CLIP > 0.3 marked as dead in combined; this was at fixed DH=20).
     Found CLIP=0.795 + DH=50 = +4.7k (combined_ship_v6).
  2. Memory of `h_only_v16` (181k standalone) flagged DH=150, TE=0.5
     as new lever sets.  Tested in combined: DH=150 holds, TE=0.5/0.6
     adds +1.2k joint.
  3. RE>0 with TE>0 — confirmed h_only_v16's warning: -5k bt.

Other variants tested and rejected this session:
  - All 8 MR-strike expansions (move ITM strikes to MR, P3 winning move):
    every one regressed -25k to -485k.  P4 wide deep-ITM spread (V4000
    ~21 ticks) breaks the P3 transfer.
  - SIGMA sensitivity verified high in P4 (peak 0.23 sharp; ±0.05 → -60k).
  - iv_dev weight peaked unchanged at 2.25.
  - PE=4/5 regress; INV_SKEW=0.013 craters -12k; H_CLIP≠33 regresses;
    H_ANCHOR≠9983 regresses; post=19 craters -35k; CLIP=0.80 craters -25k
    (cliff edge unchanged from v15/v16).

Live forecast: HYDROGEL converts at 2.27x.  +6,452 bt → +14,650 live
expected → ship_v4 live 395,880 → ship_v7 ~410,500 live.

Validator-clean (no forbidden imports).

Sleeves (unchanged from v4/v6):
  1. HYDROGEL_PACK: own-microstructure MM with vol-adaptive CLIP, AR(1)
     mean-reversion lean, anchor 9983, NEW take-edge=0.6.
  ...
  4. VEV_5400 / VEV_5500: ATM smile-EMA MM (BS_theo + slow EMA residual).
  5. VEV_6000 / VEV_6500: lottery posts (bid 0, ask 1).
  6. VELVETFRUIT_EXTRACT underlying MR: cross spread on |ema_o_dev| > 5.
...
class Trader:
    ...
    VS_MAX_POST_SIZE = 40
    VS_WIDE_SPREAD = 3
    # Imbalance-aware quote sizing for synth-MM (port from clean_alpha.py).
    # imb = (bv - av) / (bv + av). When |imb| > VS_IMB_STRONG, boost the
    # favorable side and shrink the adverse side.
    VS_IMB_STRONG = 0.30
    VS_IMB_FAVORABLE_BOOST = 1.8
    VS_IMB_ADVERSE_SHRINK = 0.2
    # Take-side imbalance gate: when imbalance points against the take direction,
    # add this much extra edge requirement (buying asks in falling-price regime).
    VS_TAKE_ADVERSE_EXTRA = 1.0
    ...
    def _trade_synth_voucher(self, name, K, od, pos, S, T):
        ...
        bv0 = book["bv"]; av0 = book["av"]
        denom0 = bv0 + av0
        imb0 = (bv0 - av0) / denom0 if denom0 > 0 else 0.0
        # Buying asks (lifting offers) is adverse when imb<<0 (price falling).
        # Selling bids (hitting bids) is adverse when imb>>0 (price rising).
        buy_take_extra = self.VS_TAKE_ADVERSE_EXTRA if imb0 < -self.VS_IMB_STRONG else 0.0
        sell_take_extra = self.VS_TAKE_ADVERSE_EXTRA if imb0 > self.VS_IMB_STRONG else 0.0
        ...
        for ap, av in book["sells"].items():
            ...
            if ap <= skew - self.VS_TAKE_EDGE - buy_take_extra:
                q = min(av, cap)
                if q > 0:
        ...
        for bp, bv in book["buys"].items():
            ...
            if cap <= 0:
                break
            skew = fair - self.VS_INV_SKEW * working
            if bp >= skew + self.VS_TAKE_EDGE + sell_take_extra:
                q = min(bv, cap)
                if q > 0:
        ...
        # Imbalance-aware sizing: boost favorable side, shrink adverse side.
        bv = book["bv"]; av = book["av"]
        denom = bv + av
        if denom > 0:
            imb = (bv - av) / denom
