"""
Round 3 combined SHIP v21 — 2026-04-25 (session 8, VEV drift carry add).

DELTA vs v20 (443,484 bt) — extends drift carry to the V5000/V5100/V5200/V5300
voucher MR strikes, gated by no-MR-signal-fired (combined < OPT_MR_THR).
When the MR is silent and pos < target, post passive bid stack (size 30 at bb,
size 20 at bb-1) up to position 300 long.

3-day backtest match-trades all:  +445,078 (137,202 / 157,416 / 150,460).
3-day backtest match-trades none: +280,208 (alpha floor unchanged from v11/v15).
vs v20 (443,484): +1,594 — entirely matching-engine fills (alpha floor flat).

Same passive-make-on-delta-1 mechanism that gave v15 its +14k bt → +18k live
(127% conversion).  Expected live: similar 1.0-1.3x conversion on the +1,594.

Discovery (session 8 by this Claude):
  - VEV drift carry target=50 → +943; 100 → +1,079; 150 → +1,202;
    200 → +1,222; 250 → +1,282; 275 → +1,563; 300 → +1,594. Caps at
    position limit (300). All 4 strikes (5000/5100/5200/5300) needed
    for full gain — subset 5000-5200 only +409.
  - Multi-level (bb / bb-1) bt-invariant; ships anyway for live queue
    priority (mirrors v20's VFE multi-level approach).

Other tests this session (rejected):
  - HYDROGEL queue-join layer at bb (size 5-30): -12 to -15k bt.  Adverse
    selection on wall walks; same mechanism as h_only_v15 layer-2 dead-list.
  - EOD VFE flat (last 100/200/500 ticks unwind): -1k to -5k.  Drift wants
    to hold to end-of-day.
  - VFE multi-level on existing bb / bb-1 / bb-2: bt-invariant (queue
    priority only).  v20 already has it.
  - Persist-during-MR carry: -47k.  Carry buys conflict with MR sells.
  - Synth-carry sizing & lottery sizing sweeps: bt-invariant.

What v21 keeps from v20 (cumulative session 4-8 work):
  - HYDROGEL h_only_v16 retune + 4-knob v12 micro-tune
  - VS_INV_SKEW=0.0 (h_only_v16 +25k single-knob)
  - VFE drift carry multi-level (target 200, 3 price levels)
  - V4000/V4500 synth carry (target 200, 2 price levels)
  - Voucher MR retune (THEO_NORM=25, OPT_MR_WINDOW=44, OPT_MR_THR=6.3)
  - L1 imbalance-aware sizing on synth-MM
  - ATM smile-EMA on 5400/5500
  - Lottery posts on 6000/6500

Validator-clean (no forbidden imports).

Live forecast: HYDROGEL converts at 2.27x; VFE drift carry at 1.27x.
The new +1,594 is delta-1-passive-make like VFE carry, so expect ~1.0-1.3x
live conversion → ~+2k live → ship_v21 ~419,500 live (vs v15 live 417,605).
"""
...
class Trader:
    ...
    def _trade_synth_voucher(self, name, K, od, pos, S, T):
        ...
        # v20: synth voucher drift carry (V4000/V4500 follow VFE drift, delta=1, limit=300).
        # Post passive bid at bb (joining wall) when below carry target. Backtest won't
        # show gain (matcher caps fills) but live should benefit from queue priority.
        SYNTH_CARRY_TARGET = 200
        SYNTH_CARRY_SIZE_BB = 80
        SYNTH_CARRY_SIZE_BB1 = 40
        if working < SYNTH_CARRY_TARGET:
            cap_total = min(self.LIMITS[name] - working, SYNTH_CARRY_TARGET - working)
            if cap_total > 0:
                # Post 80 at bb + 40 at bb-1
                sz_bb = min(cap_total, SYNTH_CARRY_SIZE_BB)
                if sz_bb > 0:
                    orders.append(Order(name, bb, sz_bb))
                    cap_total -= sz_bb
                if cap_total > 0 and bb - 1 > 0:
                    sz_bb1 = min(cap_total, SYNTH_CARRY_SIZE_BB1)
                    if sz_bb1 > 0:
                        orders.append(Order(name, bb - 1, sz_bb1))
        return orders
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        if self.ENABLE_UNDERLYING_MR:
            ...
            if ema_o_dev > self.UNDER_MR_THR and u_max_sell > 0:
                ...
                # No MR signal — multi-level passive long-bias drift carry.
                # Post at bb (queue join) + bb-1 + bb-2 with stacked sizes.
                u_book = self._book(u_od)
                if u_book is not None and u_pos < self.UNDER_DRIFT_TARGET:
                    cap_total = min(u_max_buy, self.UNDER_DRIFT_TARGET - u_pos)
                    if cap_total > 0:
                        bb = u_book["bb"]
                        # Three price levels: aggressive, moderate, deep
                        for offset, sz in [(0, 80), (-1, 50), (-2, 30)]:
                            if cap_total <= 0: break
                            actual_sz = min(cap_total, sz)
                            if actual_sz > 0 and bb + offset > 0:
                                u_orders.append(Order("VELVETFRUIT_EXTRACT", bb + offset, actual_sz))
                                cap_total -= actual_sz
