"""
Round 3 combined SHIP v15 — 2026-04-25 (session 7-8, drift carry add).

v15 = v14 + VFE drift carry (passive long bias on VELVETFRUIT_EXTRACT
to capture the empirical +0.002/tick mid drift across days).

3-day backtest match-trades all:  +443,484 (136,310 / 156,750 / 150,423).
3-day backtest match-trades none:  +280,208 (alpha floor unchanged from v11).
vs v14 (429,016): +14,468 / no alpha-floor change → entire delta is
passive-fill on VFE bid posts at bb. Day 0 +13.4k (where VFE was the
weakest sleeve in v11/v14), day 1 +6.5k, day 2 -5.2k (small offset
from skipping MR triggers in favor of carry posts).

Drift carry mechanics:
  When |ema_o_dev| < UNDER_MR_THR (no MR trigger), if u_pos < 200,
  post a bid at bb with size 30. Slow accumulation toward full long.
  Captures VFE +0.002 ticks/timestamp = +20 ticks/day at full pos.

UNDER_MR_THR plateau verified at v15 base: 5→408k, 5.5→426k, 6→443k,
6.5→435k, 7→426k. Same v11 cliff structure preserved.

Older v14 history below.
==================================================================
...
Submission-safe (no forbidden imports).
...
class Trader:
    ...
    CLIP_VOL_K = 0.795
    DMID_HISTORY = 150
    # Deep-book (L2+L3) imbalance — opposite sign from L1, β≈-20, R²≈0.11
    # over all 3 days. Fair-shift was tested: regresses all values
    # (likely already captured by AR1+inv-skew). Try size-shrink overlay
    # instead: when deep_imb > +THR (heavy deep bids → mid will fall),
    # shrink bid size; symmetric on ask side.
    H_DEEP_K = 0.0
    H_DEEP_THR = 0.0     # threshold; 0 disables
    H_DEEP_SHRINK = 1.0  # multiplier on side-against-prediction quote
    ...
    UNDER_DRIFT_TARGET = 200   # full long position when no MR signal
    UNDER_CARRY_BID_SIZE = 30  # passive bid size on bb to accumulate

    # ----- VFE aggressor-flow signal (NEW v15_aggr) ----------------------
    # Per-tick analysis of state.market_trades[VFE]: classify each trade as
    # buyer-aggressor (price >= ask) → +qty, seller-aggressor (price <= bid)
    # → -qty, sum into AGG_SIGNAL.
    #
    # Empirical (data/round3): K=1 single-tick lookback dominates; n_active
    # ≈ 440/day; corr(AGG, next_dmid) = +0.28/+0.22/+0.34 across 3 days.
    # E[next_dmid | AGG ≥ +5] = +0.55 ticks; E[ | AGG ≤ -5] = -0.18 ticks.
    # The drift carry posts only bids; aggressor lets us:
    #   (1) step inside-spread on strong buyer-flow → grab fills before mid lifts,
    #   (2) shrink bid size on strong seller-flow → avoid loading at falling mid.
    # Empirical (data/round3): K=1 single-tick lookback dominates; n_active
    # ≈ 440/day; corr(AGG, next_dmid) = +0.28/+0.22/+0.34 across 3 days.
    # E[next_dmid | AGG ≥ +5] = +0.55 ticks; E[ | AGG ≤ -5] = -0.18 ticks.
    # Sweep peak at THR=3, SHRINK=0.3, INSIDE=0, BOOST=1.0:
    # +50 jmerle bt vs v15; bundle-cal on 399113 cal_minus_official
    # shrinks from -360 (v15) to -0.3 (v15_aggr) → ~+360 expected live.
    # bb+1 INSIDE step lost -11k (queue position); BOOST has no effect
    # (matcher fill cap binds); only bid-size SHRINK on seller flow paid.
    VFE_AGGR_THR = 3
    VFE_AGGR_BID_INSIDE = 0
    VFE_AGGR_BID_BOOST = 1.0
    VFE_AGGR_BID_SHRINK = 0.3
    ...
    def _book(self, od: OrderDepth):
        ...
        sorted_b = sorted(buys.items(), reverse=True)
        sorted_a = sorted(sells.items())
        # L2+L3 deep volumes (positions 2..3 of sorted ladder)
        deep_bv = sum(v for _, v in sorted_b[1:3])
        deep_av = sum(v for _, v in sorted_a[1:3])
        return {
            "buys": dict(sorted_b),
            "sells": dict(sorted_a),
            "bb": bb, "ba": ba, "bv": buys[bb], "av": sells[ba],
            "deep_bv": deep_bv, "deep_av": deep_av,
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist):
        ...
        # Deep-book imbalance overlay (L2+L3, opposite sign vs L1)
        if self.H_DEEP_K != 0.0 and (book["deep_bv"] + book["deep_av"]) > 0:
            deep_imb = (book["deep_bv"] - book["deep_av"]) / (book["deep_bv"] + book["deep_av"])
            fair -= self.H_DEEP_K * deep_imb
        ...
        # Deep-book imbalance size overlay (negative-sign predictor of next-Δmid)
        if self.H_DEEP_THR > 0 and (book["deep_bv"] + book["deep_av"]) > 0:
            deep_imb = (book["deep_bv"] - book["deep_av"]) / (book["deep_bv"] + book["deep_av"])
            if deep_imb > self.H_DEEP_THR:
                # heavy deep bids → next-Δmid down → don't load on bid side
                bid_size = int(round(bid_size * self.H_DEEP_SHRINK))
            elif deep_imb < -self.H_DEEP_THR:
                # heavy deep asks → next-Δmid up → don't sell into rising mid
                ask_size = int(round(ask_size * self.H_DEEP_SHRINK))
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved, vfe_agg=0):
            ...
            if ema_o_dev > self.UNDER_MR_THR and u_max_sell > 0:
                ...
                if u_book is not None and u_pos < self.UNDER_DRIFT_TARGET:
                    ...
                    if cap > 0:
                        ...
                        # Aggressor overlay: optionally lift inside-spread,
                        # boost size on buyer flow, shrink on seller flow.
                        if vfe_agg >= self.VFE_AGGR_THR:
                            if self.VFE_AGGR_BID_INSIDE > 0:
                                inside = u_book["bb"] + self.VFE_AGGR_BID_INSIDE
                                if inside < u_book["ba"]:
                                    bid_price = inside
                            sz = min(u_max_buy, int(round(sz * self.VFE_AGGR_BID_BOOST)))
                        elif vfe_agg <= -self.VFE_AGGR_THR:
                            sz = int(round(sz * self.VFE_AGGR_BID_SHRINK))
                        if sz > 0:
    ...
    def run(self, state: TradingState):
        ...
        # Compute VFE aggressor signal from this tick's market_trades.
        vfe_agg = 0
        vfe_book_for_agg = self._book(u_od) if u_od is not None else None
        if vfe_book_for_agg is not None:
            mt = state.market_trades.get("VELVETFRUIT_EXTRACT", []) or []
            bb_a, ba_a = vfe_book_for_agg["bb"], vfe_book_for_agg["ba"]
            for tr in mt:
                px = tr.price; qty = tr.quantity
                if px >= ba_a - 0.5:
                    vfe_agg += qty
                elif px <= bb_a + 0.5:
                    vfe_agg -= qty
        for prod, orders in self._trade_iv_residual(state, pos, saved, vfe_agg=vfe_agg).items():
            if prod in result:
