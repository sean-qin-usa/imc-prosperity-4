"""
Round 3 combined SHIP v13 — 2026-04-25 (session 5, synth-MM imbalance gate).
Adds clean_alpha-style L1-imbalance-aware quote sizing on V4000/V4500
synth-MM only.  All other sleeves identical to v11.

3-day backtest: 428,742 (122,899 / 150,246 / 155,597) — net -12 vs v11.
Backtest is essentially neutral.  Conservative ship: should not hurt;
expected to help live when adverse-selection events fire.

Imbalance signal in data (verified):
  - On synth book, |imb_L1|>0.30 fires only in tight-spread regime
    (~1.5% of ticks; spread 7-12 on V4500, 9-12 on V4000).
  - When it fires: next-tick mid moves +5.5/-5.0 (V4000) or +4.1/-3.8
    (V4500) with >99% hit rate.  Real, sharp signal.
  - In wide-spread regime (98% of ticks) imbalance is essentially 0
    — bots pad both sides symmetrically, so the gate never trips.

Why backtest barely moves despite signal:
  - Pure sizing port (v13) shows -12 vs v11.  Negligible because the
    fill simulator's fills on synth come mostly from take-loop crosses
    (driven by VFE deltas) and a few phantom trades, not from passive
    posts inside the spread.  In tight-spread regime our bb+1/ba-1
    posts rarely match phantom trades on V4500 (zero on day 0!).
  - Aggressive fair-shift variants (lean fair +0.5..+3 ticks when
    imb fires) all REGRESSED -10k to -20k:
      shift=0.5  -10,572
      shift=1.0  -18,812
      shift=1.5  -19,581
      shift=2.0  -20,139
      shift=3.0  -20,105
    Day 0 gets a small lift (+344..+1,170) but days 1-2 lose much
    more from over-aggressive cross-the-book takes that the next-tick
    +5 move doesn't fully recoup in the simulator.
  - Take-guard variant (skip cross when imb adverse) ~0 effect since
    the take loop's `ap <= fair` condition rarely fires in tight regime
    anyway.

Therefore v13 ships ONLY the sizing port — defensive change that costs
nothing in backtest and may convert live where the fill model is more
adverse-selection sensitive.

Original v11 header below:
...
class Trader:
    ...
    VS_MAX_POST_SIZE = 40
    VS_WIDE_SPREAD = 3
    # L1 imbalance gate (clean_alpha port). On synth, |imb|>0.3 only fires
    # when spread compresses to 7-12 (~1.5% of ticks); at those moments
    # next-tick mid moves +5.5/-5 (V4000) or +4/-3.8 (V4500) with >99% hit
    # rate.  Shrink the side imb pushes against (avoid pickoff), boost the
    # side imb pushes toward (rebound capture).
    VS_IMB_STRONG = 0.30
