...
class Trader:
    ...
    # Combined gate: fire ONLY if BOTH (peak-current >= drawdown) AND
    # (current PnL <= floor). Profitable round-trips never trip; only
    # genuine sustained-bleed positions get reset.
    VFE_TRAIL_DRAWDOWN = 10000.0
    VFE_TRAIL_MIN_TICKS = 500
    VFE_TRAIL_PNL_FLOOR = -5000.0
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        if (
            ...
            cur_pnl = cur_pos * (mid - entry_mid)
            if (
                peak_pnl - cur_pnl >= self.VFE_TRAIL_DRAWDOWN
                and cur_pnl <= self.VFE_TRAIL_PNL_FLOOR
            ):
