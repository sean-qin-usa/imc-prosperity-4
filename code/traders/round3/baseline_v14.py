"""
Round 3 baseline v14 — 2026-04-24 (chase coupled alpha session).

v14 = v13c + basis-aware MAKE on V4000 / V4500.

The deep-ITM voucher fair is already BS_theo(S, K) which on these
strikes equals S - K + tiny — so the basis B = S - V_mid - K is
effectively (BS_fair - V_mid). Adding B back to fair is double-
counting. The exploit instead lives in QUOTE SIZING:

  When basis > 0 (V is cheap relative to S):
    - post BIGGER bid (collect more passive long fills)
    - post SMALLER ask (don't sell something that's about to revert up)
  When basis < 0 (V is rich):
    - post SMALLER bid
    - post BIGGER ask

  bid_size = MAX_POST_SIZE + round(BASIS_SIZE_GAIN * basis)
  ask_size = MAX_POST_SIZE - round(BASIS_SIZE_GAIN * basis)
  (each capped at [0, BASIS_SIZE_BIAS_MAX])

At extreme basis (|basis| > BASIS_GATE_THRESHOLD ≈ 5), gate the
unfavorable side entirely (set size 0). This handles the rare ~1.8%
of ticks where basis goes ±3-7 and the signal is strongest.

We do NOT cross spreads on basis arb (handoff verified that's -EV
because spread > signal). We only bias passive liquidity provision.

HYDROGEL (drift handler) unchanged from v13c. Logger now also emits
basis values for V4000 and V4500 for post-mortem.

v13c history below for reference:
...
class Trader:
    ...
    # Voucher — v14 adds basis-aware sizing
    SIGMA = 0.23
    VS_TAKE_EDGE = 0.0
    VS_PENNY_EDGE = 1.0
    VS_INV_SKEW = 0.005
    # Basis B = S - V_mid - K. Bias size by round(GAIN * B). When B = +3
    # (V is cheap by 3 ticks, ~3.8 std): bias = 90, bid_size = 190, ask_size
    # = 10. Capped to BIAS_MAX so we never go below 0 from the bias.
    BASIS_SIZE_GAIN = 50            # was 30 — try harder
    BASIS_SIZE_BIAS_MAX = 100
    BASIS_GATE_THRESHOLD = 2.0      # was 5 — gate at ~2.5 std (much more often)
    BASIS_QUOTE_THRESHOLD = 1.0     # was 2 — fire penny-inside at ~1.3 std
    ...
    def _trade_deep_itm(self, name: str, K: int, od: OrderDepth,
        ...
        # Basis = S - V_mid - K. >0 when V is cheap relative to synthetic.
        # For deep-ITM, BS_fair ≈ S - K, so basis ≈ BS_fair - V_mid; using
        # basis to bias size (not fair) avoids double-counting.
        basis = S - book["touch_mid"] - K
        working = pos
        orders: List[Order] = []
        ...
        # MAKE — basis-aware sizing
        skew = fair - self.VS_INV_SKEW * working
        buy_cap = max(0, limit - working)
        sell_cap = max(0, limit + working)

        # Basis-driven size shift: positive basis -> bigger bid, smaller ask.
        size_shift = max(-self.BASIS_SIZE_BIAS_MAX,
                         min(self.BASIS_SIZE_BIAS_MAX,
                             round(self.BASIS_SIZE_GAIN * basis)))
        bid_target = max(0, self.VS_MAX_POST_SIZE + size_shift)
        ask_target = max(0, self.VS_MAX_POST_SIZE - size_shift)

        # Hard gating at extreme basis: zero out the unfavorable side.
        if basis > self.BASIS_GATE_THRESHOLD:
            ask_target = 0
        elif basis < -self.BASIS_GATE_THRESHOLD:
            bid_target = 0

        bid_size = self._cap_size(bid_target, working, "buy",
                                  buy_cap, limit)
        ask_size = self._cap_size(ask_target, working, "sell",
                                  sell_cap, limit)

        if spread >= self.VS_WIDE_SPREAD:
            # v14: basis-driven quote bias.
            # basis > QUOTE_THRESHOLD: V cheap -> bid penny-inside (win more
            # passive long fills); ask stays at touch.
            # basis < -QUOTE_THRESHOLD: opposite.
            bid_offset = 0
            ask_offset = 0
            if basis > self.BASIS_QUOTE_THRESHOLD:
                bid_offset = 1   # bid at bb+1 (one tick inside)
            elif basis < -self.BASIS_QUOTE_THRESHOLD:
                ask_offset = -1  # ask at ba-1 (one tick inside)
            if self.VS_POST_AT_TOUCH:
                bid_price = min(bb + bid_offset, math.floor(skew - self.VS_PENNY_EDGE))
                ask_price = max(ba + ask_offset, math.ceil(skew + self.VS_PENNY_EDGE))
            else:
