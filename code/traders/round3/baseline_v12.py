"""
Round 3 baseline v11 — 2026-04-24 (AFK research session).

Hypothesis: v6's deep-ITM handler posts passively at bb+1 / ba-1 with
size=40, which fills rarely because the inside-touch queue is competitive.
If we post AT bb and AT ba (or bb-1 / ba+1) with bigger size, we might
catch:
  - stop-losers crossing the spread aggressively
  - misprice arbitrageurs sweeping multiple levels
  - occasional uninformed flow
The reasoning: VEV_4000 spread is ~21 ticks, so even filling at touch
leaves us with the spread/2 = ~10.5 ticks of theoretical edge vs fair
if our fair (BS) is correct.

v11 = v6 + aggressive passive posting on V4000/V4500:
  * VS_MAX_POST_SIZE bumped 40 → 100
  * Post AT bb and AT ba (not +1/-1) to win priority
  * Tight guard: only post if fair_diff_to_touch > 3 ticks (confidence)

If v11 regresses vs v6, it confirms passive-fill is not the missing alpha.
"""
...
class Trader:
    ...
    # Deep-ITM voucher — v11 aggressive variant
    SIGMA = 0.23
    VS_TAKE_EDGE = 0.0
    VS_PENNY_EDGE = 1.0
    VS_INV_SKEW = 0.005
    VS_MAX_POST_SIZE = 100       # was 40 — test if larger size captures more
    VS_WIDE_SPREAD = 3
    VS_POST_AT_TOUCH = True      # post AT bb/ba not bb+1/ba-1
    ...
    def _trade_deep_itm(self, name: str, K: int, od: OrderDepth,
        ...
        if spread >= self.VS_WIDE_SPREAD:
            if self.VS_POST_AT_TOUCH:
                # v11: join the bid/ask rather than jump inside
