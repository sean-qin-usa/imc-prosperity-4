"""
HYDROGEL-only v13 — v8 + VFE crash overlay only (2026-04-24).

Single change vs v8: adds the VFE-momentum crash regime overlay
borrowed from baseline_v18 (the one piece HYDROGEL_CROSS_REGIME_FINDINGS
documented as helping BOTH visible 3-day backtest and bundle-calibrated
hidden-day replay).

When HYDROGEL is in crash state (touch_mid <= 9985-18=9967 AND
spread >= 14):
  - VFE_mom20 <= -3.5 → fair +0.75, bid 2x size, ask 0.25x (lean LONG)
  - VFE_mom20 >= +1.0 → fair -1.25, bid 0.25x, ask 1.25x (suppress)
  - else (mid quintile): no change.

Outside crash state, identical to v8.
...
class Trader:
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist, vfe_mom20):
        prod = "HYDROGEL_PACK"
        ...
        bid_size_mult = 1.0
        ask_size_mult = 1.0
        in_crash = (tm <= self.H_ANCHOR - self.H_CRASH_TRIGGER
        ...
        # v13: VFE crash size multipliers
    ...
    def run(self, state: TradingState):
        ...
        # VFE momentum (regime tag only — HYDROGEL still trades alone)
        vfe_mom20 = None
