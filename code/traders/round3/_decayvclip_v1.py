...
class Trader:
    ...
    VFE_Z_ENTER = 2.0
    VFE_MAX_TAKE_PER_TICK = 8
    # Size-decay: when |z| stays > 0.5 for VFE_DECAY_GRACE ticks (no
    # mean-reversion progress), shrink target position toward 0 over the next
    # VFE_DECAY_RAMP ticks. Soft stop — no flip-and-reset cost; healthy MR
    # cycles always touch z=0 within the grace window so decay never engages.
    VFE_DECAY_GRACE = 1000
    VFE_DECAY_RAMP = 3000
    ...
    # ----- Portfolio-delta cap (post-process hedge) -----
    # After all per-product orders are computed, sum projected positions
    # converted to VFE-equivalent delta via BS. If |net delta| > cap, add
    # offsetting VFE orders. Targets the 85%-of-day-3-bleed voucher problem.
    # Pcap is GATED on Citadel decay being engaged. Training never engages
    # decay -> no pcap cost. Day-3 stuck regime engages decay -> pcap kicks in.
    ENABLE_PCAP = True
    PCAP_DELTA_CAP = 200
    PCAP_VFE_HEDGE_LIMIT = 6
    PCAP_REQUIRE_DECAY = True  # only fire when Citadel decay < 1.0
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        ticks_since_z0 = int(saved.get("vfe_ticks_since_z0", 0))
        ...
        z = dev / sigma if ticks >= self.VFE_WARMUP_TICKS else 0.0
        if abs(z) <= 0.5:
            ticks_since_z0 = 0
        else:
            ticks_since_z0 += 1
        ...
        # Size-decay: shrink target after grace expires, fully zero at grace+ramp
        decay = 1.0
        if ticks_since_z0 > self.VFE_DECAY_GRACE:
            over = ticks_since_z0 - self.VFE_DECAY_GRACE
            decay = max(0.0, 1.0 - over / self.VFE_DECAY_RAMP)

        saved["vfe_ema_long"] = ema
        saved["vfe_sigma_var"] = sigma_var
        saved["vfe_ticks"] = ticks
        saved["vfe_side"] = side
        saved["vfe_ticks_since_z0"] = ticks_since_z0

        target = int(side * self.LIMITS["VELVETFRUIT_EXTRACT"] * decay)
        diff = target - cur_pos
        if diff == 0:
            return []
    ...
    def run(self, state: TradingState):
        ...
        # 6. Portfolio-delta cap: hedge net VFE-equivalent exposure with VFE.
        # GATED on Citadel decay being engaged (saved.vfe_ticks_since_z0 > grace).
        # Means: training (no stuck regime) -> no pcap cost; day-3 stuck -> active.
        decay_engaged = False
        if self.PCAP_REQUIRE_DECAY:
            tsz = int(saved.get("vfe_ticks_since_z0", 0))
            if tsz > self.VFE_DECAY_GRACE:
                decay_engaged = True
        else:
            decay_engaged = True
        if self.ENABLE_PCAP and decay_engaged and u_od is not None:
            u_book = self._book(u_od)
            if u_book is not None:
                S_pc = u_book["touch_mid"]
                T_pc = self._tte_years(state.timestamp)
                portfolio_delta = float(pos.get("VELVETFRUIT_EXTRACT", 0))
                all_strikes = {
                ...
                for name, K in all_strikes.items():
                    pos_v = pos.get(name, 0)
                    if pos_v == 0:
                        continue
                    _, d, _ = self._opt_bs(S_pc, K, T_pc, self.SIGMA)
                    portfolio_delta += pos_v * d
                excess = abs(portfolio_delta) - self.PCAP_DELTA_CAP
                if excess > 0:
                    cur_vfe = pos.get("VELVETFRUIT_EXTRACT", 0)
                    bb_v = u_book["bb"]; ba_v = u_book["ba"]
                    if portfolio_delta > 0:
                        # Net long delta -> short VFE
                        room = self.LIMITS["VELVETFRUIT_EXTRACT"] + cur_vfe
                        qty = min(int(excess), room, self.PCAP_VFE_HEDGE_LIMIT)
                        if qty > 0:
                            hedge = Order("VELVETFRUIT_EXTRACT", int(bb_v), -qty)
                            result.setdefault("VELVETFRUIT_EXTRACT", []).append(hedge)
                    else:
                        room = self.LIMITS["VELVETFRUIT_EXTRACT"] - cur_vfe
                        qty = min(int(excess), room, self.PCAP_VFE_HEDGE_LIMIT)
                        if qty > 0:
                            hedge = Order("VELVETFRUIT_EXTRACT", int(ba_v), qty)
                            result.setdefault("VELVETFRUIT_EXTRACT", []).append(hedge)

                # 6b. While decay is engaged, actively trim voucher inventory
                # in the same direction as the over-delta. Use TOP-of-book
                # passive orders so we only sell into existing demand.
                if portfolio_delta > 0:
                    for name, K in all_strikes.items():
                        pos_v = pos.get(name, 0)
                        if pos_v <= 0:
                            continue
                        v_od = state.order_depths.get(name)
                        if v_od is None or not v_od.buy_orders:
                            continue
                        v_bb = max(v_od.buy_orders.keys())
                        v_bv = abs(int(v_od.buy_orders[v_bb]))
                        trim = min(pos_v, v_bv, 6)
                        if trim > 0:
                            result.setdefault(name, []).append(
                                Order(name, int(v_bb), -trim)
                            )
                    ...
                    for name, K in all_strikes.items():
                        ...
                        if pos_v >= 0:
                            continue
                        v_od = state.order_depths.get(name)
                        if v_od is None or not v_od.sell_orders:
                            continue
                        v_ba = min(v_od.sell_orders.keys())
                        v_av = abs(int(v_od.sell_orders[v_ba]))
                        trim = min(-pos_v, v_av, 6)
                        if trim > 0:
                            result.setdefault(name, []).append(
                                Order(name, int(v_ba), trim)
                            )
