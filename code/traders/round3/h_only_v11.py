"""
HYDROGEL-only v11 — v8 + minimal at-cap defense (2026-04-24).

Single change vs v8: when |pos| >= 0.95 * limit AND drift_raw is still
adverse (long & touch_mid <= anchor - 35, or short & touch_mid >=
anchor + 35), force-flatten H_CAP_FLATTEN_QTY=30 by hitting the
opposite side. This defends the day-2 / hidden-day trap where the
position pins at +200 while mid keeps falling for 20k+ ticks. No drift
regime, no skew change, no make-suppression — just a "pull the
ripcord at-limit when bleeding" defense.
...
class Trader:
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist):
        ...
        # ---- v11: cap-flattener defense ----
        drift_raw = (book["touch_mid"]) - self.H_ANCHOR
