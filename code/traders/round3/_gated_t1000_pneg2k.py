...
class Trader:
    ...
    # PnL-gated stop: if a directional position has been held for VFE_GATE_TICKS
    # calls and its mark-to-mid PnL is at or below VFE_GATE_PNL_THRESHOLD,
    # force-flat + reset Citadel state. Winners are never reset.
    VFE_GATE_TICKS = 1000
    VFE_GATE_PNL_THRESHOLD = -2000.0
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        # Maintain entry anchor: clear when flat, set when entering directional
        if cur_pos == 0:
            entry_mid = None
        ...
        # PnL-gated reset: only fires on a directional position that has bled
        gated_reset_now = False
        if (
            entry_mid is not None
            and (total_ticks - entry_tick) >= self.VFE_GATE_TICKS
        ):
            pnl = cur_pos * (mid - entry_mid)
            if pnl <= self.VFE_GATE_PNL_THRESHOLD:
