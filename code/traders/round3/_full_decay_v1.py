...
class Trader:
    ...
    VFE_DECAY_RAMP = 3000
    # ----- HYDROGEL anchor-stuck decay -----
    # Reset counter every tick |mid - anchor| <= H_ANCHOR_TOUCH_BAND.
    # Otherwise grow. After H_DECAY_GRACE consecutive away ticks, ramp the
    # max-allowed |position| from limit -> 0 over H_DECAY_RAMP ticks.
    # Healthy days revisit the anchor band frequently so decay never engages.
    H_ANCHOR_TOUCH_BAND = 8
    H_DECAY_GRACE = 2000
    H_DECAY_RAMP = 3000
    H_DECAY_TRIM_PER_TICK = 6
    # ----- Voucher decay -----
    # Gated on Citadel VFE_DECAY_GRACE being engaged (since vouchers track VFE).
    # When VFE is stuck, ramp each voucher's max-allowed |position| down from
    # limit -> 0 over the same ramp window.
    V_DECAY_TRIM_PER_TICK = 4
    V_DECAY_STRIKES = (
        "VEV_4000", "VEV_4500",
    ...
    def run(self, state: TradingState):
        ...
        # 6. HYDROGEL anchor-stuck decay
        h_tsa = int(saved.get("h_ticks_since_anchor", 0))
        if new_mid is not None:
            if abs(new_mid - self.H_ANCHOR) <= self.H_ANCHOR_TOUCH_BAND:
                h_tsa = 0
            else:
                h_tsa += 1
        saved["h_ticks_since_anchor"] = h_tsa
        if h_tsa > self.H_DECAY_GRACE:
            over = h_tsa - self.H_DECAY_GRACE
            h_decay = max(0.0, 1.0 - over / self.H_DECAY_RAMP)
            cur_h = pos.get("HYDROGEL_PACK", 0)
            target_max = int(self.LIMITS["HYDROGEL_PACK"] * h_decay)
            if abs(cur_h) > target_max:
                excess = abs(cur_h) - target_max
                qty = min(excess, self.H_DECAY_TRIM_PER_TICK)
                h_od = state.order_depths.get("HYDROGEL_PACK")
                if h_od is not None and qty > 0:
                    if cur_h > 0 and h_od.buy_orders:
                        h_bb = max(h_od.buy_orders.keys())
                        result.setdefault("HYDROGEL_PACK", []).append(
                            Order("HYDROGEL_PACK", int(h_bb), -qty)
                        )
                    elif cur_h < 0 and h_od.sell_orders:
                        h_ba = min(h_od.sell_orders.keys())
                        result.setdefault("HYDROGEL_PACK", []).append(
                            Order("HYDROGEL_PACK", int(h_ba), qty)
                        )

        # 7. Voucher decay (gated on Citadel VFE stuck)
        vfe_tsz = int(saved.get("vfe_ticks_since_z0", 0))
        if vfe_tsz > self.VFE_DECAY_GRACE:
            over = vfe_tsz - self.VFE_DECAY_GRACE
            v_decay = max(0.0, 1.0 - over / self.VFE_DECAY_RAMP)
            for name in self.V_DECAY_STRIKES:
                cur = pos.get(name, 0)
                if cur == 0:
                    continue
                v_limit = self.LIMITS.get(name, 200)
                v_target = int(v_limit * v_decay)
                if abs(cur) > v_target:
                    excess = abs(cur) - v_target
                    qty = min(excess, self.V_DECAY_TRIM_PER_TICK)
                    v_od = state.order_depths.get(name)
                    if v_od is None or qty <= 0:
                        continue
                    if cur > 0 and v_od.buy_orders:
                        v_bb = max(v_od.buy_orders.keys())
                        result.setdefault(name, []).append(
                            Order(name, int(v_bb), -qty)
                        )
                    elif cur < 0 and v_od.sell_orders:
                        v_ba = min(v_od.sell_orders.keys())
                        result.setdefault(name, []).append(
                            Order(name, int(v_ba), qty)
                        )
