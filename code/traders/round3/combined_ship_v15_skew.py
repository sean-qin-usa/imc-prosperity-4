...
class Trader:
    ...
    # ----- VFE skew-residual signal (s8 deep research find) -----
    # residual = (V5500_mid - V5000_mid) - (SKEW_A + SKEW_B * VFE_mid)
    # Predicts ΔVFE_{t+1} with partial corr -0.20 (after VFE Δm AR1 control).
    # Used here as: combined_dev = ema_o_dev + SKEW_WEIGHT * residual,
    # AND as a drift-carry skip gate (skip bid post when residual >> 0).
    # Cliffs not yet mapped — sweep SKEW_WEIGHT.
    SKEW_A = 4170.0
    SKEW_B = -0.85
    SKEW_WEIGHT = 0.5         # tune; 0 = baseline, +1 = full
    SKEW_CARRY_GATE = 6.0     # skip drift-carry bid when residual > this
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # s8 deep-research add: combine ema_o_dev with skew-residual signal.
        # residual = (V5500_mid - V5000_mid) - (SKEW_A + SKEW_B * u_wm)
        # partial corr with ΔVFE_{t+1} = -0.20 across all 3 days.
        skew_res = 0.0
        v5000_od = ods.get("VEV_5000")
        v5500_od = ods.get("VEV_5500")
        if v5000_od is not None and v5500_od is not None:
            v5000_book = self._book(v5000_od)
            v5500_book = self._book(v5500_od)
            if v5000_book is not None and v5500_book is not None:
                skew_t = v5500_book["touch_mid"] - v5000_book["touch_mid"]
                skew_res = skew_t - (self.SKEW_A + self.SKEW_B * u_wm)

        combined_dev = ema_o_dev + self.SKEW_WEIGHT * skew_res
        ...
        if self.ENABLE_UNDERLYING_MR:
            ...
            if combined_dev > self.UNDER_MR_THR and u_max_sell > 0:
                u_orders.append(Order("VELVETFRUIT_EXTRACT", int(u_bw + 1), -u_max_sell))
            elif combined_dev < -self.UNDER_MR_THR and u_max_buy > 0:
                u_orders.append(Order("VELVETFRUIT_EXTRACT", int(u_aw - 1), u_max_buy))
            else:
                # No MR signal — pursue passive long-bias for drift carry,
                # but skip the bid when skew-residual predicts a drop.
                if skew_res <= self.SKEW_CARRY_GATE:
