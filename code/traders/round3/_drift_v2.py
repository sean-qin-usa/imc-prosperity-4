...
class Trader:
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        # Mid history for velocity (slope over last VFE_VEL_WINDOW ticks)
        mid_hist.append(mid)
        ...
        if ticks >= self.VFE_WARMUP_TICKS:
            z = dev / sigma
            # z >= +Z_ENTER means mid >> ema -> SHORT (side=-1).
            # Only take SHORT if mid isn't still rising fast.
            if z >= self.VFE_Z_ENTER and velocity <= self.VFE_VEL_BLOCK:
                side = -1
            # z <= -Z_ENTER means mid << ema -> LONG (side=+1).
            # Only take LONG if mid isn't still falling fast.
