...
class Trader:
    ...
    # ----- Chunked MR for voucher strikes (v18 cliff-fix) -----
    # Per signal histogram: |combined| p99 = 6.5 across all strikes/days.
    # We want signal=5→fire small, signal=7→fire big. Linear scaling.
    MR_MIN_CHUNK = 30
    MR_MAX_CHUNK = 200
    MR_SIZE_SCALE = 150  # 1 unit of (|sig|-thr) → 80 shares
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # ---- MR: near-ATM strikes 5000/5100/5200/5300, CHUNKED size.
        # Per-tick fire size scales linearly with signal-magnitude past
        # threshold:  size = clamp((|combined| - thr) * MR_SIZE_SCALE,
        # min=MR_MIN_CHUNK, max=MR_MAX_CHUNK).  This is the v8 cliff-fix:
        # at THR=4 the all-or-nothing slam pinned inventory and got
        # caught wrong-way next tick.
        for name, K in MR_STRIKES.items():
            ...
            max_buy = limit - p
            orders: List[Order] = out.setdefault(name, [])
            mag = abs(combined) - OPT_MR_THR
            if mag <= 0:
                continue
            chunk = int(round(min(self.MR_MAX_CHUNK,
                                  max(self.MR_MIN_CHUNK,
                                      mag * self.MR_SIZE_SCALE))))
            if combined > 0 and max_sell > 0:
                q = min(chunk, max_sell)
                if q > 0:
                    orders.append(Order(name, int(bw), -q))
            elif combined < 0 and max_buy > 0:
                q = min(chunk, max_buy)
                if q > 0:
                    orders.append(Order(name, int(aw), q))
