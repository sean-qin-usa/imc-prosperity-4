...
class Trader:
    ...
    VFE_RESET_INTERVAL = 500  # full state reset every N calls (0 to disable)
    VFE_RESET_KEEP_SIGMA = False
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        # Periodic full reset: clear EMA + sigma + side so position closes
        # and Citadel re-derives anchor + dispersion from local regime.
        if self.VFE_RESET_INTERVAL > 0 and ticks >= self.VFE_RESET_INTERVAL:
            ema = None
