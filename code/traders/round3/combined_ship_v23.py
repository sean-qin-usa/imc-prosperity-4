"""
Round 3 combined SHIP v23 — 2026-04-25 (session 8 final).

DELTA vs v22 (447,042 bt) — adds asymmetric pos-conditional H_INV_SKEW
on HYDROGEL (port from h_only_v22):

  H_INV_SKEW_LONG  = -0.004   (was 0.014; mild fair-shift UP when long)
  H_INV_SKEW_SHORT = +0.014   (unchanged from baseline)

Joint LONG/SHORT sweep peak at LONG=-0.004 SHORT=+0.014.  +4,289 bt.
The "no-skew when long" with "normal skew when short" shape captures
the directional asymmetry in HYDROGEL's drift behavior.

3-day backtest match-trades all:  +451,331 (sweep peak)
3-day backtest match-trades none: TBD (alpha floor — likely up by ~1k)

Cumulative session-8 deltas (over v15 = 443,484):
  v15_hdrift   (drift-regime CLIP gate):  +1,964 → 445,448
  v22 (+VEV carry, target=300, multi):    +1,594 → 447,042
  v23 (+asym INV_SKEW LONG=-0.004):       +4,289 → 451,331
  Total session-8 gain over v15: **+7,847 bt**.

Each delta independently passes alpha-floor or matching-engine
mechanism checks.  All preserve the v11 chassis (VS_INV_SKEW=0,
voucher MR retune, all v11/v12/v15 base settings).

What was tested in this push and rejected:
  - HYDROGEL queue-join layer at bb (size 5-30): -12 to -15k bt.
  - EOD VFE flat (last 100/200/500 ticks): -1 to -5k.
  - VEV CARRY_TARGET sweep: 300 (=cap) is peak; 50→+943 to 300→+1,594.
  - Subset MR strikes (5000-5100 only, 5000-5200 only): partial gains.
  - Persist-during-MR carry: -47k.
  - Multi-level VEV carry: bt-invariant (same as single).
  - h_only_v22 full chassis port (TE=0.3 PE=4 AR1=0.20): -17k regress.
  - Asym INV_SKEW LONG ≤ -0.018: cliff (-60k+); only smooth peak is
    near -0.004.

Validator-clean (no forbidden imports).

Live forecast at HYDROGEL's 2.27x conversion + VFE/VEV carries' ~1.0x:
v15 live 417,605 → v23 ~424k (~+6.5k from drift-gate + asym-skew at
2.27x + +2k from VEV carry + +1k from hdrift alpha floor).
"""
