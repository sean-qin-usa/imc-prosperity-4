"""
Round 3 combined SHIP v12_carry_b — 2026-04-25 (VFE drift carry, variant B).

** DO NOT SHIP **  -- this is a *negative* result kept for reference.
The default UNDER_TARGET_LONG=0 makes this file functionally identical
to combined_ship_v11.py (the actual ship).

Hypothesis (session-4 handoff): VFE has +0.002/ts drift = ~+20 ticks/day,
so leaning target-long should capture ~+600/day per +30 base on top of MR.

Variant A (combined_ship_v12_carry.py) — passive penny-inside layer; the
layer adversely fills and lost 5-8k vs baseline regardless of target.

Variant B (this file) — asymmetric MR cap: cap MR-sell at
(-limit + UNDER_TARGET_LONG) instead of -limit; MR-buy still uses full
+limit. The natural sell→buy MR cycle leaves position averaging biased
long with no extra orders or spread cost.

Sweep over UNDER_TARGET_LONG (3-day backtest, vs v11 baseline 428,754):
  target     total       Δ     day2_vfe
       0    428,754       +0     12,614  (= baseline)
      15    428,649     -105     12,224
      30    428,565     -189     11,845
      50    428,260     -494     11,382
      80    427,825     -929     10,790
     120    427,769     -985     10,218
     170    425,685   -3,069      9,233

Slope ≈ −6 PnL per unit target at the low end; cliff above 120.

Why carry doesn't pay: VFE's MR uses 100% of position capacity each fire
and earns ~5-6 ticks of cross-spread alpha per unit. Reserving N units
for carry steals N × 5-6 of MR alpha (immediate) and gains only N × 14.2 / 3
over 3 days (avg drift across +20.5 / +28 / -6 days). Capacity is fungible;
MR's edge per unit > drift's. Day 2 (+28 drift) has a -52 tick dip at the
75% mark, so a long carry takes a big drawdown before the rally and the
forced MR sell at the bottom locks in losses.

Detailed writeup: `VFE_DRIFT_CARRY_2026_04_25.md` in this directory.

All other sleeves identical to v11.
...
class Trader:
    ...
    # ----- Underlying drift carry (asymmetric MR cap) — v12 add -----
    # 2026-04-25 sweep showed slope ≈ −6 PnL/unit; the carry steals MR
    # alpha at a higher rate per unit than drift carries it. Default 0
    # = identical to v11 baseline. See header for sweep + reasoning.
    UNDER_TARGET_LONG = 0
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # v12 carry: cap MR-sell at (-limit + TARGET_LONG) to reserve
        # TARGET worth of base-long capacity for drift carry.
        if self.ENABLE_UNDERLYING_MR:
            u_pos = pos.get("VELVETFRUIT_EXTRACT", 0)
            u_limit = self.LIMITS["VELVETFRUIT_EXTRACT"]
            target = self.UNDER_TARGET_LONG
            u_max_sell = max(0, u_pos - (-u_limit + target))  # = u_pos + u_limit - target
