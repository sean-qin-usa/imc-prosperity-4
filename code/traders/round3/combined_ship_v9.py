"""
Round 3 combined SHIP v9 — 2026-04-25 (session 4, post-393333 push).

3-day backtest match-trades all:  +399,592 (116,799 / 135,056 / 147,737).
3-day backtest match-trades none: +252,388 (alpha floor, no passive fills).
vs ship_v1 (268,008 / 124,854):  +131,584 all / +127,534 alpha (+49% / +102%).
vs 393333 submission (213,927):  +185,665 (+87%).
vs h_only_v8 standalone (171,890): +227,702.

Per-day stability:
  Day 0: 116,799  (ship_v1: 72,648  → +44,151)
  Day 1: 135,056  (ship_v1: 102,805 → +32,251)
  Day 2: 147,737  (ship_v1: 92,555  → +55,182, hidden submission day)
  μ = 133,197, σ = 12,665, CV = 9.5% (ship_v1 CV 14.1%).

Stack of independent improvements vs ship_v1 (each verified in isolation):

  HYDROGEL (h_only_v16 retune):                +12,343 standalone
    H_ANCHOR        9985  → 9983
    H_INV_SKEW      0.015 → 0.014
    CLIP_VOL_K      0.30  → 0.76    (biggest HYDROGEL lever)
  ...
  Synthetic-voucher MM (V4000/V4500):          +24,941 to total
    VS_INV_SKEW     0.005 → 0.0     (uncorrelated, large free win)

  Voucher IV-residual MR sleeve:               see joint
    THEO_NORM_WINDOW   20 → 25      (smoother per-strike residual)
    iv_dev weight     1.0 → 1.75    (per-strike alpha 1.75x ema_o)
    OPT_MR_WINDOW     30 → 44       (slower underlying EMA)
    OPT_MR_THR         5 → 6.3      (only large deviations, less noise)

  VELVETFRUIT underlying MR:
    UNDER_MR_THR       5 → 6        (matches new EMA window scale)

What was DEPRIORITIZED (verified bad, do not redo):
- Layered Timo IV-scalp on 5000/5100 alongside MR: -46k catastrophic
  (the calm-regime flatten fights the MR position).
- Add VEV_5400 to MR strike set: -8k at v9 base (vega < 1, residual
  noise-dominated even with new params).
- Drop VEV_5300: -3.4k from base. Small but real contributor.
- Drop VFE underlying MR: -38.2k from base. Keeps it in.
- VS_PENNY_EDGE sweep [0,5]: completely insensitive (synth book has
  21-tick spread; we're never crossed at these post levels).
- VEV_6000/6500 lottery posts: mid stuck at 0.5, max EV ~50 SS.
- 3-way basis arb (V4000+4000 ↔ V4500+4500 ↔ S): basis std 0.83
  but round-trip ≥4 ticks > available edge.

Critical sweep cliffs (do NOT casually retune):
- OPT_MR_THR=6.3 peak. 5.5 → 277k, 6.0 → 373k, 6.3 → 398k, 6.5 → 399k,
  6.75 → 375k. Plateau is wide-ish at 6.3-6.5; below 6.0 craters.
- OPT_MR_WINDOW=44 peak (with THR=6.3). 38 → 342k, 44 → 398k, 48 → 384k.
  Cliff on either side; correlated with THR (slower window needs more
  selective threshold).
- UNDER_MR_THR=6 peak. 5 → 371k, 6 → 398k, 7 → 377k.
- iv_dev weight 1.75 peak. 1.5 → 390k, 1.75 → 398k, 2.0 → 396k.
  Wide plateau [1.7, 2.0].
- VS_INV_SKEW=0 peak. 0 → 311k, 0.005 (was ship_v1) → 286k → -25k
  perfectly clean alpha lost to skew. Single biggest free lunch.

Underlying = VELVETFRUIT_EXTRACT (per-tick wall_mid for IV machinery).
Sleeves are independent — IV-residual machinery is upload-safe (no os,
no subprocess, only math + statistics + json + datamodel).
"""
