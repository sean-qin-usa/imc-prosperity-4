...
class Trader:
    ...
    VFE_Z_ENTER = 2.0
    VFE_MAX_TAKE_PER_TICK = 8
    # Adaptive EMA: when |z| stays elevated (regime drift), boost alpha so the
    # anchor catches up. mean_abs_z is an EMA of |z| with HL ~ 200; boost
    # factor = 1 + max(0, mean_abs_z - 1.5) * VFE_ADAPT_GAIN. At |z|=2.5
    # average, alpha is multiplied by (1+1*GAIN).
    VFE_ADAPT_MEAN_Z_ALPHA = 0.002
    VFE_ADAPT_GAIN = 4.0
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved):
        ...
        mean_abs_z = float(saved.get("vfe_mean_abs_z", 0.0))

        # Compute current dev/sigma using last-tick state for boost decision
        if ema is None:
            prev_dev = 0.0
        else:
            prev_dev = mid - ema
        prev_sigma = max(math.sqrt(sigma_var), self.VFE_SIGMA_FLOOR)
        prev_abs_z = abs(prev_dev / prev_sigma) if prev_sigma > 0 else 0.0
        ma_alpha = self.VFE_ADAPT_MEAN_Z_ALPHA
        mean_abs_z = (1.0 - ma_alpha) * mean_abs_z + ma_alpha * prev_abs_z
        boost = 1.0 + max(0.0, mean_abs_z - 1.5) * self.VFE_ADAPT_GAIN

        if ema is None:
            ema = mid
        else:
            a = min(0.5, self.VFE_EMA_ALPHA * boost)
            ema = (1.0 - a) * ema + a * mid
        dev = mid - ema
        ...
        saved["vfe_side"] = side
        saved["vfe_mean_abs_z"] = mean_abs_z
