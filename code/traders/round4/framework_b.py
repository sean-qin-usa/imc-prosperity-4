"""
Framework B — counterparty-adaptive MM via cross-product fair shift.

Builds on Framework A. Framework A established that direct take-side
trader-follow on VFE loses to spread cost (-127k) and that veto-mode
loses macro-MR profit (-46k). This framework instead uses VFE trader
signal as a CROSS-PRODUCT FAIR SHIFT propagated to the voucher MM
sleeves — Mark 67 buys VFE → all delta-positive vouchers get fair-up,
which lifts our bids and raises our asks ASYMMETRICALLY (defensive +
opportunistic).
...
class Trader:
    ...
    # ----- Framework B: cross-product fair-shift propagation -----
    # VFE trader_score predicts ticks of VFE move. Voucher fair shifts by
    # score * delta_K (deep-ITM delta ~ 1, ATM delta ~ 0.5). Asymmetric:
    # ask shifts more than bid (defensive against pickoff).
    ENABLE_FB_FAIR_SHIFT = True
    FB_SHIFT_K = 0.50               # AMPED 10x for sanity check
    FB_ASK_ASYM = 1.4
    FB_BID_ASYM = 0.6
    FB_MAX_SHIFT = 12.0             # AMPED 3x
