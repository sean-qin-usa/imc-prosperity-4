"""
Framework H — two trader-counterparty overlays on current_strategy.py
=====================================================================

RESULT (2026-04-26): DEAD END. Same class as framework_g.
Both overlays left in source as `ENABLE_*=False` so this file equals
current_strategy.py byte-for-byte when shipped. Do NOT re-run.

  baseline current_strategy:                      +440,853

  Voucher regime cap only (DECAY=0.92, THR=5):    +440,717  (-136)
  HYDROGEL M22 fade only  (per_qty=0.40):         +440,746  (-107)
  Both ON  (conservative)                         +440,610  (-243)
  Both ON  (per_qty=0.70, VS_THR=3.5)             +409,484  (-31,369)
    └─ Day 1 alone: 175,226 → 144,354 (-30,872)
  Both ON  (per_qty=1.50, VS_THR=2.0)             +408,116  (-32,737)

Pattern: at low magnitudes the signals are noise (within ±300 of
baseline = seed band). At meaningful magnitudes the M22 fade
catastrophically misfires on Day 1 — HYDROGEL down-trends with
Mark 22 sell clusters, and the fair-lift causes the strategy to bid
into the down-trend.

Why the hypothesis failed:
- Mark 22 HYDROGEL events are too rare (~6/day) for the fade signal
  to dominate noise; making it big enough to matter overshoots on
  the days when Mark 22 sells happen during real downtrends.
- Voucher regime cap blocks ~3% of MM activity → ~1k expected swing,
  drowned in seed noise.

Generalises framework_g + feedback_voucher_mr_live_conversion
+ feedback_match_trades_none_for_alpha: short-horizon trader-counterparty
signals don't convert to material bt PnL on top of the
v31 chassis at any magnitude.

Recommendation: ship current_strategy.py to live (440,853 bt, ~441k
forecast). Counterparty overlays are not the R4 lift; chase
AETHER_CRYSTAL manual or other unrelated alpha for material gain.

----- original hypothesis (kept for the record) -----

Two trader-counterparty overlays applied to DIFFERENT sleeves than
framework_g (which gated Citadel and lost):

1. ENABLE_VOUCHER_REGIME_CAP — when Mark 49 has been selling VFE
   recently (score < -THR = informed_short), the deep-ITM synth voucher
   MM (V4000/V4500, delta ~ 1) skips its buy-side posts and take-side
   buys. We don't WANT new long inventory while VFE is heading down.
   Symmetric on the long side: score > +THR skips ask-side posts and
   take-side sells. Existing inventory drains via the unblocked side.

2. ENABLE_HYDROGEL_M22_FADE — Mark 22 sells HYDROGEL_PACK very rarely
   (~6/day) but when he does, price moves UP +3.13 ticks within 1 tick
   (94.7% miss rate on Mark 22's sell side = strong fade signal).
   Inject a decaying upward fair-value shift (HL ~ 8 ticks) so the
   strategy lifts its bid and stops flat-selling into the bounce.

Why these are different from framework_g:
- VFE Citadel gate failed because the trader signal and the Citadel
  z-score share the same up/down deviation events.
- Voucher MM uses BS-theo as fair (not a long-EMA z), so the Mark 49
  signal adds an orthogonal direction-of-drift overlay.
- HYDROGEL fair is anchored at H_ANCHOR (stationary), not deviation-
  based, so a Mark 22 fade shift is also orthogonal to the existing
  fair construction.

Backtest baseline current_strategy = 440,853 (PYTHONHASHSEED=0
jmerle, IMCP2026/data/round4, 3 days).

Validator-clean: no forbidden imports, two new knob blocks + ~30 lines
in run() + _trade_synth_voucher + _trade_hydrogel.

----- current_strategy.py history below (unchanged chassis) -----
...
- `Trade.buyer` and `Trade.seller` now expose Mark counterparty IDs.

Local Round 4 counterparty scan:
- Mark 67 buying VELVETFRUIT_EXTRACT is the cleanest named-flow event
  study in the supplied capsule, but a direct follow overlay reduced
  backtest PnL. Counterparty intel is documented separately and is not
  active in this ship file.
...
class Trader:
    ...
    # ----- framework_h: VFE trader-regime score (used by VOUCHER cap) -----
    # Same signal as framework_g; here we apply it to V4000/V4500 MM,
    # not Citadel. HL = log(0.5)/log(0.92) ~ 8.3 ticks. Mark 67 avg qty
    # 9.15 → ~+18/event; Mark 49 avg qty 9.72 → ~-17.6/event.
    ENABLE_VOUCHER_REGIME_CAP = False  # dead end — see top docstring
    VS_REGIME_DECAY = 0.92
    VS_REGIME_THR = 5.0
    VFE_REGIME_WEIGHTS = {
        ("Mark 67", "buy"):  +1.970,
        ("Mark 49", "sell"): -1.811,
    ...
    # ----- framework_h: HYDROGEL Mark 22 fade fair-shift -----
    # Mark 22 sells HYDROGEL → +3.13 1-tick edge (8 events / day, 94.7%
    # miss rate confirms strong fade). HL ~ 8 ticks; per-qty shift 0.40
    # = avg event qty 3.89 → +1.6 fair shift / event, decays to ~0.8
    # in 8 ticks. Source: framework_a knobs (untested in isolation).
    ENABLE_HYDROGEL_M22_FADE = False  # dead end — see top docstring
    H_M22_DECAY = 0.92
    ...
    def _trade_hydrogel(self, od, pos, last_dmid, dmid_hist, mid_hist, m22_shift=0.0):
        ...
        fair += m22_shift  # framework_h: Mark 22 fade shift (no-op if disabled)
    ...
    def _trade_synth_voucher(self, name, K, od, pos, S, T, regime_score=0.0):
        ...
        # framework_h: voucher regime cap. delta~1 to VFE, so during
        # informed_short don't accumulate longs (skip take-side BUY +
        # bid post). Symmetric on the long side.
        block_long = (self.ENABLE_VOUCHER_REGIME_CAP
                      and regime_score < -self.VS_REGIME_THR)
        block_short = (self.ENABLE_VOUCHER_REGIME_CAP
                       and regime_score > self.VS_REGIME_THR)
        working = pos
        orders: List[Order] = []
        for ap, av in book["sells"].items():
        ...
        if block_long:
            bid_size = 0  # don't post bids during informed_short
        if block_short:
            ask_size = 0  # don't post asks during informed_long
    ...
    # ---------- framework_h: VFE trader-regime score helper ----------
    def _update_vfe_regime_score(self, state: TradingState, saved: Dict) -> float:
        score = float(saved.get("vfe_regime_score", 0.0)) * self.VS_REGIME_DECAY
        market_trades = getattr(state, "market_trades", None) or {}
        for tr in (market_trades.get("VELVETFRUIT_EXTRACT") or []):
            qty = abs(int(getattr(tr, "quantity", 0) or 0))
            if qty <= 0:
                continue
            buyer = getattr(tr, "buyer", None)
            if buyer:
                w = self.VFE_REGIME_WEIGHTS.get((buyer, "buy"))
                if w is not None:
                    score += w * qty
            seller = getattr(tr, "seller", None)
            if seller:
                w = self.VFE_REGIME_WEIGHTS.get((seller, "sell"))
                if w is not None:
                    score += w * qty
        saved["vfe_regime_score"] = score
        return score

    # ---------- framework_h: HYDROGEL Mark 22 fade fair-shift helper ----------
    def _hydrogel_m22_fair_shift(self, state: TradingState, saved: Dict) -> float:
        shift = float(saved.get("h_m22_shift", 0.0)) * self.H_M22_DECAY
        market_trades = getattr(state, "market_trades", None) or {}
        for tr in (market_trades.get("HYDROGEL_PACK") or []):
            if getattr(tr, "seller", None) == "Mark 22":
                qty = abs(int(getattr(tr, "quantity", 0) or 0))
                shift += self.H_M22_SHIFT_PER_QTY * qty
    ...
    def run(self, state: TradingState):
        ...
        # framework_h: refresh trader signals before any sleeve runs
        vfe_regime_score = (
            self._update_vfe_regime_score(state, saved)
            if self.ENABLE_VOUCHER_REGIME_CAP else 0.0
        )
        m22_shift = (
            self._hydrogel_m22_fair_shift(state, saved)
            if self.ENABLE_HYDROGEL_M22_FADE else 0.0
        )
