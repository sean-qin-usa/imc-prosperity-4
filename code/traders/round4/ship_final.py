...
class Trader:
    ...
    # ----- Counterparty additive layer: Mark 67 BUY / Mark 49 SELL on VFE -----
    # Per-symbol fwd-PnL (validated on R4 tape):
    #   Mark 67 BUY  VFE: fwd_2000_per_unit = +1.79  (predicts mid UP)
    #   Mark 49 SELL VFE: fwd_2000_per_unit = +1.85  (their direction is wrong)
    # Layer is PASSIVE-ONLY (no take). It posts a small bid 1-tick inside touch
    # when signal is positive. Sized small so it doesn't fight Citadel MR
    # short positions (only adds incremental long bias when 67/49 fire).
    # CP layer disabled: bt shows neutral or slight regression vs chassis Citadel MR.
    # Citadel z-score MR already captures the VFE directional alpha that
    # Mark 67/49 prints predict. Layering both is redundant and conflicts on
    # short entries. Kept as a knob in case live conversion differs from bt.
    CP_ENABLE = False
    CP_MARK67_COEF = 1.79
    CP_MARK49_COEF = 1.85
    CP_SIGNAL_HL = 1500.0
    CP_SIGNAL_CAP = 6.0
    CP_POST_THRESHOLD = 3.0
    CP_POST_SIZE = 6
    CP_TARGET_POS_BIAS = 40
    CP_BLOCK_IF_SHORT = True
    ...
    def _trade_cp_vfe_layer(self, state: TradingState, saved: Dict, vfe_pos: int) -> List[Order]:
        """Additive Mark 67/49 passive bid layer for VFE.

        Updates a per-symbol signal from observed market_trades. When signal
        is positive (Mark 67 buys / Mark 49 sells dominate the recent window)
        and chassis isn't already saturated long, post a small bid 1-tick
        inside the touch.
        """
        if not self.CP_ENABLE:
            return []
        ...
        sig = float(saved.get("cp_vfe_sig", 0.0))
        last_ts = int(saved.get("cp_vfe_ts", ts))
        if ts > last_ts:
            sig *= math.exp(-(ts - last_ts) * math.log(2.0) / self.CP_SIGNAL_HL)
        ...
        for t in (state.market_trades.get("VELVETFRUIT_EXTRACT") or []):
            ...
            if buyer == "Mark 67":
                sig += self.CP_MARK67_COEF * qty
            if seller == "Mark 49":
                sig += self.CP_MARK49_COEF * qty
            if seller == "Mark 67":
                sig -= self.CP_MARK67_COEF * qty
            if buyer == "Mark 49":
                sig -= self.CP_MARK49_COEF * qty

        cap = self.CP_SIGNAL_CAP
        if sig > cap:  sig = cap
        elif sig < -cap: sig = -cap
        saved["cp_vfe_sig"] = sig
        saved["cp_vfe_ts"]  = ts

        # Only ADD long bias when (a) signal positive, (b) chassis not already long-loaded,
        # and (c) chassis isn't currently short (would be fighting MR).
        if sig < self.CP_POST_THRESHOLD or vfe_pos >= self.CP_TARGET_POS_BIAS:
            return []
        if self.CP_BLOCK_IF_SHORT and vfe_pos < 0:
            return []
        bb = max(od.buy_orders.keys()); ba = min(od.sell_orders.keys())
        if ba <= bb + 1:
            return []
        post_px = bb + 1
        room = self.LIMITS["VELVETFRUIT_EXTRACT"] - vfe_pos
        qty = min(self.CP_POST_SIZE, room, self.CP_TARGET_POS_BIAS - vfe_pos)
        if qty <= 0:
            return []
        return [Order("VELVETFRUIT_EXTRACT", int(post_px), qty)]
    ...
    def run(self, state: TradingState):
        ...
        # 6. Counterparty additive layer: Mark 67/49 passive bid bias on VFE
        cp_orders = self._trade_cp_vfe_layer(state, saved, pos.get("VELVETFRUIT_EXTRACT", 0))
        if cp_orders:
            existing = result.get("VELVETFRUIT_EXTRACT", [])
            result["VELVETFRUIT_EXTRACT"] = existing + cp_orders
