...
class Trader:
    ...
    # Round 4 full-candidate overlays.
    #
    # Counterparty IDs are useful, but only after separating fill edge from
    # post-fill markout. Mark 67 buys and Mark 49 sells are bullish VFE
    # markouts; use them as short vetoes, not as a replacement for the VFE
    # model. Keep the window short so one sparse Mark print cannot dominate.
    ENABLE_MARK_VFE_DEFENSE = True
    MARK_VFE_COOLDOWN_TS = 500
    VFE_BULLISH_BUYERS = ("Mark 67",)
    VFE_BULLISH_SELLERS = ("Mark 49",)
    VFE_BEARISH_BUYERS = ("Mark 49",)

    # Portfolio delta brake. Public R4 has a large day-3 400k-500k correlated
    # loss while long VFE + long calls. Local PnL likes that carry, but hidden
    # robustness needs a cap and a fast-underlying downtrend trim.
    ENABLE_PORTFOLIO_DELTA_BRAKE = True
    PORT_DELTA_CAP = 2000.0
    PORT_DELTA_DOWNTREND_CAP = 900.0
    PORT_DELTA_HARD = 2200.0
    PORT_DELTA_DOWNTREND_HARD = 1100.0
    RISK_TREND_LOOKBACK = 500
    RISK_DOWNTREND_TRIGGER = -30.0
    RISK_UPTREND_TRIGGER = 30.0
    RISK_UNWIND_PER_PRODUCT = 24
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved, ts=0, market_trades=None):
        ...
        if self.ENABLE_MARK_VFE_DEFENSE and market_trades:
            for t in (market_trades.get("VELVETFRUIT_EXTRACT") or []):
                buyer = getattr(t, "buyer", "") or ""
                seller = getattr(t, "seller", "") or ""
                if buyer in self.VFE_BULLISH_BUYERS or seller in self.VFE_BULLISH_SELLERS:
                    saved["vfe_mark_bull_until"] = ts + self.MARK_VFE_COOLDOWN_TS
                if buyer in self.VFE_BEARISH_BUYERS:
                    saved["vfe_mark_bear_until"] = ts + self.MARK_VFE_COOLDOWN_TS
        ...
        trade_side = side
        if self.ENABLE_MARK_VFE_DEFENSE:
            bull_until = int(saved.get("vfe_mark_bull_until", -1))
            bear_until = int(saved.get("vfe_mark_bear_until", -1))
            if ts < bull_until and bull_until >= bear_until and trade_side < 0:
                trade_side = 0
            elif ts < bear_until and bear_until > bull_until and trade_side > 0:
                trade_side = 0
        ...
        target = trade_side * self.LIMITS["VELVETFRUIT_EXTRACT"]
        diff = target - cur_pos
        if diff == 0:
            return []
    ...
    def _risk_mid_and_trend(self, state: TradingState, saved):
        od = state.order_depths.get("VELVETFRUIT_EXTRACT")
        book = self._book(od) if od is not None else None
        if book is None:
            return None, 0.0

        mid = float(book["touch_mid"])
        hist = saved.get("risk_vfe_mids", [])
        if not isinstance(hist, list):
            hist = []
        hist.append(mid)
        max_len = self.RISK_TREND_LOOKBACK + 1
        if len(hist) > max_len:
            hist = hist[-max_len:]
        saved["risk_vfe_mids"] = hist

        if len(hist) > self.RISK_TREND_LOOKBACK:
            trend = mid - float(hist[-1 - self.RISK_TREND_LOOKBACK])
        elif len(hist) > 1:
            trend = mid - float(hist[0])
        else:
            trend = 0.0
        return mid, trend

    def _risk_delta_map(self, S: float, T: float) -> Dict[str, float]:
        deltas: Dict[str, float] = {"VELVETFRUIT_EXTRACT": 1.0}
        all_vev = {
            **self.SYNTH_STRIKES,
            **MR_STRIKES,
            **self.ATM_STRIKES,
            **self.LOTTERY_STRIKES,
        }
        for name, K in all_vev.items():
            _, delta, _ = self._opt_bs(S, K, T, self.SIGMA)
            deltas[name] = max(0.0, min(1.0, float(delta)))
        return deltas

    @staticmethod
    def _portfolio_delta(pos, deltas: Dict[str, float]) -> float:
        return sum(float(pos.get(name, 0)) * delta for name, delta in deltas.items())

    @staticmethod
    def _clamp_delta_qty(cur_delta: float, unit_delta: float, qty: int,
                         cap_pos: float, cap_neg: float) -> int:
        if qty == 0 or unit_delta <= 0:
            return qty
        delta_qty = unit_delta * qty
        next_delta = cur_delta + delta_qty
        if delta_qty > 0:
            if cur_delta >= cap_pos:
                return 0
            if next_delta > cap_pos:
                return max(0, int(math.floor((cap_pos - cur_delta) / unit_delta)))
        elif delta_qty < 0:
            if cur_delta <= -cap_neg:
                return 0
            if next_delta < -cap_neg:
                return min(0, int(math.ceil((-cap_neg - cur_delta) / unit_delta)))
        return qty

    def _append_delta_unwind(self, result: Dict[str, List[Order]], state: TradingState,
                             pos, deltas: Dict[str, float], cur_delta: float,
                             cap_pos: float, cap_neg: float,
                             hard_pos: float, hard_neg: float) -> None:
        if cur_delta > hard_pos:
            need = cur_delta - cap_pos
            sign = 1
        elif cur_delta < -hard_neg:
            need = -hard_neg - cur_delta
            sign = -1
        else:
            return

        names = [
            "VELVETFRUIT_EXTRACT", "VEV_4000", "VEV_4500", "VEV_5000",
            "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500",
        ]
        names.sort(key=lambda name: deltas.get(name, 0.0), reverse=True)
        for name in names:
            if need <= 0:
                break
            unit_delta = deltas.get(name, 0.0)
            if unit_delta <= 0:
                continue
            ...
            if sign > 0:
                ...
                room = self.LIMITS.get(name, 0) + p - existing_sell
                qty = min(p, room, self.RISK_UNWIND_PER_PRODUCT,
                          int(math.ceil(need / unit_delta)))
                if qty <= 0:
                    continue
                result.setdefault(name, []).append(Order(name, int(price), -qty))
                need -= qty * unit_delta
            else:
                price = min(od.sell_orders.keys())
                existing_buy = sum(int(o.quantity) for o in existing if int(o.quantity) > 0)
                room = self.LIMITS.get(name, 0) - p - existing_buy
                qty = min(-p, room, self.RISK_UNWIND_PER_PRODUCT,
                          int(math.ceil(need / unit_delta)))
                if qty <= 0:
                    continue
    ...
    def _apply_portfolio_delta_brake(self, result: Dict[str, List[Order]],
                                     state: TradingState, pos, saved):
        S, trend = self._risk_mid_and_trend(state, saved)
        if S is None:
            return result
        T = self._tte_years(state.timestamp)
        deltas = self._risk_delta_map(S, T)
        cur_delta = self._portfolio_delta(pos, deltas)

        cap_pos = self.PORT_DELTA_CAP
        cap_neg = self.PORT_DELTA_CAP
        hard_pos = self.PORT_DELTA_HARD
        hard_neg = self.PORT_DELTA_HARD
        if trend <= self.RISK_DOWNTREND_TRIGGER:
            cap_pos = self.PORT_DELTA_DOWNTREND_CAP
            hard_pos = self.PORT_DELTA_DOWNTREND_HARD
        elif trend >= self.RISK_UPTREND_TRIGGER:
            cap_neg = self.PORT_DELTA_DOWNTREND_CAP
            hard_neg = self.PORT_DELTA_DOWNTREND_HARD
        ...
        for name, orders in result.items():
            ...
            for order in orders:
                ...
                if unit_delta > 0:
                    qty = self._clamp_delta_qty(
                        cur_delta, unit_delta, qty, cap_pos, cap_neg,
                    )
                if qty == 0:
                    continue
                kept.append(order if qty == int(order.quantity) else Order(name, int(order.price), qty))
                cur_delta += unit_delta * qty
            if kept:
                filtered[name] = kept

        self._append_delta_unwind(
            filtered, state, pos, deltas, cur_delta,
            cap_pos, cap_neg, hard_pos, hard_neg,
        )
        return filtered
    ...
    def run(self, state: TradingState):
        ...
        if self.ENABLE_PORTFOLIO_DELTA_BRAKE:
            result = self._apply_portfolio_delta_brake(result, state, pos, saved)
