"""
Framework C — directional voucher take on strong Mark 67 fires + explicit exit.

Hypothesis: when VFE trader_score crosses TAKE_THR (cumulative Mark 67 burst),
the upcoming +1.97 ticks/event of VFE move propagates to deep-ITM vouchers
(VEV_4000, delta~1) and we capture it via a take + timed-exit cycle.
Deep-ITM has narrower spread than ATM, so spread cost is < expected move
when several events stack within the decay window.

----- Original Framework B doc (legacy) -----

Scope:
- VEV_4000/4500 synth MM: fair_shift = VFE_score * voucher_delta * K
- VEV_5400/5500 ATM smile MM: fair_shift = VFE_score * voucher_delta * K
- Asymmetry: ask shifts +alpha*shift, bid shifts +beta*shift with alpha>beta
  so we don't get picked off when score is positive but still capture
  some bid-side fills.
...
class Trader:
    ...
    # Disabled in Framework B (replaced by fair-shift below).
    ENABLE_VOUCHER_PREPOS = False
    VOUCHER_PREPOS_THR = 5.0
    VOUCHER_PREPOS_BOOST = 1.6

    # ----- Framework B (legacy, OFF in C) -----
    ENABLE_FB_FAIR_SHIFT = False
    FB_SHIFT_K = 0.50
    FB_ASK_ASYM = 1.4
    FB_BID_ASYM = 0.6
    FB_MAX_SHIFT = 12.0

    # ----- Framework C: directional voucher take on strong Mark 67 fires -----
    # When |vfe_score| crosses FC_TAKE_THR, take VEV_4000 (deep-ITM, delta~1)
    # at the ask (long) or bid (short). Hold position; exit at touch_mid
    # after FC_HOLD_TICKS, OR exit early when |score| decays below FC_EXIT_THR.
    ENABLE_FC_VOUCHER_DIRECTIONAL = True
    FC_TARGET_SYMBOL = "VEV_4000"
    FC_TAKE_THR = 25.0          # cumulative score needed to fire
    FC_EXIT_THR = 8.0           # exit when |score| drops below this
    FC_HOLD_TICKS = 30          # max hold ticks
    FC_TAKE_QTY = 30            # qty per take (limit room permitting)
    FC_MAX_POS = 90             # cap directional inventory in this sleeve
    ...
    def _trade_synth_voucher(self, name, K, od, pos, S, T, trader_score=0.0,
        ...
        if spread >= self.VS_WIDE_SPREAD:
            # Framework B: cross-product fair shift (asymmetric).
            bid_price = min(bb + 1, math.floor(skew - self.VS_PENNY_EDGE + bid_shift))
            ask_price = max(ba - 1, math.ceil(skew + self.VS_PENNY_EDGE + ask_shift))
            bid_price = min(int(bid_price), ba - 1, math.floor(fair + bid_shift) - 1)
            ask_price = max(int(ask_price), bb + 1, math.ceil(fair + ask_shift) + 1)
            if bid_price < ask_price:
                if bid_size > 0:
    ...
    def _trade_atm(self, name, K, od, pos, S, T, residual, bid_shift=0.0, ask_shift=0.0):
        limit = self.LIMITS[name]
        ...
        if spread >= self.ATM_WIDE_SPREAD:
            ...
            # Framework B: cross-product fair shift (asymmetric).
            bid_price = min(bb, math.floor(skew - self.ATM_PENNY_EDGE + bid_shift))
            ask_price = max(ba, math.ceil(skew + self.ATM_PENNY_EDGE + ask_shift))
            bid_price = min(int(bid_price), ba - 1, math.floor(fair + bid_shift) - 1)
            ask_price = max(int(ask_price), bb + 1, math.ceil(fair + ask_shift) + 1)
            if bid_price < ask_price:
                if bid_size > 0:
    ...
    # ---------- Framework C: directional voucher take ----------
    def _trade_fc_voucher_directional(self, od, cur_pos, vfe_score, ts, saved):
        """Take VEV_4000 directionally on strong Mark 67 / Mark 49 fires.

        Entry: |vfe_score| >= FC_TAKE_THR and current sleeve_pos hasn't
        already maxed in that direction.
        Exit: hold sleeve_pos for up to FC_HOLD_TICKS, OR exit when
        |score| < FC_EXIT_THR.
        """
        if od is None or not od.buy_orders or not od.sell_orders:
            return []
        ...
        sleeve_pos = int(saved.get("fc_sleeve_pos", 0))
        entry_ts = saved.get("fc_entry_ts")
        orders: List[Order] = []

        # Exit logic first (overrides entry).
        should_exit = False
        if sleeve_pos != 0:
            held_ticks = (ts - entry_ts) / 100 if entry_ts is not None else 0
            if held_ticks >= self.FC_HOLD_TICKS:
                should_exit = True
            elif abs(vfe_score) < self.FC_EXIT_THR:
                should_exit = True
            elif sleeve_pos > 0 and vfe_score < -self.FC_EXIT_THR:
                should_exit = True
            elif sleeve_pos < 0 and vfe_score > self.FC_EXIT_THR:
                should_exit = True
        ...
        if should_exit:
            if sleeve_pos > 0:
                # sell at bb to close
                qty = min(sleeve_pos, abs(int(od.buy_orders[bb])))
                if qty > 0:
                    orders.append(Order(self.FC_TARGET_SYMBOL, int(bb), -qty))
                    sleeve_pos -= qty
            elif sleeve_pos < 0:
                qty = min(-sleeve_pos, abs(int(od.sell_orders[ba])))
                if qty > 0:
                    orders.append(Order(self.FC_TARGET_SYMBOL, int(ba), qty))
                    sleeve_pos += qty
            saved["fc_sleeve_pos"] = sleeve_pos
            if sleeve_pos == 0:
                saved["fc_entry_ts"] = None
            return orders

        # Entry logic.
        if vfe_score >= self.FC_TAKE_THR and sleeve_pos < self.FC_MAX_POS:
            room = self.FC_MAX_POS - sleeve_pos
            qty = min(self.FC_TAKE_QTY, room, abs(int(od.sell_orders[ba])))
            limit_room = self.LIMITS[self.FC_TARGET_SYMBOL] - cur_pos
            qty = min(qty, max(0, limit_room))
            if qty > 0:
                orders.append(Order(self.FC_TARGET_SYMBOL, int(ba), qty))
                if sleeve_pos == 0:
                    saved["fc_entry_ts"] = ts
                sleeve_pos += qty
        elif vfe_score <= -self.FC_TAKE_THR and sleeve_pos > -self.FC_MAX_POS:
            room = self.FC_MAX_POS + sleeve_pos
            qty = min(self.FC_TAKE_QTY, room, abs(int(od.buy_orders[bb])))
            limit_room = self.LIMITS[self.FC_TARGET_SYMBOL] + cur_pos
            qty = min(qty, max(0, limit_room))
            if qty > 0:
                orders.append(Order(self.FC_TARGET_SYMBOL, int(bb), -qty))
        ...
        if u_od is not None:
            ...
            if u_book is not None:
                ...
                for name, K in self.SYNTH_STRIKES.items():
                    ...
                    if od is not None:
                        # Framework B: deep-ITM delta ~ 1; full propagation.
                        if self.ENABLE_FB_FAIR_SHIFT and abs(vfe_score) > 0:
                            raw = self.FB_SHIFT_K * vfe_score   # already signed
                            raw = max(-self.FB_MAX_SHIFT, min(self.FB_MAX_SHIFT, raw))
                            bs = raw * self.FB_BID_ASYM
                            asks = raw * self.FB_ASK_ASYM
                        else:
                            bs = asks = 0.0
                        result[name] = self._trade_synth_voucher(
                            name, K, od, pos.get(name, 0), S_synth, T_synth,
                            trader_score=vfe_score, bid_shift=bs, ask_shift=asks,
                        )
        ...
        if u_od is not None:
            ...
            if u_book is not None:
                ...
                for name, K in self.ATM_STRIKES.items():
                    od = state.order_depths.get(name)
                    if od is not None:
                        # Framework B: ATM delta ~ 0.5 (rough proxy); half propagation.
                        if self.ENABLE_FB_FAIR_SHIFT and abs(vfe_score) > 0:
                            raw_atm = 0.5 * self.FB_SHIFT_K * vfe_score
                            raw_atm = max(-self.FB_MAX_SHIFT, min(self.FB_MAX_SHIFT, raw_atm))
                            bs_atm = raw_atm * self.FB_BID_ASYM
                            asks_atm = raw_atm * self.FB_ASK_ASYM
                        else:
                            bs_atm = asks_atm = 0.0
                        orders, new_r = self._trade_atm(
                            name, K, od, pos.get(name, 0), S_atm, T_atm,
                            atm_residuals.get(name),
                            bid_shift=bs_atm, ask_shift=asks_atm,
                        )
