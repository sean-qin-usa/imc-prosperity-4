"""
Round 4 SHIP v1 — re-tune of current_strategy on full R4 day-1+2+3 set.

Verified facts (2026-04-27):
- R4 day_1 prices == R3 day_1 prices (md5 identical).
- R4 day_2 prices == R3 day_2 prices (md5 identical).
- R4 day_3 = NEW. This is the day that was hidden during R3 evaluation.
- R4 trades expose Mark counterparty IDs; R3 trades did not.
- Same product set and limits as R3.

Implication: v31/v30 sweeps were fit on {R3 d0, d1, d2}. With R4 day_3
now visible, two knobs re-optimize meaningfully:

  VFE_WARMUP_TICKS   65   -> 90    (R4 day-3 needs more sigma history
                                    before Citadel takes a side)
  SIGMA              0.23 -> 0.252  (R3-fit BS vol under-stated R4
                                    day-3 realized vol; ATM voucher
                                    fair was too tight)

Local R4 backtest (PYTHONHASHSEED=0, jmerle, --no-out):
  baseline current_strategy (W=65, SIGMA=0.23):  440,853
  W=90 alone:                                    442,269  (+1,416)
  SIGMA=0.252 alone:                             ~447k    (+~6.9k)
  W=90 + SIGMA=0.252 (this ship):                452,050  (+11,197)

R3 day-0 cross-check (NOT in R4 eval set, sanity only):
  baseline 147,462  ->  this ship ~151k (+~4k) — also positive,
  so this is not overfit to R4 day-3 noise.

Day-3 worst correlated bucket (ts 400-499k) is essentially unchanged
(~-76k). Per memory, six chassis-level patches for this bucket all
regressed; we leave it. The +11k gain comes from the rest of the day.

Counterparty layer (Mark 01-67) intentionally NOT enabled. Per memory:
all CP overlays tested in R4 work regressed bt vs chassis at every
magnitude. The new info from R4 (counterparty IDs) does not move the
needle via direct overlay; the R4 win above is purely a chassis
re-tune unlocked by R4 day-3 visibility.

Discovery sweep (2026-04-27):
  HYDROGEL knobs (H_POS_THR/VK_DN_HIGH/VK_UP):  all regressed -676 to -10k
  VFE z-enter / take-cap:                       all regressed
  SIGMA (BS vol) sweep:
    0.23  baseline      0.245  444,666     0.250  451,415
    0.252 452,050 PEAK  0.254  449,134     0.256  448,472
    0.260 447,759       0.270  435,951     0.280  428,874
  WARMUP fine sweep:
    65 440,853  80 441,925  88 442,261  90 442,269 PEAK
    92 442,221  95 442,204 100 442,268 110 441,974
...
class Trader:
    ...
    VFE_SIGMA_FLOOR = 1.0
    VFE_WARMUP_TICKS = 90  # R4 v1: re-tuned on R4 d1+d2+d3 (was 65 on R3 only)
    VFE_Z_ENTER = 2.0
    VFE_MAX_TAKE_PER_TICK = 8
    ...
    SIGMA = 0.252  # R4 v1: re-tuned on R4 d1+d2+d3 (was 0.23 on R3 only)
    VS_TAKE_EDGE = 0.0
    VS_PENNY_EDGE = 1.0
    VS_INV_SKEW = 0.0
    VS_MAX_POST_SIZE = 40
