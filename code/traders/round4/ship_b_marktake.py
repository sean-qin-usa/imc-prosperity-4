...
class Trader:
    ...
    # ----- Mark-follow VFE (replaces Citadel z-score MR in Ship B) -----
    # Mark 67 BUY VFE: 165 events/day, 95.8% hit rate at horizon 1, +1.97 ticks edge
    # Mark 49 SELL VFE: 105 events/day, -1.81 to -2.12 edge across horizons
    # Strategy: target +TARGET when Mark 67 active, -TARGET when Mark 49 active,
    # 0 when neither (auto-flatten). Position duration bounded by cooldown,
    # so no 700k stuck holds possible.
    ...
    VFE_INFORMED_BUYERS = ("Mark 67",)
    VFE_INFORMED_SELLERS = ("Mark 49",)
    ...
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved, ts=0, market_trades=None):
        ...
        if market_trades:
            for t in (market_trades.get("VELVETFRUIT_EXTRACT") or []):
                ...
                if buyer in self.VFE_INFORMED_BUYERS:
                    saved["mark_buyer_until"] = ts + self.MARK_COOLDOWN_TICKS
                if seller in self.VFE_INFORMED_SELLERS:
                    saved["mark_seller_until"] = ts + self.MARK_COOLDOWN_TICKS

        buyer_until = saved.get("mark_buyer_until", -1)
        seller_until = saved.get("mark_seller_until", -1)
        # Most recent active signal wins; both inactive → flatten.
        if buyer_until > seller_until and ts < buyer_until:
            side = +1
        elif seller_until > buyer_until and ts < seller_until:
            side = -1
        else:
            side = 0
        saved["vfe_side"] = side
