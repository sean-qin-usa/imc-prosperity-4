...
class Trader:
    ...
    # ----- Mark-follow VFE (corrected with fill-edge / markout decomposition) -----
    # Mark 67 BUY VFE is bullish after the fill. Mark 49 SELL VFE is also
    # bullish from our POV: his fill edge is positive, but the next mid move is
    # against his sell. Fade Mark 49 instead of following his signed trade.
    # Position duration is bounded by cooldown, so no 700k stuck holds possible.
    ENABLE_CITADEL_VFE_MR = True   # kept name for code compatibility
    VFE_MAX_TAKE_PER_TICK = 8
    MARK_FOLLOW_TARGET = 100        # target VFE position during Mark cooldown
    MARK_COOLDOWN_TICKS = 5000      # extends with each new event
    VFE_BULLISH_BUYERS = ("Mark 67",)
    VFE_BULLISH_SELLERS = ("Mark 49",)
    VFE_BEARISH_BUYERS = ("Mark 49",)
    # Legacy constants for compat with existing code paths
    ...
    def _trade_iv_residual(self, state: TradingState, pos, saved):
        ...
        # ---- Mark-follow VFE (replaces Citadel z-score MR) ----
    ...
    # ---------- Mark-follow VFE (Ship B: replaces Citadel z-score MR) ----------
    def _trade_citadel_vfe_mr(self, od, cur_pos, saved, ts=0, market_trades=None):
        """Replaces Citadel z-score MR with Mark 67/49 follow.

        Position auto-flattens after MARK_COOLDOWN_TICKS of no events. Bounded
        position duration prevents 700k stuck holds (the R3 day-3 sticky).
        """
        if od is None or not od.buy_orders or not od.sell_orders:
            return []

        # Read counterparty trades; extend cooldowns.
        if market_trades:
            for t in (market_trades.get("VELVETFRUIT_EXTRACT") or []):
                ...
                if buyer in self.VFE_BULLISH_BUYERS or seller in self.VFE_BULLISH_SELLERS:
                    saved["mark_bull_until"] = ts + self.MARK_COOLDOWN_TICKS
                if buyer in self.VFE_BEARISH_BUYERS:
                    saved["mark_bear_until"] = ts + self.MARK_COOLDOWN_TICKS

        bull_until = saved.get("mark_bull_until", -1)
        bear_until = saved.get("mark_bear_until", -1)
        # Most recent active signal wins; both inactive → flatten.
        if bull_until > bear_until and ts < bull_until:
            side = +1
        elif bear_until > bull_until and ts < bear_until:
            side = -1
        else:
            side = 0
        saved["vfe_side"] = side

        target = side * self.MARK_FOLLOW_TARGET
        diff = target - cur_pos
        if diff == 0:
            return []
