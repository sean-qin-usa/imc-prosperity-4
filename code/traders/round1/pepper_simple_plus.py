"""Round 1 trader with a simpler but less blunt PEPPER carry leg.

This is a middle ground between `pepper_simple_core.py` and the full
`path_anchor_strategy.py`:

- keep the current ASH_COATED_OSMIUM leg unchanged
- keep a simple PEPPER path/carry anchor
- make PEPPER passive-first on reloads
- only touch PEPPER when it is genuinely cheap to the path
- start unwinding earlier, first passively and then aggressively
...
class Trader:
    ...
    # INTARIAN_PEPPER_ROOT: simpler path/carry model with softer execution.
    ...
    IPR_EARLY_TAKE_MAX_QTY = 8
    IPR_RELOAD_TAKE_QTY = 4

    IPR_TOUCH_CHEAP_EDGE = 0.75
    IPR_TOUCH_NARROW_PATH_CAP = 0.25
    IPR_TOUCH_NARROW_SPREAD_MAX = 2
    ...
    IPR_PASSIVE_BID_BONUS_WHEN_CHEAP = 2

    IPR_NO_RELOAD_START = 92_500
    IPR_SOFT_UNWIND_START = 97_000
    IPR_SOFT_UNWIND_TARGET = 48
    IPR_SOFT_UNWIND_POST_QTY = 8
    IPR_SOFT_UNWIND_HIT_QTY = 8
    IPR_SOFT_UNWIND_ASK_EDGE = 2.0
    IPR_SOFT_UNWIND_HIT_EDGE = 3.0
    ...
    def run(self, state: TradingState):
        ...
        if "INTARIAN_PEPPER_ROOT" in state.order_depths:
            ipr_orders, ipr_anchor = self.trade_ipr_simple_plus(
    ...
    def trade_ipr_simple_plus(
        ...
        if anchor is None or day_reset:
            ...
            anchor = (
                (1.0 - self.IPR_ANCHOR_ALPHA) * float(anchor)
                + self.IPR_ANCHOR_ALPHA * observed_anchor
            )
        ...
        strong_buy_signal = best_ask <= benchmark_path - self.IPR_TOUCH_CHEAP_EDGE
        narrow_buy_signal = (
            spread <= self.IPR_TOUCH_NARROW_SPREAD_MAX
            and best_ask <= benchmark_path + self.IPR_TOUCH_NARROW_PATH_CAP
        )

        if timestamp <= self.IPR_EARLY_TAKE_WINDOW and working_position < self.IPR_EARLY_TAKE_TARGET:
            if strong_buy_signal or narrow_buy_signal:
                qty = min(
                    self.IPR_EARLY_TAKE_TARGET - working_position,
                    self.IPR_EARLY_TAKE_MAX_QTY,
        ...
        if timestamp >= self.IPR_FINAL_UNWIND_START and working_position > self.IPR_FINAL_UNWIND_TARGET:
            unwind_orders, working_position = self._sell_into_bids(
                product,
                buy_orders,
                working_position,
                self.IPR_FINAL_UNWIND_TARGET,
                self.IPR_FINAL_UNWIND_MAX_QTY,
            )
            if unwind_orders:
                orders.extend(unwind_orders)
                sold_this_tick = True
        elif timestamp >= self.IPR_SOFT_UNWIND_START and working_position > self.IPR_SOFT_UNWIND_TARGET:
            soft_excess = working_position - self.IPR_SOFT_UNWIND_TARGET

            if spread > 2:
                ask_price = max(
                    best_bid + 1,
                    min(best_ask - 1, math.ceil(benchmark_path + self.IPR_SOFT_UNWIND_ASK_EDGE)),
                )
                ask_qty = min(self.IPR_SOFT_UNWIND_POST_QTY, soft_excess)
                if ask_qty > 0 and best_bid < ask_price < best_ask:
                    orders.append(Order(product, int(ask_price), -ask_qty))
                    sold_this_tick = True
            elif best_bid >= benchmark_path + self.IPR_SOFT_UNWIND_HIT_EDGE:
                qty = min(soft_excess, self.IPR_SOFT_UNWIND_HIT_QTY, abs(int(buy_orders[best_bid])))
                if qty > 0:
                    orders.append(Order(product, best_bid, -qty))
                    working_position -= qty
                    sold_this_tick = True

        remaining_to_core = max(0, self.IPR_CORE_TARGET - working_position)
        if allow_reloads and (not sold_this_tick) and remaining_to_core > 0 and spread > self.IPR_PASSIVE_BID_DISTANCE:
            bid_price = min(best_bid + self.IPR_PASSIVE_BID_DISTANCE, best_ask - 1)
            bid_base = self.IPR_PASSIVE_BID_SIZE + (
                self.IPR_PASSIVE_BID_BONUS_WHEN_CHEAP if strong_buy_signal else 0
            )
            bid_qty = self._cap_post_size(bid_base, working_position, "buy", remaining_to_core)
            if bid_qty > 0 and bid_price < best_ask:
                orders.append(Order(product, int(bid_price), bid_qty))

        remaining_to_core = max(0, self.IPR_CORE_TARGET - working_position)
        if allow_reloads and remaining_to_core > 0 and (strong_buy_signal or narrow_buy_signal):
            qty = min(
                remaining_to_core,
                self.IPR_RELOAD_TAKE_QTY,
                abs(int(sell_orders[best_ask])),
            )
            if qty > 0:
                orders.append(Order(product, best_ask, qty))
