"""Benchmark-push variant with smaller execution clips.

Goal:
- keep the stronger PEPPER benchmark-push structure
- move both symbols toward the better-supported 5-12 size regime
- test whether lower average fill size survives under the official-hybrid model
"""
...
class Trader:
    ...
    ACO_MAX_POST_SIZE = 10
    ACO_PASSIVE_OFFSET = 3.5
    ACO_MAX_TAKE_SIZE = 10
    ACO_MAX_REDUCE_SIZE = 8
    ...
    IPR_LATE_BAND_QTY = 2
    IPR_PASSIVE_BID_SIZE = 6
    IPR_EARLY_TAKE_WINDOW = 500
    IPR_EARLY_TAKE_CLIP = 8
    IPR_VAR_ALPHA = 0.06
    IPR_BOTTOM_ZSCORE_THRESHOLD = -1.10
    IPR_BOTTOM_EXTRA_QTY = 2
    ...
    def trade_aco_mean_reversion(self, order_depth: OrderDepth, position: int) -> List[Order]:
        ...
        for ask_price, ask_volume in sell_orders.items():
            ...
            if ask_price <= fair_skewed - self.ACO_TAKE_EDGE:
                qty = min(ask_volume, buy_capacity, self.ACO_MAX_TAKE_SIZE)
                if qty > 0:
        ...
        for bid_price, bid_volume in buy_orders.items():
            ...
            if bid_price >= fair_skewed + self.ACO_TAKE_EDGE:
                qty = min(bid_volume, sell_capacity, self.ACO_MAX_TAKE_SIZE)
                if qty > 0:
