"""Final standalone Round 1 submission file.

Selected from the full local strategy ledger as the robustness-first branch:
- `pepper_benchmark_push_core70_completion_early.py`
- promoted `current_strategy.py` target

This copy stays standalone so it can be uploaded directly to the official site
without relying on sibling imports.
"""
...
class Trader:
    ...
    # ASH_COATED_OSMIUM: same fixed-fair leg as the promoted production branch.
