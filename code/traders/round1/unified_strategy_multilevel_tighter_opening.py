"""Round 1 unified candidate with a tighter PEPPER opening residual ladder.

This starts from the stronger late split-ladder candidate and only gives a very
small opening residual to passive PEPPER accumulation:
- visible-take to 60 in the first 500 timestamps
- only the last 4 units get a brief opening `+2/+3` chance
- require enough spread so the residual ladder stays genuinely inside-spread
"""
...
class Trader(_BASE_MODULE.Trader):
    ...
    def trade_ipr_core_band(
        ...
        if residual_opening_phase:
            ...
            if total_bid_qty > 0 and spread > self.IPR_OPENING_PRIMARY_DISTANCE:
                ...
                secondary_bid_qty = 0
                if (
                    spread > self.IPR_OPENING_SECONDARY_DISTANCE
                    and secondary_bid_price < best_ask
                    and secondary_bid_price > primary_bid_price
                    and remaining_target > 1
                ):
                    secondary_bid_qty = min(
                        self.IPR_OPENING_SECONDARY_SIZE,
                        total_bid_qty,
                        remaining_target,
                    )
