"""Official-site PEPPER follow-up probe.

Purpose:
- isolate PEPPER by removing the losing ACO leg
- focus on the only entry style that showed evidence in the official run:
  passive buys 2-3 ticks inside the spread
- hold inventory much longer before flattening to test whether the early-fill
  edge is mainly a fill problem or a premature-exit problem
...
class Trader:
    ...
    # Phase format: (inside_distance, quote_size, target_inventory).
    #
    # The prior official run only filled at +2 and +3 inside, so this probe
    # removes the +1 level entirely and keeps sizes modest to stay diagnostic.
    ...
    IPR_SOFT_REDUCE_START = 90_000
    IPR_FLATTEN_START = 97_000
    IPR_MAX_NET_LONG = 20
    ...
    def trade_ipr_probe(self, order_depth: OrderDepth, position: int, timestamp: int) -> List[Order]:
        ...
        if timestamp >= self.IPR_SOFT_REDUCE_START:
            if position > 10:
                qty = min(position - 10, buy_orders[best_bid])
                if qty > 0:
                    orders.append(Order(product, best_bid, -qty))
            return orders
