"""Round 1 benchmark-focused trader with gentler wide-spread ACO taking.

This file is intentionally standalone so it can be uploaded directly to the
official Prosperity site without depending on sibling strategy files.
"""
...
class Trader:
    ...
    # ASH_COATED_OSMIUM: same fixed-fair leg as the main path-anchor strategy.
    ...
    ACO_WIDE_TAKE_SPREAD = 8
    ACO_WIDE_TAKE_EDGE = 0.5
    ...
    def trade_aco_mean_reversion(self, order_depth: OrderDepth, position: int) -> List[Order]:
        ...
        if spread >= self.ACO_WIDE_TAKE_SPREAD:
            take_edge = self.ACO_WIDE_TAKE_EDGE
    ...
    def trade_ipr_core_band(
        ...
        # Add inventory only on statistically cheap path dips instead of
        # tightening the whole strategy around a mean-reversion filter.
