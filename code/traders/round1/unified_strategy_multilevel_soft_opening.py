"""Round 1 unified candidate with mild PEPPER opening residual ladder.

This starts from the better late split-ladder candidate and only softens the
opening build slightly:
- visible-take to 56 instead of 64 in the first 500 timestamps
- use a short `+2/+3` residual ladder to try to earn the last 8 units
- keep the stronger late-session split passive PEPPER logic unchanged
"""
...
class Trader(_BASE_MODULE.Trader):
    IPR_EARLY_TAKE_TARGET = 56
    IPR_OPENING_RESIDUAL_TARGET = 64
    IPR_OPENING_RESIDUAL_END = 5_000
    IPR_OPENING_PRIMARY_DISTANCE = 2
    IPR_OPENING_SECONDARY_DISTANCE = 3
    IPR_OPENING_PRIMARY_SIZE = 6
    IPR_OPENING_SECONDARY_SIZE = 2
    ...
    def trade_ipr_core_band(
        ...
        if residual_opening_phase:
            ...
            if (
                ...
                and remaining_target > 2
            ):
                secondary_bid_qty = min(self.IPR_OPENING_SECONDARY_SIZE, total_bid_qty, remaining_target)
