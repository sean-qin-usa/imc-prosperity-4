...
class Trader:
    ...
    def _book(self, od: OrderDepth, spread_gate_mid: int) -> Optional[Dict]:
        ...
        # fall back to the touch midpoint.  Two-sided gate (enable micro
        # at tight spreads too) was tested in v3 and regressed PnL by
        # ~8 k; signal is real (r=+0.87 at spread 6-10) but strategy
        # execution can't convert it — the +2-pt fair shift costs more
        # on adverse-selected takes than it gains on forecasted moves.
    ...
    def _trade_aco(self, od: OrderDepth, pos: int, ts: int) -> List[Order]:
        ...
        # End-of-day unwind (ACO): in the last 1 % of day, close residual
        # inventory at touch if the touch is within ACO_UNWIND_EDGE of fair.
        # Avoids carrying ACO inventory overnight where we would be marked
        # at mid but face bid/ask on the next day's open.  Capped at
        # ACO_UNWIND_MAX per tick so we don't move the book against us.
