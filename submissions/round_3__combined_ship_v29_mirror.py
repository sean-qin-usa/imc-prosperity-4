...
triggers parallel directional take on both legs (±280 each, cap=10).

3-day jmerle bt:               **+582,120** (179,750 / 206,445 / 195,924)
3-day match-trades-none floor:  +444,546 (+89,648 over v26's 354,898)

vs v26 (511,088): +71,032 bt / +89,648 alpha-floor
Bundle-cal on 427141: cal_minus_official **+379.9** (best of the ladder;
v25 -5,572, v26 -1,277, v27-mirror-only -1,804, v28 +380).

Live forecast at v25's 1.057× bt-to-live ratio: **~615,300**.

Why V4000/V4500 mirror works
  Both legs deep-ITM delta=1.000 at S=5250, T=5/365, σ=0.23.  Their
  mid moves track VFE mid 1:1 (basis sd 0.83).  Layering the same
  long-EMA z-score signal that drives the VFE Citadel onto V4000/V4500
  multiplies the directional bet from ±200 to ±200+±280+±280 = ±760
  underlying-units per signal (3.8× exposure on the same alpha).

V4500 dominates the new alpha (+53k over 3 days).  Historically had 0
trades / ~18k of synth-MM PnL.  With Citadel-driven ±280 inventory
against visible book (8.9 / side, sp=16), the directional take builds
real take-side PnL.

V4000_TARGET / V4500_TARGET sweep (cap=10):
  100/200 → 526,028   200/200 → 544,918   250/250 → 563,734
  270/280 → 575,170   **280/280 → 582,120**   285/285 → 579,258
  290/290 → 537,236  (cliff: synth-MM blocked at LIMIT margin)

Cap sweep at 280/280:
  6 → 563,986   8 → 577,732   **10 → 582,120**   12 → 579,254
  15 → 569,402  18 → 562,038

Cliffs (do NOT cross)
  - TARGET ≥ 290 (LIMIT margin): synth-MM blocked → -45k+
  - TARGET ≤ 100: under-extracts → -25 to -50k
  - cap ≤ 6: misses Z-cross peaks → -18k
  - cap ≥ 15: whipsaws on local Z noise → -12k

Per-product attribution vs v26 (3-day match-trades-all)
  HYDROGEL:    188,202 → 188,720  +518     (427141 chassis port)
  VFE:         105,366 → 110,162  +4,796   (427141 chassis port)
  V4000:        39,579 →  38,070  -1,509   (synth-MM reshuffle, neutral)
  **V4500:      18,093 →  70,976  +52,883**  ← MIRROR ALPHA
  Other:       unchanged
  Total:       511,088 → 582,120  +71,032

Citadel-HYDROGEL — REJECTED (don't redo)
  Tested target ∈ {10, 20, 30, 50, 75, 100, 200} all regressed -167k
  to -202k.  Static-anchor MM and directional layer compete for
  inventory; the MM needs near-zero pos to post both sides.
  Confirms `feedback_static_anchor_is_the_feature.md`.

Sample-path risk
  3-day VFE drift was -6/+20.5/+28 (net +43).  V4000/V4500 inherit 1:1.
  If live VFE drifts substantially differently the directional bet may
  converge differently.  The +90k alpha-floor jump survives most drift
  patterns; the take-side mid-mtm depends on historical bot patterns.

Conservative fallbacks
  - 427141.py / repo's combined_ship_v27.py (mirror-only on v26 chassis):
    576,528 bt; the V4000/V4500 mirror in isolation
  - 427141.py: 516,679 bt (chassis-only)
  - v25: 497,324 bt, **525,560 verified live** (empirical floor)

Validator-clean (no forbidden imports — re-checked).

Older v27 / v26 / v25 / v24 history below.
==================================================================

Round 3 combined SHIP v27 (now this file's repo content was overwritten;
...
class Trader:
    ...
    # ----- Citadel mirror on delta=1 vouchers (v28 NEW) ---------------
    # V4000 / V4500 have BS delta = 1.000 at S=5250, T=5/365 — track VFE
    # 1:1 (basis mean ≈ 0, sd 0.83 across all 3 days). Same `vfe_side`
    # signal triggers parallel directional take. Peaks: TARGET=280 each,
    # cap=10. Cliff at 290 (synth-MM at LIMIT margin -60k+).
    ENABLE_CITADEL_V4000_MR = True
    V4000_CITADEL_TARGET = 282
    ...
    def _trade_citadel_mirror_v28(self, name, od, cur_pos, saved, target_abs, cap_per_tick):
        """Citadel mirror — V4000/V4500 directional take using vfe_side flag."""
        if od is None or not od.buy_orders or not od.sell_orders:
            return []
        side = int(saved.get("vfe_side", 0))
        if side == 0:
                ...
                for name, K in self.SYNTH_STRIKES.items():
                    ...
                    if od is not None:
                        ...
                        if name == "VEV_4000" and self.ENABLE_CITADEL_V4000_MR:
                            mirror = self._trade_citadel_mirror_v28(
