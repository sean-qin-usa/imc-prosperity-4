...
My research was solid. My approach to the contest was not. I went in optimizing for the kind of stable, real-world-deployable trading I'd built up at Hilbert — wider spreads, conservative inventory skew, structural carry — and treated the contest as a fast version of the same problem. It isn't. Prosperity scores you on one realized path against a curated seed under zero fees and ranked-not-returns scoring, which is a different objective than the one my defaults were tuned for. The gap from there to a top finish is on me, not on the contest design.
...
The audit discipline at each shipping decision — re-examining the four assumptions above, not just iterating on the backtest — is the lever I'd pull harder next time. Item 4 is the canonical one: a single first-principles question on day one of round 1 (*do my MM-sizing defaults actually hold under zero fees?*) would have re-sized every shipped strategy. The other three follow the same shape.

A meta-game observation I'd add to the same checklist: in retrospect, the late rounds rewarded tight-quote market making more than the early rounds did, and my arb-style sleeves (IV-residual MR, family-pair-trade) had less room to generate edge under those conditions. Whether that's a deliberate seed-design choice by IMC or natural drift is an empirical question I haven't quantified yet; either way, the read I should have made by round 3 is "the regime has moved against my strategy class — pivot." I treated round-to-round result variance as noise instead of a regime signal. Adding seed/regime-shift monitoring at each round boundary is the cleanest single piece of next-time discipline.

For next time, the practical fix is a hard ship cutoff well before the 50%-time mark (48-hour rounds are tight, so call it ~16–18 hours of build with the rest as audit), plus a first-principles checklist at each cutoff: fee assumptions, seed robustness, conversion ratios, cross-sleeve interaction, the inherited defaults, and the seed/regime read on which archetype the contest is currently rewarding. The audit is the actual task that closes the gap to a top finish.

What I shipped held up: positive expectancy on every round, a defensible answer on every manual, and the structurally correct strategies built (if not always shipped) for R3, R4, and R5. The gap from there to a top finish is the audit discipline above.
...
The realized path put the EV-optimal portfolio's edge inside the noise band. That's the failure mode of EV-sizing on a single-seed contest — the math is right in expectation; the score is one draw. The point of the manual challenge, on that reading, isn't whether you can price BS; it's whether you size with humility against variance you can't see. Variance-aware sizing is the structural fix, and the R3 manual sizing instinct ((775, 875) beating textbook-optimal (751, 836)) was the right pattern to port — I didn't.
...
The pattern is the same in every case. I had the right structural instinct and chose the safer, locally-validated answer. The gap was conviction, not insight. Conviction here comes from understanding how the contest scores you: knowing the local backtest isn't the live score, knowing variance reduction is what beats an adversarial seed, knowing "first-cut backtest delta" isn't the right rubric in a ranking-not-returns contest. The discipline to ship a structurally correct strategy when the first-cut backtest disagrees is the gap I'd close next time.
...
Zero fees isn't the only mechanic pointing this direction. Under zero fees + ranked-not-returns scoring, the contest rewards tight-quote market making more cleanly than it rewards anything else. The late rounds compressed the room arb-style sleeves had to generate edge, and I should have read that compression as a regime signal by round 3 and pivoted the strategy class harder. I didn't. The lesson is regime-aware re-sizing at each round boundary, not "the contest was against me."
