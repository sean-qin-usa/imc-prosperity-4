...
For next time, the practical fix is a hard ship cutoff well before the 50%-time mark (48-hour rounds are tight, so call it ~16–18 hours of build with the rest as audit), plus a first-principles checklist at each cutoff: fee assumptions, seed robustness, conversion ratios, cross-sleeve interaction, the inherited defaults. The audit isn't validation theater. It's the actual task that closes the gap to a top finish.

For the record: I'm proud of what I shipped. Every round had positive expectancy, every manual had a defensible answer, and I built the structurally correct strategies (including the R5 family-pair-trade and the R4 regime-gated VFE sleeve) even when I didn't ship them. The gap to a top finish wasn't research chops. It was the audit discipline above.
